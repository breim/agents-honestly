...
[The previous chapter](/book/mcp/what-mcp-is) deferred three things here: transports, authorization, and version skew. They look like three topics and they are one, because a single decision determines all of them.
...
**`authInfo` / `ctx.auth` is where the requester comes from.** Not the arguments. This is [the same rule as every in-process tool](/book/tools/tools-are-apis-for-models): the model chooses the subject, your code supplies the requester. The difference is that the requester now arrives as a validated token claim instead of a session variable. The rule did not change; its source did.

**Publishing a server publishes your Part VIII work to strangers.** Descriptions, [namespacing](/book/tools/tools-are-apis-for-models), [result shaping](/book/tools/result-design), [error text](/book/tools/errors-as-instructions): all of it now reaches consumers whose prompts you will never see and cannot fix. A vague description is a support ticket from someone else's product.
...
Cross-call state, where a server genuinely needs it, becomes an explicit server-issued handle passed back as an ordinary tool argument. That is [the parameterized-tool discipline](/book/tools/schema-and-granularity) the protocol arrived at independently.
...
Teams that read the auth spec and start building login flows have misread which half of OAuth they are on, and they are about to own an identity provider they did not want. Related: [Identity](/book/security/identity) and [Least Privilege](/book/security/least-privilege).
</Aside>

There is an agent-specific wrinkle the spec does not solve for you. The token represents a **user**, but the caller is an agent acting on that user's behalf, possibly hours later, possibly after [a run paused three days waiting for an approval](/book/human-in-the-loop/approval-gates). Token lifetimes and agent run lifetimes are not the same clock, and the same mismatch shows up in [dedup windows](/book/tools/idempotency). Decide deliberately whether a resumed run re-authenticates or carries a refreshable credential.
...
Then the operational discipline, which is [the previous part's](/book/tools/tool-discovery) rather than the protocol's: allowlist each server down to the tools you actually use, and treat a server version bump like any other dependency upgrade, because a reworded description changes your selection accuracy without changing a line of your code.
...
**Publishing.** Meridian runs one Streamable HTTP server. OAuth 2.1 with audience binding, and scopes split along [the read/write line](/book/tools/read-and-write-tools): a `crm.read` scope that the sales agent gets, and a `erp.credit` scope that it does not. Stateless, so it sits behind the ordinary load balancer with no special routing.
...
You can build one now, consume one, and authorize it, which makes this a fair moment to ask whether you should. Next: [When a Plain Function Is Better](/book/mcp/when-a-function-is-better), because a protocol boundary you do not need is a protocol boundary that can fail.
