...
Atlas leaves this part holding the tools it started with and far less room to misuse them: taint that stops a write, permissions scoped to the run's own entities, a person's authority carried through, and a registry naming every copy of a customer's data. Next: [AI Governance](/book/production/ai-governance), Part XVIII, on inventory, ownership, evidence, and retiring an agent safely.
