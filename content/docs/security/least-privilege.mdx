...
A minimal permission set is still one permission set, held by an agent that serves everybody. Next: [The Agent Is Not a Superuser](/book/security/identity), on carrying the person's own authority through the run, so it can never read what they could not.
