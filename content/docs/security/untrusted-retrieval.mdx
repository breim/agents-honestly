...
| Model-generated summaries in the index | Your own agent | [Poisoned, potentially](/docs/context/context-poisoning) |

Most production corpora contain at least three rows from the bottom half. And the [ingestion pipeline](/docs/retrieval/ingestion-pipeline) flattens all of them into the same chunk table with the same schema, which is exactly the moment the provenance is lost.
...
**Request-time filtering never sees it.** The [classifier you deployed](/docs/security/prompt-injection) inspects the user's message. The payload is not in the user's message; it arrives from your own database, on the trusted side of every check you wrote.
...
- *"Reranking will drop it."* [Reranking](/docs/retrieval/hybrid-and-reranking) scores relevance, and the document is genuinely, deliberately relevant. Rerankers promote it.
...
This is the same discipline as [metadata-as-authorization](/docs/retrieval/metadata-and-authorization), on the other axis. That chapter was about *who may read this chunk*: confidentiality. This is *who was able to write it*: integrity. They are independent, they are both required, and a chunk table with the first and not the second is half a security model.
...
A corpus is a versioned artifact with the same lifecycle needs as [prompts and graphs](/docs/production/versioning): a manifest of what is in it, a diff between builds, a way to see who introduced a document, and a rollback. If your answer to "what changed in the index last Tuesday" is a shrug, you cannot investigate this class of incident at all.
...
Next: [Excessive Agency and Least Privilege](/docs/security/least-privilege), scoping what an agent can do to the smallest set that still does the job.
