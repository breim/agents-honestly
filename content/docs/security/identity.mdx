...
Everything the previous chapter asked for is expressible here: the scope is the [minimum mined from traces](/docs/security/least-privilege), the expiry makes it a run-scoped credential, and `act` is what turns "the service account did it" into "Atlas did it for user 8841 during run 4471."
...
Suppose you keep the service credential for retrieval but filter the *results* by the user's access before showing them. Authorization now happens, and the system is still wrong in three ways, all of which [the retrieval chapter](/docs/retrieval/metadata-and-authorization) established and which generalize past retrieval:
...
- The model saw it. A summary, a [memory write](/docs/context/memory), or a cached intermediate now carries content the user cannot access, and the filter does not follow it there.
...
Everything above is standard practice and would be unremarkable, except that agents break its central assumption. Token exchange assumes a request-scoped flow: a user token arrives, you exchange it, you call the backend, you respond. Agents [pause for approval](/docs/human-in-the-loop/approval-gates), run for hours, [resume from a checkpoint](/docs/langgraph/persistence) three days later, and execute on schedules with no user present at all.
...
Row three is where correctness usually gets lost. A run that paused on Monday and resumes on Thursday must not simply replay Monday's authorization. The person may have changed roles, left the company, or had the relevant grant revoked. An agent that resumes with Monday's rights is a revocation that silently didn't happen, the same defect as [caching group membership in an index](/docs/retrieval/metadata-and-authorization).
...
Next: [Multi-Tenant Isolation](/docs/security/tenant-isolation), RBAC, ABAC, row-level security, and retrieval filters as an authorization boundary.
