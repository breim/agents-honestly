...
Identity settles what one user may reach. It does not settle whose data a run is allowed to touch. Next: [Multi-Tenant Isolation](/book/security/tenant-isolation), where a filter, a cache key and a resumed run all have to agree about which customer this is.
