...
Atlas serves eleven customers from one deployment. The retrieval filter is correct, and [Part IV spent a chapter on why](/book/retrieval/metadata-and-authorization): `tenant_id` on every chunk, the principal from the live token, the model unable to touch the parameter.
...
| Vector index | Corpus chunks | [Covered in Part IV](/book/retrieval/metadata-and-authorization) |
...
| **Memory / fact store** | Extracted facts across sessions | A fact learned from A [surfaces for B](/book/context/memory) |
...
**The eval set.** [Datasets built from production runs](/book/evals/datasets) are exactly the right practice, and they turn one customer's data into a permanent artifact in a repository with different access rules than the system it came from. Sample deliberately, scrub, and record consent. Not because the eval is dangerous, but because it outlives everything around it.
...
**The reason is returned.** A deny carrying *"account not in the ticket's scope"* is [an escalation](/book/human-in-the-loop/approval-gates) rather than an error, which is the difference between a control operators work with and one they route around.
...
Tenancy propagates fine through a synchronous request. Agents are not synchronous. A run [pauses for approval](/book/human-in-the-loop/interrupts-vs-signals), resumes on a different worker in a different process days later, calls out to a [Temporal activity](/book/durability/agent-as-workflow) that retries on a third machine, and emits a trace from a background exporter. Every one of those hops is a place the tenant can be lost. Losing it usually means falling back to a default, which means falling back to *no filter*.

Three rules keep it attached:

**Tenant is part of the run's identity, not its context.** It goes in the workflow ID and the thread key, where it is structurally present, rather than in a mutable state field that a [reducer](/book/langgraph/reducers-and-state) could drop.

**Resumption re-derives, never restores.** On resume, the tenant comes from the run's identity and the [delegation is re-minted](/book/security/identity). A checkpoint is data; treating the tenant in it as authoritative means a corrupted or hand-edited checkpoint is an authorization bypass.

**Every fan-out carries it explicitly.** A [sub-agent](/book/patterns/context/subagent-isolation), a parallel tool call, a background summarizer, a trace exporter: each gets the tenant as a required argument. Ambient context propagation via thread-locals or async-locals works until an executor boundary, and then it silently doesn't.

## Test the boundary, not the feature

Isolation is one of the few properties in agentic systems that admits a hard, deterministic test, and [the retrieval chapter's negative sets](/book/retrieval/metadata-and-authorization) generalize to every store on the list.
...
**Fault injection with a wrong tenant.** [Failure injection](/book/reliability/failure-injection) applied to authorization: resume a checkpoint under a different tenant and assert the run refuses to continue. This is the one that catches restored-rather-than-re-derived resumption.
...
| Temporal workflows | Tenant in the workflow ID | Structural; also gives per-tenant [queue routing](/book/patterns/scale/tenant-fairness) |
...
Every boundary so far has been drawn around data. Next: [Sandboxing and Credential Boundaries](/book/security/sandboxing), on where code runs and where secrets live, and making certain those are never the same process.
