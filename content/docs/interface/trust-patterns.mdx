...
There are two ways to be wrong. **Under-trust** produces disuse: the agent is correct and ignored, and the investment returns nothing. **Over-trust** produces misuse: the agent is wrong and believed, which is [the confident-wrong-answer failure](/book/retrieval/sql-is-still-the-answer) this book has been circling since Part IV, now arriving through the interface.
...
That is not an argument against citations, but it does change the argument *for* them. It is the measured case for [the verification the retrieval part builds](/book/patterns/retrieval/grounded-citations) rather than trusting the marker. Three reasons survive:

**They are checkable by the people who matter.** Not the user in the flow: [the reviewer at an approval gate](/book/human-in-the-loop/approval-gates), the auditor eleven months later, the engineer during an incident. Those readers do check, and for them the link is the whole artifact.

**They impose discipline on the system.** A pipeline that must produce a citation cannot answer from nothing. Much of the value is extracted before the interface exists, in [the retrieval layer that now has to return provenance](/book/tools/result-design).

**They are the difference between a claim and an assertion in the record.** [The audit trail](/book/human-in-the-loop/escalation-and-audit) needs the source; the UI is just where it also happens to be visible.
...
[Streaming UX](/book/interface/streaming-ux) covered the mechanics. The reason it belongs in a chapter about trust is a distinct failure mode with a good name: **the black-box launch**, a clean interface showing inputs and outputs and nothing in between.
...
But an interface built over [Part XI](/book/durability/compensation) has to be honest about a distinction most UIs collapse:
...
Showing an Undo button on an email that has been delivered is a lie the interface tells on behalf of the system. The button must inherit its label from [the tool's reversibility class](/book/tools/read-and-write-tools), which means the catalogue's classification reaches all the way to the front end. If it does not, someone will add a hopeful Undo to a class ⑤ write.
...
That is the same principle as [preferring an architecture whose failure mode is a refusal](/book/retrieval/sql-is-still-the-answer), surfaced. A stated reason for hesitating is checkable; a percentage is not.

## Failure states, the most under-designed surface

Three requirements, and most agent interfaces satisfy none of them:

**Say what failed, in the user's terms.** Not "something went wrong," and not a stack trace. [The error taxonomy from Part VIII](/book/tools/errors-as-instructions) has already produced text written for a consumer; the UI's job is not to discard it.
...
Next: [Eval Discipline](/book/evals/eval-discipline), Part XIV, and the question this part cannot answer: whether any of it is actually working.
