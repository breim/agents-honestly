...
[The AI SDK chapter](/book/interface/ai-sdk) ended on a rule: one loop, and it lives wherever your durability lives. This chapter is the case where that rule gets hard, because the AI SDK now ships a durability runtime of its own.
...
Read that list against [the previous two chapters](/book/interface/streaming-ux) and it is conspicuous: **it is the durable-run-plus-reconnectable-projection architecture, shipped as a feature.** Everything Part XIII has been describing as something you assemble is here as something you import.
...
- [Two loops](/book/interface/ai-sdk): the SDK's and the workflow's, with different step counts.
- [Two checkpointers](/book/human-in-the-loop/interrupts-vs-signals): LangGraph's and Temporal's, with different ideas of position.
...
**Choose Temporal when** waits are measured in weeks, retry and backoff behaviour is complex, the system is polyglot, the agent is one part of a larger set of business processes, you need the event history as an [audit artifact](/book/human-in-the-loop/escalation-and-audit), or you already run it for other things.
...
Use the AI SDK for what has no competitor: generation, streaming, `useChat`, the transport boundary, [tool-call rendering](/book/interface/generative-ui-and-approvals). Let the workflow own steps, retries, timers, and recovery. Do not use `WorkflowAgent`.
...
One runtime owns durability and the other renders, and nothing yet connects a browser to either. Next: [Wiring the UI to a Durable Backend](/book/interface/ui-to-durable-backend), start, signal, query, reconnect, over runs that outlive the tab.
