...
The pieces are specified, and they come from two different parts, which is the reason this chapter exists. Part XI supplied [a run with a business identity](/docs/durability/not-a-background-job) and [a validated acknowledgement in one round trip](/docs/durability/latency); Part XIII has supplied [a buffered stream that survives a refresh](/docs/interface/streaming-ux) and [rendered tool calls](/docs/interface/generative-ui-and-approvals). This chapter connects them, and the connecting layer turns out to carry the security of the whole design.
...
| --- | --- | --- | --- |
| Opens a ticket that has no run | **Start** | [Update-with-start](/docs/durability/latency) | A validated acknowledgement, in one trip |
...
This is the same rule as [the requester rule from Part VIII](/docs/tools/tools-are-apis-for-models), one layer out: **the caller proposes the subject, your code supplies the authority.** The pattern has now appeared at the tool boundary, the policy layer, and the HTTP layer, which is a reasonable sign that it is the actual invariant of the whole system.
...
The fix is the same [stream-first ordering](/docs/interface/streaming-ux) this book has now needed three times:
...
**Do not hold the request open for the run.** [Eight minutes against a sixty-second proxy timeout](/docs/durability/not-a-background-job). The whole of Part XIII exists because of this.
...
Next: [Interface Patterns for Trust](/docs/interface/trust-patterns). Provenance, progress, undo, confidence, failure states: the vocabulary that makes an agent usable by sceptics.
