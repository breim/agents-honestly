...
Next: [LangGraph on Temporal](/book/durability/langgraph-on-temporal). The official plugin: choosing `execute_in` per node, what may run inside a workflow, and what must not.
