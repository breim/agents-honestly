...
[Atlas v0](/book/first-agent/the-loop-by-hand) was about eighty lines: a `for` loop, three bounds, a union of outcomes. This chapter is the same program on durable execution, and the interesting part is not the port. It is the five things the port forces you to change, each of which was a latent bug in the original.
...
[Result design](/book/tools/result-design) said to shape at the source rather than cut at the boundary. Here that advice becomes structural: the truncation, the field projection, the row cap all happen **inside the activity, before it returns**, so the oversized payload never enters the journal at all.
...
Atlas keeps the transcript inline for now, because one ticket is bounded. [Long-Lived Agents](/book/durability/long-lived-agents) is where it stops being bounded and where continue-as-new enters.
...
Note also that the tokens leave through a side channel rather than the return value. An activity returns once, at the end; live output needs a separate path, which is [Part XIII's subject](/book/interface/ui-to-durable-backend).

## 5 · Everything else stayed the same, deliberately

Parallel tool calls remain parallel. Awaiting several activities concurrently is deterministic, because the results come back from history in a recorded order rather than a raced one. The [read/write split](/book/tools/read-and-write-tools) still applies inside the dispatcher: reads fan out, writes serialize.
...
Event histories are durable, exported, and retained. If a tool argument or a tool result carries personal data, that data now lives in an operational log with its own retention policy, which is a compliance question rather than an engineering one. The mitigations are the usual ones: pass identifiers rather than records, fetch the sensitive parts inside the activity, and configure a data converter that encrypts payloads. [Part XVII](/book/security/pii-and-compliance) is where they are argued properly.
...
[Part II ended with a table](/book/first-agent/the-loop-by-hand) of seven decisions a framework would have made silently. Here is what owns each one now:
...
Durable execution made the **machine** reliable. The [six failure clusters](/book/durable-execution/everything-that-breaks) that belong to the model are untouched, and a system with a perfect event history and a bad tool catalogue fails every ticket identically, forever, with an immaculate record of having done so.
...
Atlas is a workflow now, and it was a graph two parts ago. Next: [LangGraph on Temporal](/book/durability/langgraph-on-temporal), on whether it should be both, and what it costs to satisfy two determinism models at once.
