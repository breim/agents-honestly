...
**A deploy kills it.** Every run in flight is executing code that is about to stop existing, [the failure nobody counts](/docs/durable-execution/everything-that-breaks) because it is a normal Tuesday.

**Retrying means starting over.** The client's retry button re-runs from step zero, which re-runs the refund. [The third state](/docs/tools/idempotency) arrives at the HTTP layer, where it is even less visible.
...
| Output | One result | An [outcome union](/docs/first-agent/the-loop-by-hand): answered, escalated, halted |
...
Which is precisely the thing [Part X spent five chapters building](/docs/durable-execution/temporal-in-forty-minutes), and the mapping is one-to-one:
...
| To be started by an event that may arrive twice | [Signal-with-start](/docs/durable-execution/signals-and-children) |
| To accept an approval and report whether it took | An [update](/docs/durable-execution/signals-and-children) |
...
A job's answer is a status field polled every few seconds. An agent's answer is [intermediate output](/docs/langgraph/streaming-and-control): what it is doing, what it found, what it is about to do. That is a product requirement rather than a nicety, because a system that says nothing for eight minutes is indistinguishable from a broken one.

The mechanism is a stream decoupled from the compute: the run publishes events, the client subscribes, and the connection carrying the stream is not the connection running the agent. [Part XIII](/docs/interface/ui-to-durable-backend) builds it; the reason it needs building is this chapter.
</Aside>
...
- **Model calls and tool calls are activities**, so a crash does not re-run the ones that completed. That port is [the next chapter](/docs/durability/agent-as-workflow).
...
Next: [The Agent as a Workflow](/docs/durability/agent-as-workflow), Atlas ported: the loop becomes a workflow, every model call and tool becomes an activity.
