...
A three-week thread with tool calls and model responses runs into the ceiling [Part X named](/docs/durable-execution/signals-and-children): **51,200 events or 50 MB per execution.** Both are reachable, and the payload figures make it concrete. Ten substantial conversational turns approach 40 KB of raw content before Temporal's own per-event metadata.
...
That last one deserves emphasis because it is a design constraint rather than an API detail: **an entity workflow under continuous load cannot recycle its history.** The accumulator above helps by batching bursts into single runs, which is fewer events. But a genuinely relentless stream needs the work partitioned across [child workflows](/docs/durable-execution/signals-and-children) instead.

## What crosses the boundary

Continue-as-new hands the new execution one thing: its input. So the design question is what state deserves to survive, and this is where [the transcript problem](/docs/durability/agent-as-workflow) deferred from two chapters ago comes due.
...
**A summary.** Compact, lossy, and it forces you to decide what mattered. This is exactly [the compaction decision from Part III](/docs/context/compaction), arriving with a deadline attached. The wall makes you choose, where before you could postpone.
...
Atlas carries the third, plus a rolling summary for the model's context, because the store answers *"what was said"* and the summary answers *"what matters,"* and those are [different questions](/docs/context/memory).
...
Anything measured against the case rather than against the current execution must be an absolute timestamp carried forward. That covers SLA clocks, retention windows, and escalation ladders. And [the SLA timer pattern](/docs/patterns/durability/sla-timer) is this rule with the cancel-and-re-arm machinery attached.
...
| Children | None, so the [parent-close policy trap](/docs/durable-execution/signals-and-children) does not apply, which is worth confirming rather than assuming |
...
Next: [When Tools Have Side Effects](/docs/durability/compensation), the saga pattern for refunds, emails, and inventory, plus the "we already sent it" problem.
