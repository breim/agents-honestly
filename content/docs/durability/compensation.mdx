...
Not in the workflow. In the catalogue, as part of what it means to be a [class ③ or above tool](/docs/tools/read-and-write-tools). A tool without a declared compensation is a tool that cannot participate in a saga, and that should be a property you can check rather than something discovered during an incident.
...
Which is why the [event history](/docs/durability/agent-as-workflow) matters more here than anywhere else in Part XI. It is the authoritative record of *which* effects actually occurred in *this* run, and that record is precisely what has to be unwound.
...
Some steps have no compensation, and pretending otherwise is the anti-pattern this chapter exists to name. You cannot un-send an email. The [class ⑤ writes](/docs/tools/read-and-write-tools) are mail, webhooks, messages, anything already in someone else's system. They are irreversible not because the API lacks a delete but because **a human has already read it.**
...
They are activities, so everything from [Part X](/docs/durable-execution/determinism-and-retries) applies, with three adjustments:
...
**Idempotent, always.** A compensation may be retried by policy, and the whole workflow may replay. Releasing the same hold twice must be a no-op, the same [derived-key discipline](/docs/tools/idempotency) as the forward steps, because compensations are writes.
...
Next: [Scale and Quality of Service](/docs/durability/scale-and-qos), fan-out across thousands of cases, provider quotas, priority queues, and fairness across tenants.
