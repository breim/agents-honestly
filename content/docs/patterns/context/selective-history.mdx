...
Every other approach to this is wrong in a different way. Sending all 240 turns is expensive and hits the window. [Compacting](/docs/patterns/context/context-compaction) or [summarizing](/docs/patterns/context/rolling-summary) will have compressed turn 31 into something like *"discussed logistics"* long ago, because at the time it was compressed nobody knew which turn would matter. Keeping the last N verbatim drops it entirely.
...
- **Retrieved turns land mid-context**, where [attention is weakest](/docs/foundations/context-window).
...
**Put retrieved turns after the stable prefix and before the recent window.** They are the part that changes per request, so they must sit below the [cacheable region](/docs/patterns/context/stable-prefix), and they are the part most at risk of being [lost in the middle](/docs/foundations/context-window), which the explicit label partly mitigates.
...
The `conversationId` filter is not an optimization. It is the same [metadata-as-authorization](/docs/retrieval/metadata-and-authorization) rule as any other index: a history index without it will eventually surface one customer's turn in another customer's conversation, and it will do so as a fluent paraphrase with no access log.
...
**Retrieval can miss.** This is the pattern's characteristic failure and it is worse than it sounds, because a miss presents as the assistant forgetting something the user knows it was told. Mitigate as you would any retrieval: [hybrid search](/docs/retrieval/hybrid-and-reranking) so exact strings and IDs match lexically, and a generous recent window so the common case never depends on retrieval at all.
...
**Index maintenance.** Another store that must be [tenant-scoped, retained deliberately, and reachable by an erasure request](/docs/security/pii-and-compliance). It is a full-fidelity copy of every conversation, which is exactly the kind of store [the compliance chapter](/docs/security/tenant-isolation) warns gets built without anyone deciding to build it.

## When not to use it

**Short conversations.** Under a few dozen turns, send them all. This is the most common over-engineering in this group.

**Task runs rather than conversations.** A run that investigates a ticket over twenty turns has no useful "past" to retrieve: everything is recent and everything is relevant. Use a [scratchpad](/docs/patterns/context/structured-scratchpad) and [compaction](/docs/patterns/context/context-compaction).

**When the facts, not the turns, are what matter.** If what you need from turn 31 is *the address*, the right home is [a fact store](/docs/context/memory) or the scratchpad, not a retrieval over conversation text. Retrieving turns to recover a value is a workaround for state that should have been typed.

**When exactness is not required.** If a summary would genuinely do, a [rolling summary](/docs/patterns/context/rolling-summary) is cheaper and simpler.

<Aside type="note" title="This is the memory ambiguity, made concrete">
[Three unrelated things are called memory](/docs/appendix/glossary), and this pattern sits precisely on the seam between two of them: it is *retrieval* applied to *the transcript*.
...
- [Rolling Summary](/docs/patterns/context/rolling-summary): compress instead of retrieve, when exactness is not needed
- [Context Compaction](/docs/patterns/context/context-compaction): rebuild at a boundary, for task runs rather than conversations
- [Structured Scratchpad](/docs/patterns/context/structured-scratchpad): where a *value* from turn 31 should have lived
- [Retrieval on Demand](/docs/patterns/context/retrieval-on-demand): the same idea, with the agent deciding when to search
- [Memory](/docs/context/memory): the three things that share the word, and which one you actually need
