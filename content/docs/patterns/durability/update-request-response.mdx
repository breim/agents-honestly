...
[A signal](/book/patterns/durability/signal-with-start) cannot do this. It is fire-and-forget: the client gets an acknowledgement that the signal was *delivered*, not that it was *acted on*. So the console posts the approval, gets a 202, and then has to answer "did it work?" some other way.
...
- **Invalid requests should not pollute the history**, which is [size-limited](/book/patterns/durability/continue-as-new-memory) and permanent.
...
**Authorization belongs in the handler, not the validator.** [Authority is re-derived at the moment of the action](/book/security/identity), and that usually needs an activity, which validators cannot call. Validate shape cheaply, then check authority where you can do real work.

**Keep the handler bounded.** An update whose handler waits on a human is a request that hangs for three days. Use an update to *record* a decision and return quickly; use [the approval gate](/book/patterns/durability/approval-gate) for the waiting.
...
**Client disconnects do not cancel the work.** The update proceeds and completes; the caller simply does not see the result. Design for that: the caller should be able to re-ask (via a [query](/book/durable-execution/signals-and-children)) rather than assume failure and retry, which would be a second update.
...
**When the caller does not need a result.** A customer reply webhook has nobody waiting. Use [signal-with-start](/book/patterns/durability/signal-with-start), which is cheaper and can create the workflow.
...
**When the handler has to wait on a human.** That is an [approval gate](/book/patterns/durability/approval-gate). Use an update to *deliver* the human's decision and return immediately; the waiting is the workflow's job, not the caller's.
...
The tell that you picked wrong: if your client sends something and then polls to find out what happened, you wanted an update. If it polls to watch state change over time, you wanted a query on a schedule. Or, more likely, [a signal into the workflow so the workflow can act](/book/durable-execution/signals-and-children) rather than the client watching.
</Aside>

## Related

- [Signal With Start](/book/patterns/durability/signal-with-start): the fire-and-forget sibling, and the one that can create the workflow
- [Approval Gate](/book/patterns/durability/approval-gate): where the waiting belongs; the update only delivers the decision
- [Agent as Entity Workflow](/book/patterns/durability/agent-as-entity-workflow): the running execution being addressed
- [Signals and Children](/book/durable-execution/signals-and-children): the three ways to talk to a running execution
- [Generative UI and Approvals](/book/interface/generative-ui-and-approvals): the console on the other end of this call
