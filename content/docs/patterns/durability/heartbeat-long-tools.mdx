...
So a failure that happened at second four is discovered at second one hundred and twenty. The customer waits two minutes for a retry that could have started immediately, and the [run deadline](/docs/reliability/timeouts-and-retries) burns a third of its budget on a worker that no longer exists.
...
**Do not confuse it with the resumption checkpoint.** The same mechanism carries a payload used by [resumable activities](/docs/patterns/durability/resumable-activity), but detection needs no payload at all. Heartbeat for liveness on every long activity; carry details only when resumption is actually implemented.
...
Heartbeating on token arrival gives a second property for free: it is also [the inactivity signal](/docs/reliability/timeouts-and-retries) the streaming chapter asked for. A stream that stalls mid-response stops heartbeating, so a stalled provider and a dead worker are detected by the same mechanism at the same latency, which is a rare case of one control covering two failure modes without compromise.

## Trade-offs

**Overhead per ping.** Heartbeats are cheap and throttled, so the cost is negligible at sane intervals and real if you ping in a tight loop over thousands of items. Batch them, as [the resumable-activity pattern](/docs/patterns/durability/resumable-activity) does.
...
**Outside durable execution.** Without a service tracking activity liveness there is nothing to heartbeat *to*. The equivalent is [the inactivity timeout](/docs/reliability/timeouts-and-retries) on the stream itself, which every agent should have regardless.
...
An activity that never checks in cannot be told to stop. So when the workflow hits its deadline, when the run's [budget is exhausted](/docs/production/cost-engineering), when the customer cancels, or when a [breaker opens](/docs/reliability/fallbacks-and-breakers): a silent long-running model call keeps generating tokens against a result nobody will read, and keeps doing so until its own timeout.
...
- [Resumable Activity](/docs/patterns/durability/resumable-activity): the same mechanism carrying a checkpoint payload
- [Tool as Activity](/docs/patterns/durability/tool-as-activity): where the per-activity timeouts are configured
- [Timeouts, Retries, and Backoff](/docs/reliability/timeouts-and-retries): the inactivity timeout this mirrors outside durable execution
- [Agent as Entity Workflow](/docs/patterns/durability/agent-as-entity-workflow): the workflow whose deadline the cancellation comes from
- [Determinism and Retries](/docs/durable-execution/determinism-and-retries): the four timeouts, and the two you set
