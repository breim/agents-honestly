...
**Make the repeated slice safe.** Between the last heartbeat and the crash, some work happened and was not recorded, so it *will* be redone. If those items have side effects, they need [idempotency keys](/book/reliability/idempotency-in-practice) or the pattern converts a lost hour into duplicated writes.
...
**Resumption logic is a second code path.** The resume branch runs rarely and is therefore [the least-tested code in the activity](/book/reliability/failure-injection). Injecting a worker kill at minute thirty is the test that proves it works, and it belongs in CI.
...
**When the batch is genuinely parallel.** Four thousand independent classifications are a [fan-out](/book/patterns/scale/fan-out) or a [map-reduce tree](/book/patterns/scale/mapreduce-tree), not one long activity. Each item becomes its own activity, each retries independently, and a redeploy loses only what was in flight.
...
- [Heartbeat for Long Tools](/book/patterns/durability/heartbeat-long-tools): the same mechanism used for detection rather than resumption
- [Tool as Activity](/book/patterns/durability/tool-as-activity): why payloads should be references
- [Batch Iterator](/book/patterns/scale/batch-iterator): walking a large set as a workflow rather than an activity
- [Fan-Out](/book/patterns/scale/fan-out): the parallel alternative when the work is independent
- [Failure Injection](/book/reliability/failure-injection): killing the worker mid-activity, which is how the resume path gets tested
