...
- **The timer must survive** deploys, crashes, and [continue-as-new](/book/patterns/durability/continue-as-new-memory).
...
**Store the deadline, derive the timer.** State holds an absolute timestamp; the sleep duration is computed from it at arming time. The reverse, storing "9 days remaining," is wrong after any pause and meaningless after a [continue-as-new](/book/patterns/durability/continue-as-new-memory), where the new run has no idea when the old one started.
...
`computeDeadline` being an activity is the subtle part. Business-hours arithmetic reads a calendar, the calendar changes, and workflow code that reads it directly produces a different answer on replay than it did originally. That is [divergence](/book/durable-execution/determinism-and-retries), and it will surface as a workflow task failure months after the code was written.

## Trade-offs

**Timers are cheap but not free.** Each pending timer is a scheduled event on the service. Thousands of tickets each holding one is normal and worth knowing about; each recomputation adds cancel-and-arm events to the [history](/book/patterns/durability/continue-as-new-memory), so a ticket with a chatty customer accumulates them.
...
**When the run is short.** A deadline shorter than the run's typical duration belongs to the [run budget](/book/reliability/timeouts-and-retries), not to an SLA mechanism.
...
**When you would use it to poll.** A timer that fires every five minutes to check something is a poll wearing a timer's clothes. Wait on the event itself, or use a [signal](/book/patterns/durability/signal-with-start).
...
- [Approval Gate](/book/patterns/durability/approval-gate): the other thing the workflow races a timer against
- [Signal With Start](/book/patterns/durability/signal-with-start): how the customer reply that resets the clock arrives
- [Continue-As-New for Memory](/book/patterns/durability/continue-as-new-memory): why the deadline must be absolute
- [Agent as Entity Workflow](/book/patterns/durability/agent-as-entity-workflow): the long-lived workflow that holds the deadline
- [Signals and Children](/book/durable-execution/signals-and-children): the mechanics of waiting on one of several things
