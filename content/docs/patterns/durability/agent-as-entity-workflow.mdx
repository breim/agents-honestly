...
This is the [Entity Workflow](https://temporal-sa.github.io/temporal-design-patterns/) pattern applied to agents. It composes with [Continue-As-New for Memory](/docs/patterns/durability/continue-as-new-memory), [Signal With Start](/docs/patterns/durability/signal-with-start), and [Updatable SLA Timer](/docs/patterns/durability/sla-timer). In practice you will use all four together.
</Aside>
...
**You pay in event history.** Every signal, activity, and timer is persisted. Agent turns are chatty, so history grows fast, which is why [Continue-As-New for Memory](/docs/patterns/durability/continue-as-new-memory) is not optional here but part of the pattern.

**Latency floor rises.** Each activity round-trip adds a few milliseconds of persistence. Irrelevant against a multi-second model call; noticeable if you wrap something that took 200 microseconds. See [Local Activities](/docs/patterns/durability/tool-as-activity).
...
**Workflow versioning is now your problem.** Changing the agent loop while thousands of instances are mid-flight requires Temporal's patching mechanism. Covered in [Deploying and Versioning Agents](/docs/production/versioning). Plan for it before you need it, not after.
...
- [Continue-As-New for Memory](/docs/patterns/durability/continue-as-new-memory): required companion once history grows
- [Signal With Start](/docs/patterns/durability/signal-with-start): deliver the first message without a separate "does it exist" check
- [Request-Response via Update](/docs/patterns/durability/update-request-response): ask a running agent something and get a validated answer back
- [Updatable SLA Timer](/docs/patterns/durability/sla-timer): deadlines that move when new information arrives
- [Saga for Tool Side Effects](/docs/patterns/durability/saga-for-tools): when the refund succeeded and the email didn't
- Chapter: [Long-Lived Agents](/docs/durability/long-lived-agents): the narrative walkthrough this entry compresses
