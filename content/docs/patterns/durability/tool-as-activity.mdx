...
The agent loop is running inside a [durable workflow](/book/patterns/durability/agent-as-entity-workflow), and someone calls the payments API directly from workflow code.

It works in development. In production it fails in a way that is hard to even describe: the worker crashes at step nine, Temporal [replays the workflow from its history](/book/durable-execution/determinism-and-retries) to rebuild state, and the replay *calls the payments API again*, because there is no record of the first call in the history, only the code that made it. The credit is issued twice, and the second one has no event anywhere saying why.
...
- **Retry policy is per-operation.** A read retries freely; [an irreversible write does not](/book/tools/read-and-write-tools).
- **Every boundary costs a payload round trip** through the history, which is [size-limited](/book/patterns/durability/continue-as-new-memory).
...
`policy_for(tool)` deriving timeouts and retry rules from the tool class is what keeps this from becoming forty hand-tuned constants. The class is already recorded in the catalogue for [security reasons](/book/security/least-privilege); reuse it.

## Trade-offs

**Payload round trips.** Every argument and result serializes into the history. This is the cost that bites first on agents, because [the transcript is the payload](/book/durability/agent-as-workflow). Passing the whole context to the model activity on every turn writes it to history on every turn. Pass a reference to state held outside, or you will hit the size limit long before the event count.
...
**Two places to look when debugging.** The workflow shows the decisions; the activity shows what happened. Good tracing joins them under one run ID, and [without that join](/book/observability/what-a-trace-must-answer) an investigation crosses a boundary with nothing linking the sides.

**Non-retryable classification is load-bearing.** Getting it wrong in one direction burns money retrying a permanent failure; in the other it fails a run that would have succeeded on the second attempt. This is [the error taxonomy](/book/reliability/error-taxonomy) with real consequences attached.
...
**When you are not using durable execution.** Outside a workflow, this is just "call your tools through a dispatcher", which [you should do anyway](/book/production/reference-architecture), without the activity machinery.
...
- [Agent as Entity Workflow](/book/patterns/durability/agent-as-entity-workflow): the workflow this pattern lives inside
- [Heartbeat for Long Tools](/book/patterns/durability/heartbeat-long-tools): detecting a hung activity before its timeout
- [Resumable Activity](/book/patterns/durability/resumable-activity): restarting a long activity from a checkpoint
- [Saga for Tool Side Effects](/book/patterns/durability/saga-for-tools): undoing activities that succeeded before a later failure
- [Determinism and Retries](/book/durable-execution/determinism-and-retries): what replay actually compares, and the four sources of divergence
