...
- **Duplicate delivery is normal.** Webhooks are [at-least-once](/docs/reliability/idempotency-in-practice).
...
The `_seen` set is not optional. Webhook providers retry on any non-2xx and occasionally on success, so the same reply arrives more than once. Unlike a [tool call](/docs/reliability/idempotency-in-practice), a duplicated inbound message does not fail loudly. It makes the agent answer the same question twice.
...
**The workflow ID becomes a public contract.** Anything that can construct `ticket-9104` can signal it, so signal payloads are [untrusted input](/docs/security/threat-model) and the sender's authority is checked at the API boundary, not inside the workflow.
...
**When you need a response.** Signals are fire-and-forget. If the caller has to know what the agent decided, that is [an update](/docs/patterns/durability/update-request-response), which is a different operation and now generally available.
...
Initialize anything a handler touches in the constructor or in the handler itself. The same caution applies after a [workflow reset](/docs/durable-execution/signals-and-children), where preserved signals are replayed against a workflow that has not initialized yet.
</Aside>

## Related

- [Agent as Entity Workflow](/docs/patterns/durability/agent-as-entity-workflow): why the entity ID is the workflow ID
- [Updatable SLA Timer](/docs/patterns/durability/sla-timer): the reply that arrives here is what resets the clock
- [Request-Response via Update](/docs/patterns/durability/update-request-response): when the caller needs an answer back
- [Approval Gate](/docs/patterns/durability/approval-gate): the same signal mechanism, waiting on a person instead of a customer
