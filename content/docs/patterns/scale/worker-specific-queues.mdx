...
Each step is [an activity](/book/patterns/durability/tool-as-activity), and activities are dispatched to whichever worker polls first. So step one downloads the file to worker A's local disk, step two runs on worker C, which does not have it, and either fails or downloads it again.

The workaround everyone reaches for is to push the file through the activity boundary, which fails sooner and harder than people expect: a single payload is capped at 2 MB, so a 200 MB file is rejected outright rather than bloating the [event history](/book/patterns/durability/continue-as-new-memory). The other workaround is object storage round trips, which is correct and costs two transfers per step for data that was already sitting on a local disk.
...
**Handle the pinned worker dying.** The whole point is that the state is not durable, so a worker failing mid-sequence means the local file is gone. The recovery is to re-acquire and redo the sequence from the start, which means the sequence must be safe to repeat, the same [idempotency](/book/tools/idempotency) requirement as anywhere else.
...
- [Tool as Activity](/book/patterns/durability/tool-as-activity): why payloads should be references, and what the boundary costs
- [Resumable Activity](/book/patterns/durability/resumable-activity): durable checkpoints, for when the sequence is too long to pin
- [Fan-Out Over Items](/book/patterns/scale/fan-out): the pooled default this pattern deliberately gives up
- [Priority Task Queues](/book/patterns/scale/priority-queues): routing by urgency rather than by capability
- [Sandboxing and Credential Boundaries](/book/security/sandboxing): one sandbox per run, destroyed after, for the same locality reasons
