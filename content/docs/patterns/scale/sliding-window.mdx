...
**Size the window from the [token budget](/docs/reliability/backpressure), not from a thread count.** The binding constraint is input tokens per minute, and the arithmetic yields tens rather than thousands. A window sized by CPU is a window sized by the wrong resource.

**Propagate backpressure at the door.** When the window is full and the source is a stream, stop pulling, do not buffer internally. An unbounded internal queue converts a capacity problem into a memory problem plus [a fleet of runs you cannot finish](/docs/reliability/backpressure).
...
`Promise.race` / `FIRST_COMPLETED` is the entire difference from a [fan-out with a barrier](/docs/patterns/scale/fan-out). One line, and it changes utilization from sawtooth to flat.
...
**When ordering is required.** Any sequential dependency between items rules this out. Use [a batch iterator](/docs/patterns/scale/batch-iterator).
...
In [retrieval](/docs/patterns/retrieval/two-stage-rerank): do not wait for the slow exact scorer over everything, score cheaply and wide, then expensively and narrow. In [the workflow layer](/docs/patterns/scale/fan-out): do not synchronize on a barrier when the items are independent. Here: do not let the slowest item in a group gate the rest.
...
- [Fan-Out Over Items](/docs/patterns/scale/fan-out): the same bounded parallelism with a barrier at the end
- [Batch Iterator](/docs/patterns/scale/batch-iterator): when order matters and the set is walked
- [Concurrency, Rate Limits, Backpressure](/docs/reliability/backpressure): where the window size comes from, and why admission beats queueing
- [Downstream Rate Limiting](/docs/patterns/scale/downstream-rate-limiting): bounding the window by a quota you do not control
- [Priority Task Queues](/docs/patterns/scale/priority-queues): separate windows per class, so bulk cannot fill the interactive one
