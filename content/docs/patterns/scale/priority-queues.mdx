...
- **The unit is not requests.** Agent work is metered in [tokens](/docs/reliability/backpressure), and a "small" batch item can cost more than a live one.
...
**Let idle capacity be borrowed, and yield by admission.** Batch may exceed its share when interactive is quiet, and must fall back within one scheduling window when it is not. Yielding means *stop admitting new work*, never killing runs in flight: [a half-finished run has spent money and produced nothing](/docs/reliability/backpressure).
...
**When batch can be moved off the shared quota entirely.** This is the better fix when available: put evals and backfills on the [batch endpoint](/docs/patterns/cost/batch-api-offload), which is separately metered and cheaper. Scheduling contention is worse than not having it.
...
The instinct when capacity is short is to queue everything and let a scheduler sort it out. For agents that is subtly wrong, and the reason is [the property no ordinary request has](/docs/reliability/backpressure): **a run that starts and does not finish has spent real money and produced nothing.**
...
- [Concurrency, Rate Limits, Backpressure](/docs/reliability/backpressure): the chapter, including why admission beats queueing
- [Tenant Fairness](/docs/patterns/scale/tenant-fairness): the same problem within a class, across customers
- [Downstream Rate Limiting](/docs/patterns/scale/downstream-rate-limiting): the same shares applied to a quota you do not own
- [Batch API Offload](/docs/patterns/cost/batch-api-offload): removing the contention instead of scheduling it
- [Worker-Specific Queues](/docs/patterns/scale/worker-specific-queues): routing by capability rather than by urgency
