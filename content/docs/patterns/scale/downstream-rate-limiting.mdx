...
Then a [fan-out](/book/patterns/scale/fan-out) over eight hundred tickets calls `get_delivery_status` on each. Eight hundred requests in ninety seconds against a limit of sixty per minute.

What comes back is not a clean refusal. Some calls succeed, some return `429`, the [retry policy](/book/reliability/timeouts-and-retries) turns each `429` into three more requests, and the carrier's edge starts returning `503` to everything, including the nightly sync, which now fails, and the interactive agent, whose customer is waiting.
...
**One limiter per downstream, in the dispatcher.** Every tool call already passes through [one choke point](/book/production/reference-architecture); the limiter belongs there, keyed by the downstream rather than by the tool, because three tools hitting the same carrier share one quota.
...
**Honour `Retry-After` over your own arithmetic.** When the provider tells you when it will be ready, that is better information than any backoff curve. A large value is [backpressure](/book/reliability/backpressure): stop admitting new work rather than politely waiting.
...
**It cannot fix an over-wide fan-out.** A limiter turns a burst into a long queue; it does not make the job faster or cheaper. The fix for a job that overwhelms a downstream is [a smaller concurrency window](/book/patterns/scale/fan-out), and the limiter is the safety net.

## When not to use it

**When the downstream is yours and elastic.** Your own service that scales with load does not need you throttling on its behalf. Give it [backpressure](/book/reliability/backpressure) instead and let it shed.
...
**Instead of a batch endpoint.** If the work is latency-tolerant, [batch it](/book/patterns/cost/batch-api-offload): separately metered, cheaper, and it removes the contention rather than scheduling it.
...
Which makes this a compatibility problem as much as a scaling one. Before pointing an agent at an internal service, find out what its actual limit is and who else is using it: the answer is frequently *nobody knows*, and finding out during an incident is the expensive version. [The threat model's advice applies to capacity too](/book/security/threat-model): enumerate what you are attached to before something else does it for you.
</Aside>

## Related

- [Concurrency, Rate Limits, Backpressure](/book/reliability/backpressure): the same discipline applied to the model provider's quota
- [Fan-Out Over Items](/book/patterns/scale/fan-out): the burst source, and where the window should be sized
- [Priority Task Queues](/book/patterns/scale/priority-queues): deciding whose requests wait when the limiter is saturated
- [Timeouts, Retries, and Backoff](/book/reliability/timeouts-and-retries): why a 429 must not become three requests
