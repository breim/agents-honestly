...
[Priority classes](/docs/patterns/scale/priority-queues) do not help: all nine hundred are interactive. The contention is *within* a class, between peers, and no amount of separating urgent from batch addresses it.
...
**A fair system can still be an overloaded one.** Fairness distributes shortage; it does not remove it. If every tenant is degraded equally, the answer is capacity or [admission control](/docs/reliability/backpressure), not a better scheduler.

## When not to use it

**Single-tenant systems.** No peers, no contention. [Priority classes](/docs/patterns/scale/priority-queues) are the relevant control.
...
This is the same shape as [measuring retrieval recall per tenant rather than globally](/docs/patterns/retrieval/filtered-retrieval), and the same shape as [the cost dashboard needing a tenant dimension](/docs/observability/cost-accounting). A multi-tenant system whose metrics are all aggregates is a system that cannot see its own worst customer experience, which is usually the one about to churn.
</Aside>

## Related

- [Priority Task Queues](/docs/patterns/scale/priority-queues): fairness between urgency classes; this is fairness within one
- [Concurrency, Rate Limits, Backpressure](/docs/reliability/backpressure): per-tenant caps, and admission before starting a run
- [Multi-Tenant Isolation](/docs/security/tenant-isolation): the correctness boundary this shares a dimension with
- [Token and Cost Accounting](/docs/observability/cost-accounting): the per-tenant spend this schedules against
- [Sliding Window](/docs/patterns/scale/sliding-window): the in-flight bound the scheduler feeds
