...
Sending the top 5 misses it. Sending the top 20 includes it, along with fifteen chunks that scored highly *because they resemble the answer*, which is the most expensive material you can put in a prompt: near-misses compete for attention with the thing they resemble, and [degradation tracks confusability](/book/context/context-as-budget).
...
**Filter before you rerank, never after.** A hosted reranker receives every candidate you send it, including ones that were about to be dropped. Sending restricted content to a vendor and then filtering the response is [not an authorization boundary](/book/retrieval/metadata-and-authorization), it is a display preference.
...
**Cost per query.** Hosted rerankers price per query-plus-documents unit, so the second stage has a per-search cost that a vector search does not. On a run doing [three searches](/book/patterns/context/retrieval-on-demand), that is three units.

**Another dependency in the request path.** A reranker that is slow or down now gates every retrieval. It needs the same [breaker and fallback](/book/reliability/fallbacks-and-breakers) treatment as any dependency, and the fallback is simply: skip stage two and send the fused top-N.

**A vendor sees your corpus, one query at a time.** Add it to the [subprocessor list](/book/security/pii-and-compliance). This is the component that most often arrives without a review, because it is added to improve a metric.

**LLM-as-reranker is a different trade.** More accurate on nuanced relevance and able to follow instructions like *prefer the current version*, and slower, more expensive per candidate, and [nondeterministic](/book/foundations/nondeterminism), so the same candidate set can order differently across runs. Reasonable for a small candidate set on a high-value path; not a default.

## When not to use it

**When recall is the actual problem.** If the correct chunk is not in the top 50, reranking is the wrong stage entirely. Fix [chunking](/book/retrieval/chunking), [fusion](/book/patterns/retrieval/hybrid-fusion), or the corpus.
...
**When you have no retrieval evals.** Without recall@k and nDCG you cannot tell whether the reranker helped, and you will not be able to justify keeping it or removing it. [Measure first](/book/retrieval/retrieval-evals).

<Aside type="note" title="The shape recurs, which is why it is worth internalizing">
Retrieve wide and cheap, then cut narrow and expensive. The same structure appears when [rescoring quantized vectors](/book/retrieval/ann-indexes), when [selecting context](/book/context/dynamic-selection), and, in a different register, in the [model cascade](/book/patterns/cost/model-cascade), where a cheap model handles the easy cases and an expensive one handles what survives.
...
- [Hybrid Fusion](/book/patterns/retrieval/hybrid-fusion): produces the candidate list this stage rescores
- [Query Rewriting](/book/patterns/retrieval/query-rewriting): the resolved query is what the reranker should score against
- [Filtered Retrieval](/book/patterns/retrieval/filtered-retrieval): why the predicates must run before the reranker sees anything
- [Model Cascade](/book/patterns/cost/model-cascade): the same cheap-then-expensive shape applied to generation
- [Hybrid and Reranking](/book/retrieval/hybrid-and-reranking): the chapter, including why a cross-encoder cannot be an index
