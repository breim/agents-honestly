...
**Deny by default.** A chunk with no tenant tag is invisible, not public. Enforce it in the schema, `tenant_id NOT NULL`, so an [ingestion job](/book/retrieval/ingestion-pipeline) that fails to attach metadata produces unreachable documents rather than universally readable ones.

**The principal is a required argument.** One retrieval function, no overload without it. If any path can retrieve without a principal, a background job, a cache warmer, an [eval harness](/book/capstone/hardening-it), that path is the vulnerability, and it will be written by someone in a hurry.
...
**Filter inside the query, at the lowest level that can enforce it.** Not in application code afterwards. Post-filtering means restricted content was read out of storage, travelled through your process, entered your logs, and, if you [rerank before filtering](/book/patterns/retrieval/two-stage-rerank), was sent to a third party.
...
The `cacheKey` function is on this page deliberately. Authorization has to travel with the *data*, not stop at the retrieval call: summaries, [memory writes](/book/context/memory), cached answers, and reranker inputs all inherit the restriction, and a cache keyed by question text alone will serve one tenant's answer to the next asker.

## Trade-offs

**Recall interacts with filtering in ANN indexes.** A restrictive predicate applied during an approximate search can leave the index unable to find `k` results within its search budget, quietly returning fewer or worse hits. This is a real and under-discussed effect: [measure recall per tenant](/book/retrieval/retrieval-evals), not just globally, because a small tenant can have materially worse retrieval than the average suggests.
...
- [Metadata Is Authorization](/book/retrieval/metadata-and-authorization): the chapter, with the stale-permission and derived-artifact arguments in full
- [Two-Stage Rerank](/book/patterns/retrieval/two-stage-rerank): why filtering must precede the reranker
- [Hybrid Fusion](/book/patterns/retrieval/hybrid-fusion): predicates run inside each leg, never on the fused list
- [Multi-Tenant Isolation](/book/security/tenant-isolation): the same boundary across every other store
- [Untrusted Retrieval](/book/security/untrusted-retrieval): the trust predicate, which is integrity rather than confidentiality
