...
**Resolve against this run's retrieved set, not the corpus.** A citation naming a real chunk the run never retrieved is still fabricated. Check membership in `ctx.retrievedIds`, which the [trace already records](/book/observability/replay-debugging) for replay and incident scoping.
...
The split between the two functions is the design. The mechanical check is a [guardrail](/book/evals/online-evals): it runs on every response, fails closed, and costs nothing. The faithfulness score is an *eval*: it samples, reports a distribution, and is never on the critical path.
...
**Entailment judges are themselves models.** An LLM-as-judge or an NLI model has its own error rate and its own [drift](/book/observability/drift). Pin the judge version like any other model, and treat a moving faithfulness score with a stable system as a judge problem until proven otherwise.
...
**Before recall is fixed.** If the right chunk was never retrieved, grounding checks correctly report that the answer is unsupported, which is true, useless, and points at the wrong stage. [Fix retrieval first](/book/retrieval/retrieval-evals).
...
Faithfulness does not. It compares the response against the context that produced it, both of which you already have on every single request. That makes it the rare measurement that runs on real traffic, at real volume, on questions nobody wrote fixtures for, and it is the signal behind the [sustained-drop alert](/book/production/operating-manual) that catches a regression before anyone files a complaint.

It is also, precisely, a detector for the [semantic failure class](/book/reliability/error-taxonomy): the one with no exception type and no error rate.
</Aside>

## Related

- [Two-Stage Rerank](/book/patterns/retrieval/two-stage-rerank): grounding fails when the right chunk is not in the prompt; fix that first
- [Parent–Child Chunks](/book/patterns/retrieval/parent-child-chunks): cite the child, send the parent
- [Trajectory Evals](/book/evals/trajectory-evals): grounding claims against tool results, not just retrieved text
- [Online Evals](/book/evals/online-evals): the sampling discipline the semantic layer needs
- [Context Poisoning](/book/context/context-poisoning): the failure this detects when the bad value came from the agent itself
