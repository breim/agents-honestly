...
- **A rewrite can destroy information**, particularly exact identifiers, which are the tokens [lexical search](/book/retrieval/lexical-search) most depends on.
...
**Search the original too.** Keep the user's literal string as one of the candidate queries and [fuse the result lists](/book/patterns/retrieval/hybrid-fusion). The rewrite is a hypothesis, and a bad rewrite should not be able to lose a result the original would have found.
...
**Multi-query multiplies cost.** Three queries mean three retrievals and a fusion step. The recall gain is real and the precision cost is also real. [Measure it](/book/retrieval/retrieval-evals) rather than assuming more queries is better.

**It is another place personal data flows.** The rewritten query is sent to a model and logged in your traces. If the question contains customer information, so does the rewrite, [handles, not values](/book/security/pii-and-compliance), applies here too.
...
If the right chunk is not in the index, is chunked so the answer spans a boundary, or is outranked by a superseded version, no query phrasing recovers it. Check [recall@k with the ideal query](/book/retrieval/retrieval-evals) first: if a hand-written perfect query still misses, the problem is [chunking](/book/retrieval/chunking), [filtering](/book/retrieval/metadata-and-authorization), or the corpus, and rewriting will just make the failure harder to diagnose.
</Aside>

## Related

- [Hybrid Fusion](/book/patterns/retrieval/hybrid-fusion): how the original and rewritten queries' result lists get merged
- [Retrieval on Demand](/book/patterns/context/retrieval-on-demand): where the rewrite runs when the agent drives the search
- [Two-Stage Rerank](/book/patterns/retrieval/two-stage-rerank): fixing precision after retrieval instead of before
- [Lexical Search](/book/retrieval/lexical-search): why preserving identifiers verbatim matters
- [Retrieval Evals](/book/retrieval/retrieval-evals): how to tell whether the rewrite helped
