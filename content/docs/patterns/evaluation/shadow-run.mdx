...
A prompt change passes the [golden set](/book/patterns/evaluation/golden-set). Two hundred and fifty curated cases, all green.
...
The obvious next step is [a canary](/book/patterns/evaluation/canary-eval), but that exposes real customers to an unvalidated version. Between "tested on fixtures" and "serving customers" there is a gap, and for a change with any risk you want something in it.
...
**Replay goes stale the moment the prompt or the model moves.** This is the limit that surprises people: the cheap, deterministic, free variant covers dispatcher, policy, and post-retrieval changes and cannot cover the two changes you most want to test. [Divergence detection](/book/observability/replay-debugging) is what stops it silently answering the wrong question.
...
**For a pure refactor, use replay alone.** The expected diff is zero, no model calls are needed, and it costs nothing. [The zero-diff assertion](/book/observability/replay-debugging) is a stronger check than any sampled comparison.
...
The rungs are: [fixtures](/book/patterns/evaluation/golden-set), replay, shadow execution, [canary](/book/patterns/evaluation/canary-eval), full. Each one trades exposure for realism.
...
- [Canary Eval](/book/patterns/evaluation/canary-eval): the next rung, where real customers are served
- [Golden Set](/book/patterns/evaluation/golden-set): the rung below, with no exposure and no realism
- [Replay-Driven Debugging](/book/observability/replay-debugging): the replay variant, and why divergence detection matters
- [Rollout and In-Flight Migration](/book/production/rollout): the full ladder and the blast-radius gating
- [Trajectory Assertion](/book/patterns/evaluation/trajectory-assertion): the decision fields worth diffing
