...
Some things a [golden set](/book/patterns/evaluation/golden-set) needs to check have no mechanical answer. Does the reply address what the customer actually asked? Is the tone right for someone on their third follow-up? Is the explanation complete enough to act on?
...
**Cost and latency per judged item.** Six criteria in one structured call is one model pass, which is why they go in one call rather than six. On a [golden set](/book/patterns/evaluation/golden-set) that runs on every commit this is a real per-build cost.
...
**Judges drift.** A pinned model still changes behaviour, so a moving score on a stable system is a judge problem until proven otherwise. The [frozen canary](/book/observability/drift) should include judge fixtures, not just agent fixtures.

**Rubric changes invalidate history.** Adding a criterion changes what the score means, so trend lines break. Version the rubric with the [config bundle](/book/production/versioning) and report per-criterion rates rather than only the total, so one added criterion does not erase the series.

## When not to use it

**When a deterministic check exists.** Citation resolution, amount grounding, schema validity, [forbidden retrievals](/book/patterns/evaluation/trajectory-assertion). Those are free, total, and cannot be lenient. A judge is for what they cannot express.

**As a gate on an interactive path.** A judge is an eval, not a [guardrail](/book/patterns/evaluation/online-guardrail). It samples and reports; it does not sit between the agent and the customer.
...
- [Golden Set](/book/patterns/evaluation/golden-set): the cases the judge scores
- [Trajectory Assertion](/book/patterns/evaluation/trajectory-assertion): the deterministic checks to exhaust first
- [Scoring](/book/evals/scoring): the chapter, including rubric design and inter-rater agreement
- [Critic and Reviser](/book/patterns/control/critic-reviser): the same independence argument, applied at runtime rather than in evals
- [Online Guardrail](/book/patterns/evaluation/online-guardrail): why a judge is not a gate
