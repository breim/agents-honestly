...
The honest answer, without a dataset, is that nobody knows. The available evidence is that the person who wrote it tried four examples and liked the results, which is [the same four examples that motivated the change](/docs/evals/eval-discipline), so of course it does better on them.
...
**Include cases the agent should fail.** Out-of-scope requests, ambiguous tickets, things that must escalate. A set of only-successes measures how well the agent does the easy thing, and [escalation recall is the metric held highest](/docs/capstone/the-brief) precisely because missing one is the expensive error.

**Promote every production failure.** [Replay](/docs/observability/replay-debugging) turns an incident into a deterministic fixture. This is the source that compounds: the set grows at exactly the rate the system breaks, which beats any sampling strategy designed up front.

**Separate what gates from what reports.** Invariants, such as no cross-account data, no credit above cap, no uncited policy claim, fail the build with no threshold and no discussion. Rates are noisy on small samples, so they report as a distribution against a [flake budget](/docs/evals/ci-for-agents); gating a noisy metric on one run produces a suite people re-run until it passes.
...
**Small sets are noisy.** Detecting a three-point change on 250 cases at meaningful confidence is not always possible. That is why rates report rather than gate, and why the [canary](/docs/patterns/evaluation/canary-eval) on real traffic exists: CI has the labels and production has the sample size.
...
**Promoted cases carry customer data.** Scrub and pseudonymize at extraction: [the eval-fixture row in the compliance table](/docs/security/pii-and-compliance) is exactly this pipeline, and fixtures outlive the system they came from.
...
**Before the problem is understood.** [Writing twenty cases is the cheapest available test of whether you understand the task](/docs/first-agent/the-problem), and failing to write them is the signal, not a reason to skip ahead.
...
**Do not use it to measure production quality.** A curated set measures your changes against a fixed reference. What real traffic is doing needs [online scoring on a sample](/docs/evals/online-evals).
...
The same discipline that makes [prompts and bundles versioned artifacts](/docs/production/versioning) applies here, and for the same reason: without it, "we tested this" is a claim about a moment nobody can reconstruct.
</Aside>

## Related

- [CI for Agents](/docs/evals/ci-for-agents): set sizing, flake budgets, and what a gate actually needs
- [Trajectory Assertion](/docs/patterns/evaluation/trajectory-assertion): asserting on the path, not only the outcome
- [Judge With Rubric](/docs/patterns/evaluation/judge-with-rubric): scoring the cases whose expected outcome is not mechanical
- [Replay-Driven Debugging](/docs/observability/replay-debugging): how a production failure becomes a fixture
- [Canary Eval](/docs/patterns/evaluation/canary-eval): the sample size CI cannot afford
