...
Everything else in this group runs *around* production. [Golden sets](/docs/patterns/evaluation/golden-set) gate a release, [shadow runs](/docs/patterns/evaluation/shadow-run) compare versions, [canaries](/docs/patterns/evaluation/canary-eval) score a slice, [judges](/docs/patterns/evaluation/judge-with-rubric) sample and report.
...
- **Blocking spends human attention**, from [the same finite budget as approvals](/docs/human-in-the-loop/risk-tiers).
...
**Tier 2 never blocks.** It routes to review, samples for measurement, and feeds [the quality alert](/docs/production/operating-manual). The moment a classifier blocks, its false-positive rate becomes a product outage and its false-negative rate becomes a security claim you cannot support.
...
Every tier-1 check reads something the agent could not influence: `retrievedIds` was written by the retriever, `scratchpad` only accepts [tool-written values](/docs/patterns/context/structured-scratchpad), `ticket.contact` came from the record. That is what makes them assertions rather than opinions, and it is why they are safe to block on.

## Trade-offs

**Latency on every response.** Tier 1 is microseconds: string extraction and set lookups. Tier 2 is milliseconds to seconds, which is why anything expensive [samples rather than gates](/docs/patterns/retrieval/grounded-citations).
...
**As an injection defense.** The misuse this pattern must warn against. An injected instruction produces a well-formed, in-policy action: `issue_credit(4471, 250000)` is not a harmful *output*, so a check on the reply text never sees it. [Capability constraints](/docs/patterns/security/tool-permission-boundary) stop it.
...
**As a substitute for evals.** A guardrail is a runtime check on one response. It cannot tell you whether quality is drifting, and a team watching block rates instead of [sampled scores](/docs/evals/online-evals) is measuring its filter rather than its product.

<Aside type="caution" title="Guardrail, invariant, and eval are three different things">
The word does most of the damage. "Guardrail" suggests something that keeps you on the road; in practice this is a check with a miss rate, at the last possible moment, on the output of a system whose failures are [confident and well-formed](/docs/reliability/error-taxonomy).
...
And [a prompt instruction is none of the three](/docs/first-agent/the-problem).
</Aside>

## Related

- [Output Guardrail](/docs/patterns/security/output-guardrail): the same mechanism catalogued from the security side
- [Grounded Citations](/docs/patterns/retrieval/grounded-citations): the three-layer version of the grounding checks in tier 1
- [Trajectory Assertion](/docs/patterns/evaluation/trajectory-assertion): the same deterministic assertions, applied in CI rather than at delivery
- [Online Evals](/docs/evals/online-evals): why a guardrail is not an eval, and what to measure instead
- [Partial Result Return](/docs/patterns/failure/partial-result-return): the `coverage` field tier 1 gates on
