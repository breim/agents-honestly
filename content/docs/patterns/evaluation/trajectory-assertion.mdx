...
- **The path is recorded already**: [the trace has the tool calls, arguments, and retrieved IDs](/docs/observability/what-a-trace-must-answer).
...
**Write the forbidden assertions first.** *Never retrieved another tenant's chunk. Never called `issue_credit` on a read-only path. Never acted after the taint ceiling fired.* These are the [invariants](/docs/patterns/evaluation/golden-set), they gate with no threshold, and they are the rare place where a probabilistic system offers a hard yes-or-no.

**Ground the answer to the path.** The strongest required assertion is that the output's claims trace to something the run actually retrieved or computed, which is [grounding](/docs/patterns/retrieval/grounded-citations) expressed as a trajectory property, and it is what catches the opening scene.
...
**It needs a trace with arguments, not just tool names.** [The same instrumentation replay and incident scoping need](/docs/observability/replay-debugging), which is three requirements landing on one field, usually a sign it is not optional.
...
**When the trace is not reliable.** If tool calls are dispatched from several places and only some are instrumented, the assertions are checking a partial record: [one dispatcher](/docs/production/reference-architecture) is a prerequisite for this pattern being meaningful.
...
Those are the assertions that can gate a build with no threshold and no discussion, and they are how every [invariant](/docs/capstone/the-brief) in this book becomes testable: cross-tenant isolation, argument scoping, the taint ceiling, the no-uncited-policy rule.
...
- [Golden Set](/docs/patterns/evaluation/golden-set): the cases these assertions run against
- [Trajectory Evals](/docs/evals/trajectory-evals): the chapter, including claim-level grounding across steps
- [Grounded Citations](/docs/patterns/retrieval/grounded-citations): the grounding assertion, as its own pattern
- [What a Trace Must Answer](/docs/observability/what-a-trace-must-answer): the instrumentation this depends on
- [Filtered Retrieval](/docs/patterns/retrieval/filtered-retrieval): the isolation invariant these assertions enforce
