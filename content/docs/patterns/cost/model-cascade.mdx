...
The obvious fix is to route each node to a model chosen at design time, and [that is the right first move](/docs/production/cost-engineering). It runs out where the difficulty varies *within* a node: most damage assessments are straightforward and some are genuinely ambiguous, and the node cannot know which it has until it looks.
...
**Never let the cheap model handle a consequential call.** Route by [risk tier](/docs/human-in-the-loop/risk-tiers) before routing by difficulty: anything above tier 0 goes to the large model regardless of how confident the cheap one was. A discount on the decision to move $2,500 is not a saving.
...
**Escalate with the original input, not the cheap model's output.** The large model should answer the question, not review a draft. Passing the first attempt biases it toward agreeing, which is [the correlated-reviewer failure](/docs/patterns/control/critic-reviser) in a new place.
...
**Two models to operate.** Two versions to pin, two eval runs, two [drift surfaces](/docs/observability/drift), two sets of formatting quirks. A cheap model that silently changes behaviour raises the escalation rate, which reads as a cost regression until someone checks the canary.
...
**When per-node routing already covers it.** If the node's difficulty does not vary, [assign the model at design time](/docs/production/cost-engineering): testable, reviewable, rollback-able, and with no runtime gate to be wrong.
...
[Two-stage rerank](/docs/patterns/retrieval/two-stage-rerank) retrieves cheap and wide, then rescores narrow and expensive. A cascade answers cheap and often, then re-answers narrow and expensive. [Fallback ladders](/docs/patterns/failure/fallback-model-ladder) descend the same axis under a different trigger.
...
- [Cost Engineering](/docs/production/cost-engineering): the lever ordering, and why per-node routing comes first
- [Two-Stage Rerank](/docs/patterns/retrieval/two-stage-rerank): the same cheap-then-expensive structure in retrieval
- [Fallback Model Ladder](/docs/patterns/failure/fallback-model-ladder): descending models for availability rather than cost
- [Token Budget Enforcement](/docs/patterns/cost/token-budget-enforcement): the ceiling a cascade helps you stay under
- [Risk Tiers](/docs/human-in-the-loop/risk-tiers): the gate that runs before the cheap model is considered
