...
**One tenant consuming everyone's capacity.** Which is [a fairness problem](/book/patterns/scale/tenant-fairness) *and* a margin problem: a customer whose agent usage exceeds their subscription is unprofitable, and nothing in a global number says which one.
...
**Derive the run cap from what the run is worth.** [Cost per resolved outcome against margin on one](/book/production/cost-engineering) is the frame: a ticket worth eight dollars gets a cap derived from eight dollars. A number picked because it felt safe is a number that gets raised the first time it fires.

**Degrade before you stop.** At the soft threshold, switch to a cheaper model, narrow retrieval, and drop speculative calls. A hard stop produces a truncated run at the worst possible moment; a degrade produces a cheaper completion: [the same shape as a fallback](/book/patterns/failure/fallback-model-ladder), applied to money.

**Count everything the run spends.** Retries, [reflection passes](/book/patterns/control/reflection), critic calls, abandoned branches, and [cascade escalations](/book/patterns/cost/model-cascade) all bill at full price. A budget that counts only productive turns is not a budget.

**Route each ceiling to its right owner.** A run cap fires at the [escalation ladder](/book/patterns/control/escalation-ladder). A principal cap alerts whoever owns that key. A tenant cap is a commercial conversation, and paging on-call for a customer exceeding their plan trains people to ignore the pager.
...
**When cost is genuinely immaterial.** A low-volume internal tool does not need three ceilings. The [run cap alone](/book/patterns/control/bounded-autonomy) still earns its place, because it bounds the pathological loop.

**When value is not per-run.** [A batch job](/book/patterns/cost/batch-api-offload) has a job-level budget rather than a per-item one; applying a per-run cap to 340,000 items is 340,000 individually reasonable numbers and no ceiling on the total.

**When the tenant scope has no commercial meaning.** Internal single-tenant deployments have nobody to bill, so the tenant ceiling is a capacity control and belongs with [priority shares](/book/patterns/scale/priority-queues) instead.

**As a substitute for the cheaper levers.** [Cutting turns and shrinking what is re-sent](/book/production/cost-engineering) reduce cost with no quality risk. A budget stops the bleeding; it does not make the system efficient.
...
That answer needs [per-run, per-tenant, per-feature, per-node attribution](/book/observability/cost-accounting), recorded at the [model gateway](/book/production/reference-architecture), because provider billing has no tagging model and the invoice cannot be decomposed after the fact.
...
- [Bounded Autonomy](/book/patterns/control/bounded-autonomy): the run ceiling among the other four bounds
- [Token and Cost Accounting](/book/observability/cost-accounting): the attribution that makes these numbers choosable
- [Tenant Fairness](/book/patterns/scale/tenant-fairness): capacity sharing, which the tenant ceiling complements
- [Partial Result Return](/book/patterns/failure/partial-result-return): what a run produces when its cap fires
- [Cost Engineering](/book/production/cost-engineering): the levers to pull before reaching for a ceiling
