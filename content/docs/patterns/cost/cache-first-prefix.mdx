...
[Stable prefix](/docs/patterns/context/stable-prefix) covers the layout rule: order by stability, breakpoint at the last invariant position. This pattern is the economic consequence, which is a different design decision: **once the prefix is nearly free, put more in it.**
...
**Prefix material must earn its place on quality, not just on price.** "It is nearly free" is a reason to *allow* eight worked examples, not a reason to add them. Measure whether they help: [an eval run](/docs/evals/ci-for-agents) with and without is a one-afternoon experiment.
...
**It fights [dynamic tool selection](/docs/context/dynamic-selection).** Trimming the catalogue per request saves a few thousand tokens of schema and loses the cache on everything after it. Select per conversation, or append rather than substitute.
...
Which is why [the tool catalogue hash belongs in the config bundle](/docs/production/versioning) and why cache hit rate belongs on the [cost dashboard with an alert](/docs/observability/cost-accounting). The alert is the only thing standing between a one-line edit and a quiet 10× on your largest line item.
</Aside>

## Related

- [Stable Prefix](/docs/patterns/context/stable-prefix): the layout rule and the full list of silent invalidators
- [Prompt Caching](/docs/foundations/prompt-caching): the mechanism, the break-even arithmetic, and how to verify
- [Batch API Offload](/docs/patterns/cost/batch-api-offload): the other discount, which stacks with this one
- [Token and Cost Accounting](/docs/observability/cost-accounting): metering cached input separately, and alerting on hit rate
- [Dynamic Selection](/docs/context/dynamic-selection): the trade this pattern is usually in tension with
