...
Only the first has anyone waiting. The other two run on the interactive endpoint at interactive prices, and at 02:04 they cause [the incident where a job with no deadline starves the workload with customers in it](/book/patterns/scale/priority-queues).
...
- **The discount [stacks with prefix caching](/book/patterns/cost/cache-first-prefix)**, and a shared preamble across thousands of items is exactly the case where it compounds.
...
**Handle per-item outcomes.** A batch does not succeed or fail as a unit: each request has its own result or error. Treat it exactly as [a fan-out](/book/patterns/scale/fan-out): isolate per-item failures, break on a systemic rate, and write a manifest saying what is missing.
...
**Asynchrony is a real code change.** Submit, persist, poll, collect, handle partials. That is a [durable workflow](/book/patterns/durability/agent-as-entity-workflow) rather than a function call, and the polling loop must survive deploys.

**The stacking is conditional on the cache TTL, and the default one loses.** The discounts do compose: cache multipliers apply on top of the batch discount. But a batch runs for hours, and [the short-TTL entry expires in about five minutes](/book/foundations/prompt-caching). Every request after the first few pays the write premium and never reads it, which is worse than not caching. Use the long TTL for batch work, keep the prefix identical across every request in the batch, and treat a low `cache_read_input_tokens` on a batch job as the signal that the stacking never happened.
...
The main reason is that **batch traffic is metered separately**, so the eval suite and the backfill stop competing with live traffic for the same [token quota](/book/reliability/backpressure). That removes the 2am incident structurally rather than scheduling around it: no [priority shares](/book/patterns/scale/priority-queues) to tune, no borrowing rules, no risk that a misconfigured concurrency setting starves the workload with customers in it.
...
- [Cache-First Prefix](/book/patterns/cost/cache-first-prefix): the discount that stacks with this one on a shared preamble
- [Priority Task Queues](/book/patterns/scale/priority-queues): the contention this removes rather than schedules
- [Fan-Out Over Items](/book/patterns/scale/fan-out): the per-item failure handling a batch result needs
- [Batch Iterator](/book/patterns/scale/batch-iterator): walking a job too large for a single submission
- [Cost Engineering](/book/production/cost-engineering): where this sits in the lever ordering
