...
**Delete URL-shaped parameters rather than validating them.** A `webhook_url` argument with a hostname allowlist is a policy someone must maintain; a tool with no such argument has no policy to get wrong. This is [the same move as removing the recipient from `send_reply`](/book/patterns/security/tool-permission-boundary), and it is the strongest available form of this pattern.
...
**Alert on denials, do not just drop them.** A blocked connection to an unexpected host is the highest-signal detection available anywhere in [Part XVII](/book/security/threat-model). It is what a successful injection looks like from the outside, and a silent drop throws that away.
...
**Allowlists must be maintained.** A new provider region, a new internal service, a vendor changing hostnames. Stale allowlists cause outages, so the list belongs in the [config bundle](/book/production/versioning) with a review path, not in a firewall rule nobody can find.
...
**Full denial is not always possible.** [`send_reply` is a class ⑤ external write](/book/tools/read-and-write-tools): the product *is* sending bytes to a customer. The mitigation is that the recipient comes from the ticket record and the body carries no remote resources, which bounds the channel rather than closing it.
...
**When there is no untrusted content.** [The trifecta needs all three](/book/security/prompt-injection). If nothing attacker-influenced can enter the context, egress is a hygiene measure rather than a control, and re-check that assumption whenever a new ingestion path appears.

**As the only control.** Egress bounds the *leak*; it does nothing about an agent induced to issue a credit or send a wrong reply. [Capability constraints](/book/patterns/security/tool-permission-boundary) are a separate axis.

**Where it would break a core workflow with no proxy path.** Then be explicit: document the exception, narrow it to specific hosts, and raise the [risk tier](/book/human-in-the-loop/risk-tiers) on that path rather than quietly allowing everything.

<Aside type="caution" title="This is usually the cheapest element of the trifecta to remove">
[Private data, untrusted content, and an exfiltration vector](/book/security/prompt-injection): any two are survivable, all three is an incident. For most agents the first two are the product: reading customer records and processing customer-written text is the job.
...
- [Prompt Injection](/book/security/prompt-injection): the lethal trifecta, and why element three is the one to remove
- [Sandboxing and Credential Boundaries](/book/security/sandboxing): egress policy for generated code, and the broker socket
- [Tool Permission Boundary](/book/patterns/security/tool-permission-boundary): removing URL parameters rather than validating them
- [Output Guardrail](/book/patterns/security/output-guardrail): the other check on the way out, and why it is mitigation
- [PII, Audit, and Compliance](/book/security/pii-and-compliance): what is actually at stake in the bytes that leave
