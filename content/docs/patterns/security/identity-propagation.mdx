...
**When the backend has no authorization model.** Propagating identity to a service that ignores it produces a false sense of enforcement. Fix the backend or put the enforcement in [the dispatcher](/docs/patterns/security/tool-permission-boundary), and be honest about which you did.
...
The `act` claim exists precisely to avoid this trade. [RFC 8693](/docs/security/identity) has supported it since 2020. You do not have to choose between authorizing correctly and knowing who acted.
</Aside>

## Related

- [The Agent Is Not a Superuser](/docs/security/identity): the chapter, with the three identity models and the resumption cases
- [Tool Permission Boundary](/docs/patterns/security/tool-permission-boundary): scoping what the token may do, once it says who
- [Filtered Retrieval](/docs/patterns/retrieval/filtered-retrieval): the same principal, enforced in the query rather than the token
- [Multi-Tenant Isolation](/docs/security/tenant-isolation): the boundary this must survive across every async hop
- [Dual Control](/docs/patterns/security/dual-control): when one identity is not enough authority
