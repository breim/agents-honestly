...
**Deny by escalating, never by erroring.** A refusal carrying *"account 9917 is not in scope for this run"* is a well-formed [approval request](/docs/human-in-the-loop/approval-gates). A stack trace is a control someone turns off during an incident.
...
**Argument bounds produce false denials.** A legitimate $1,200 refund on a $1,000 cap routes a customer to a queue. Size the caps from data: [query which amounts were historically approved unchanged](/docs/capstone/operating-it) rather than from a guess, and expect to move them.
...
**A dynamic catalogue invalidates the audit.** If [MCP servers or runtime tool loading](/docs/security/least-privilege) can add tools, "we reviewed the permissions" is a claim about a moment. Pin the servers, hash the definitions, and let the dispatcher's allowlist be authoritative over whatever the catalogue advertises.

## When not to use it

**When every tool is a pure read over the same data.** A read-only assistant against one corpus does not need nine credentials. It still needs [identity propagation](/docs/patterns/security/identity-propagation), which is the control that actually matters there.
...
**Before you know the real usage.** Mining the minimum from ninety days of traces beats guessing, and enforcing a guessed policy breaks the rare legitimate path at the worst moment. Run it in [shadow mode](/docs/evals/online-evals) first: log what *would* have been denied for a week.

<Aside type="note" title="This is where the security part converges">
Almost every control in [Part XVII](/docs/security/threat-model) lands on the same function. [Taint](/docs/security/prompt-injection) sets the ceiling, [delegation](/docs/patterns/security/identity-propagation) says as whom, argument scoping says on what and how much, [risk tiers](/docs/human-in-the-loop/risk-tiers) say whether a person signs, and [idempotency](/docs/tools/idempotency) says whether repeating is safe.

That convergence is the argument for [one dispatcher](/docs/production/reference-architecture), and it is the cheapest structural decision in this book. Four dispatch sites means implementing five controls four times, and you will implement them in three, which is how a system ends up with a security model that is correct in the paths people remembered.
</Aside>

## Related

- [Excessive Agency and Least Privilege](/docs/security/least-privilege): the chapter, with the six scoping axes and trace-based permission mining
- [Identity Propagation](/docs/patterns/security/identity-propagation): the credential says *as whom*; this says *to do what*
- [Dual Control](/docs/patterns/security/dual-control): what happens above the cap this pattern enforces
- [Risk Tiers](/docs/human-in-the-loop/risk-tiers): computing blast radius from the arguments, at call time
- [Sandboxing](/docs/security/sandboxing): the broker, which routes generated code through this same check
