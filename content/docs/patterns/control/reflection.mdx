...
**The critique must have an input the draft did not.** A validator's errors, a failing test, a [grounding check's list of unsupported claims](/docs/patterns/retrieval/grounded-citations), a re-read of the record. Without that, the review is the same model, on the same information, producing a different sample.

**Prefer a deterministic signal where one exists.** Schema validation, a linter, a compile step, a citation-resolution check. These are free, total, and cannot hallucinate a problem, and if the only feedback you can produce is a model's opinion, that is [a critic](/docs/patterns/control/critic-reviser), with its own caveats.
...
**It does not catch what the checks do not cover.** A fluent, well-cited, correctly-grounded paragraph that misreads the customer's actual question passes everything here. That is [the semantic class](/docs/reliability/error-taxonomy), and it needs [evals](/docs/evals/eval-discipline), not a reflection step.
...
- [Critic and Reviser](/docs/patterns/control/critic-reviser): a separate model as the external signal, and why independence matters
- [Grounded Citations](/docs/patterns/retrieval/grounded-citations): the best deterministic critique available to a retrieval agent
- [Output Guardrail](/docs/patterns/security/output-guardrail): the same checks, positioned as a gate rather than a revision loop
- [Evaluator-Optimizer](/docs/workflows/evaluator-optimizer): the chapter, including bounded iteration
- [Bounded Autonomy](/docs/patterns/control/bounded-autonomy): why the pass count is a hard cap
