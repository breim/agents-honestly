...
**Size the caps per run, from what the run is worth.** A global monthly ceiling tells you nothing about whether this ticket deserved forty calls. The unit is [cost per resolved outcome against margin on one](/book/production/cost-engineering): a ticket worth eight dollars gets a cap derived from eight dollars.

**Degrade before you stop.** At seventy percent of budget, switch to a cheaper model, narrow retrieval, and drop speculative calls. A hard stop produces a truncated run at the worst moment; a degrade produces a cheaper, slightly worse completion, the [same shape as a fallback](/book/reliability/fallbacks-and-breakers), applied to money.

**Every cap produces a defined outcome, and none of them is an exception.** A partial result marked incomplete, or an escalation carrying what was established. Budget exhaustion is [the system working](/book/reliability/error-taxonomy), and logging it at `ERROR` teaches everyone to ignore the error channel.

**Count what the model spends on its own behalf.** Retries, [reflection passes](/book/patterns/control/reflection), critic calls, and abandoned branches all bill at full price. A budget that only counts productive turns is not a budget.
...
**A partial result is a product decision, not a technical one.** What does the customer see when a run stops at seventy percent? Usually the right answer is that they see nothing and a person sees the work, which means every cap is also a load on the [escalation queue](/book/human-in-the-loop/risk-tiers).

**Degrading changes the output quality silently.** A run that finished on the cheap rung produced a different answer than one that did not. Record which bounds fired, the same way you [record which model served](/book/reliability/fallbacks-and-breakers), or quality analysis is comparing incomparable runs.
...
**Skip the wall-clock deadline for async work.** A [durable run that pauses for approval](/book/patterns/durability/approval-gate) legitimately spans days. Bound its *active* time, not its elapsed time, or the approval pause trips the deadline.
...
**Skip the turn cap when the work is a fixed plan.** [Plan-then-execute](/book/patterns/control/plan-then-execute) bounds itself by construction. The plan has a step count, and a replan budget is the cap that matters instead.
...
And [inject the exhaustion deliberately](/book/reliability/failure-injection). Force a run to hit each bound in CI and assert the outcome: does it produce a partial result, does the escalation carry the work, does it reach a terminal state, is the cost bounded on the failure path too. The bounds are the code that runs when things go wrong, which makes them [the least-exercised code in the system](/book/reliability/failure-injection), and the code you most need to be correct.
</Aside>

## Related

- [ReAct Loop](/book/patterns/control/react-loop): the loop these bounds wrap, and where each check goes
- [Cost Engineering](/book/production/cost-engineering): deriving the money cap from unit economics
- [Escalation Ladder](/book/patterns/control/escalation-ladder): what a bound hands the work to
- [Timeouts, Retries, and Backoff](/book/reliability/timeouts-and-retries): the deadline, propagated per call
- [Token Budget Enforcement](/book/patterns/cost/token-budget-enforcement): the money bound, catalogued from the cost side
