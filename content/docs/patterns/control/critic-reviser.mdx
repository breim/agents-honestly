...
[Reflection](/docs/patterns/control/reflection) works when there is an external signal, a schema, a test, a citation check. Some of the things you actually want to catch have no such check.
...
The `sources` and `toolResults` lines are the pattern. Remove them and this becomes an expensive [intrinsic self-review](/docs/patterns/control/reflection), which is [measured not to help](https://arxiv.org/abs/2310.01798), the model count changed and the information did not.
...
**Critic leniency is the default failure.** Without calibration you get a critic that finds nothing and a system that feels reviewed. Keep a small set of deliberately-broken fixtures in [CI](/docs/evals/ci-for-agents) and assert the critic catches them, a critic that stops catching them has drifted.

**A different prompt is weaker independence than a different model.** Same weights, same blind spots, different instructions. Using a different model family for the critic buys more genuine independence and costs operational complexity plus a second [drift surface](/docs/observability/drift).
...
**When a deterministic check exists.** Schema, citation resolution, amount grounding, [use those first](/docs/patterns/control/reflection). They are free, total, and cannot be lenient. Reserve the critic for what they cannot express.
...
**As a substitute for evals.** A critic is a runtime check on one response and tells you nothing about whether quality is drifting. [Sampled scoring](/docs/evals/online-evals) is what answers that, and a team watching critic-finding rates is measuring their critic.
...
Which means it belongs in the [config bundle](/docs/production/versioning) with a pinned model and a hashed prompt; it needs its own [eval set](/docs/evals/ci-for-agents), scored against known-good and known-bad fixtures; it drifts when the provider changes something underneath it, and a [frozen canary](/docs/observability/drift) is the only thing that will tell you; and its findings are [prompt text](/docs/tools/errors-as-instructions) that the reviser reasons over, so a badly-worded finding produces a badly-targeted revision.
...
- [Reflection](/docs/patterns/control/reflection): the cheaper version, and why the external signal is the whole thing
- [Dual Control](/docs/patterns/security/dual-control): the same independence argument, with humans
- [When Not To](/docs/multi-agent/when-not-to): why a reviewer reading a conclusion is a second vote
- [Evaluator-Optimizer](/docs/workflows/evaluator-optimizer): the chapter, with bounded iteration and convergence
- [Scoring](/docs/evals/scoring): rubrics, and calibrating a model that judges
