...
*Should I look up the order?* The ticket names an order; always look it up. *Is the amount within the limit?* A comparison. *Should this escalate?* [A tier lookup](/book/human-in-the-loop/risk-tiers) from the arguments. *Did the customer ask about policy?* A classification, which is a model call but a closed one.
...
The cost is not only tokens. Every delegated decision is a decision that can differ between runs, cannot be unit-tested, does not appear in a diff, and can be [influenced by a retrieved document](/book/security/prompt-injection).
...
**Constrain the model's output even where judgment is required.** A closed label set, a [validated schema](/book/foundations/structured-output), a bounded numeric range. The model decides *which*; code decides *what happens as a result of which*.

**Never let the model own a safety property.** [Authorization, tier computation, idempotency, and budget checks](/book/capstone/building-it) are code that runs regardless of what the model concluded. If it must always hold, it cannot be a prompt instruction. [A prompt that says never exceed the limit is a request](/book/first-agent/the-problem).
...
**Over-railing produces a bad workflow.** If every branch is coded, you have written a state machine and are paying a model to fill in adjectives. [The determinism test](/book/workflows/the-determinism-test) runs both directions: a decision you *cannot* write down in advance belongs to the model, and coding it produces a system that fails on every case you did not anticipate.

**The boundary moves.** A decision that needed judgment at ten cases becomes a lookup table at a thousand, once you know the distribution. Revisit it, the [rung and route distributions](/book/patterns/control/escalation-ladder) are where the evidence shows up.
...
**When the sequence genuinely cannot be fixed.** Open-ended investigation where the next question depends on the last answer. That is [a ReAct loop](/book/patterns/control/react-loop), and railing it removes the adaptivity that was the point.
...
**When the rules change faster than you can deploy.** If policy shifts weekly and deploys are monthly, a prompt loaded from a [versioned config bundle](/book/production/versioning) is more honest than a rail that will be stale.
...
[Router](/book/patterns/control/router) is deterministic rails on the path. [Plan-then-execute](/book/patterns/control/plan-then-execute) is deterministic rails on the sequence. [Bounded autonomy](/book/patterns/control/bounded-autonomy) is deterministic rails on the stopping condition. [The dispatcher's eight checks](/book/capstone/building-it) are deterministic rails on authority.
...
- [The Determinism Test](/book/workflows/the-determinism-test): the chapter, and the test applied at the system level
- [Router](/book/patterns/control/router): rails on which path a request takes
- [Plan Then Execute](/book/patterns/control/plan-then-execute): rails on the order of operations
- [Bounded Autonomy](/book/patterns/control/bounded-autonomy): rails on when to stop
- [Building It](/book/capstone/building-it): the dispatcher, where the safety rails all land
