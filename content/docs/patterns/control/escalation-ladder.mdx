...
That binary produces two bad outcomes and no good ones. When it cannot finish, the customer gets *"I'm unable to help with this"* and a human gets a ticket with none of the work attached, [an escalation that followed the rule and failed the contract](/docs/first-agent/the-problem). When it can *almost* finish, it finishes anyway, because the alternative is giving up entirely.

The same binary shows up on the other side. The system is either fully autonomous on a path or fully gated, so the response to any doubt is to add an approval, and [approvals spend a finite budget of attention](/docs/human-in-the-loop/risk-tiers) that an adversary can also draw against.
...
**Map triggers to rungs deliberately.** [Budget exhaustion](/docs/patterns/control/bounded-autonomy) and *no progress* are different failures and belong on different rungs: the first has usable partial work and goes to assisted; the second is stuck and goes to human-owned. A [taint ceiling](/docs/security/prompt-injection) or a [policy refusal](/docs/reliability/error-taxonomy) goes straight to approval or human-owned regardless of how much work was done.
...
**Every rung needs an owner and an SLA.** Rung 2 with no queue and rung 3 with no approver are both hangs. This is organizational work, and it is the part that determines whether the ladder functions, [the capstone's first month](/docs/capstone/operating-it) found six of eight backlog items were about exactly this.
...
[The map has an escalation ladder too](/docs/the-map), and it answers a different question. That one is about **architecture**: function, workflow, rules, agent, multi-agent, each rung more capable and less predictable, chosen once at design time.
...
- [Risk Tiers](/docs/human-in-the-loop/risk-tiers): the tier of a call, which is one of the triggers that sets a rung
- [Bounded Autonomy](/docs/patterns/control/bounded-autonomy): the caps whose exhaustion promotes a run
- [Approval Gate](/docs/patterns/durability/approval-gate): the mechanism behind rung 3
- [Escalation and Audit](/docs/human-in-the-loop/escalation-and-audit): what the handover package must contain
- [Fallbacks and Circuit Breakers](/docs/reliability/fallbacks-and-breakers): the human rung as a fallback when infrastructure degrades
