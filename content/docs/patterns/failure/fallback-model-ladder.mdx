...
The naive ladder traded a loud failure for a quiet one. For a system whose [worst failure mode is plausible output](/docs/reliability/error-taxonomy), that is usually a bad trade, and it is the default outcome unless the ladder is designed against it.
...
**Only transient failures descend.** Falling back on [a content refusal](/docs/patterns/failure/non-retryable-model-errors) is shopping for a provider that will comply; falling back on a malformed request just fails again elsewhere. The eligibility check is the class, not the status code.

**High-stakes work does not degrade: it escalates.** A [tier-2 decision](/docs/human-in-the-loop/risk-tiers) served by the cheapest model still answering is a bad answer at a discount. Above tier 0 the correct rung is the human one.
...
**The fallback path is never exercised.** It runs 0.1% of the time and debuts during an incident. Two disciplines fix it: **evaluate the fallback configuration as its own variant** in [CI](/docs/evals/ci-for-agents) against the same suite with its own quality bar, and **route a small continuous share**, 1% of tier-0 traffic, through it permanently, so it is warm and monitored rather than theoretical.
...
- [Fallbacks and Circuit Breakers](/docs/reliability/fallbacks-and-breakers): the chapter, with the six rungs and the four ways fallbacks fail
- [Tool Circuit Breaker](/docs/patterns/failure/tool-circuit-breaker): the per-rung breaker that decides when to skip
- [Non-Retryable Model Errors](/docs/patterns/failure/non-retryable-model-errors): the class check that gates descent
- [Escalation Ladder](/docs/patterns/control/escalation-ladder): rung 6, and why it is a first-class outcome
- [Detecting Drift](/docs/observability/drift): quality segmented by served model, as an ongoing measurement
