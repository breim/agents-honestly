...
- **Agents amplify this**: [four retry layers](/book/reliability/timeouts-and-retries) plus a fan-out means one blip can produce thousands of aligned retries.
...
**Cap the ceiling.** Exponential growth without a cap produces a retry scheduled beyond any plausible recovery, and beyond the run's [deadline](/book/patterns/control/bounded-autonomy), which makes the wait pure waste. Thirty seconds is a common ceiling for interactive work.
...
**Randomness makes reproduction harder.** A test asserting retry timing needs a seeded generator. Inject the RNG rather than calling it globally, which is also required if the code runs inside [a durable workflow](/book/durable-execution/determinism-and-retries).

**It does not reduce total load.** Jitter spreads the same number of retries. During a real outage, the thing that reduces load is [the breaker](/book/patterns/failure/tool-circuit-breaker) and the [retry budget](/book/reliability/timeouts-and-retries). Jitter only stops the load arriving in spikes.

## When not to use it

**In workflow code that must replay deterministically.** `Math.random()` is [a source of divergence](/book/durable-execution/determinism-and-retries). Use the SDK's replay-safe random, or let the platform's own retry policy apply the jitter.

**When the delay is a business rule.** *"Retry the payment at 09:00 tomorrow"* is a schedule, not a backoff. Jittering it is wrong.

**When the operation is not retryable at all.** [Context-length errors, refusals, and unknown-outcome writes](/book/patterns/failure/non-retryable-model-errors) never reach this function.
...
Which is why this is one of the few places in this book where a one-line change, multiply the delay by a random number in `[0, 1]`, is the whole fix, and why leaving it out is one of the most common production incidents in distributed systems. Agents make it worse only in scale: [four retry layers](/book/reliability/timeouts-and-retries) and a fan-out means one blip can synchronize thousands of calls instead of hundreds.
</Aside>

## Related

- [Fast and Slow Retries](/book/patterns/failure/fast-slow-retries): the two phases this jitter is applied within
- [Tool Circuit Breaker](/book/patterns/failure/tool-circuit-breaker): what actually reduces load when the dependency is down
- [Timeouts, Retries, and Backoff](/book/reliability/timeouts-and-retries): the chapter, including retry budgets and the four layers
- [Downstream Rate Limiting](/book/patterns/scale/downstream-rate-limiting): avoiding the 429 that starts this in the first place
- [Determinism and Retries](/book/durable-execution/determinism-and-retries): why `Math.random()` cannot live in workflow code
