...
Each run makes the call, waits thirty seconds, [retries with backoff](/book/patterns/failure/fast-slow-retries), waits again, fails, and the agent, reading a tool error in its transcript, decides to try once more. A run that would normally take twelve seconds now takes four minutes and produces nothing.

Meanwhile every one of those attempts is load on a service that is trying to recover, and every one holds a worker slot and a chunk of the run's [deadline budget](/book/patterns/control/bounded-autonomy) waiting for a timeout that is guaranteed to fire.
...
Returning `error_for_model` rather than raising is the agent-specific part. An ordinary service breaker throws and the caller handles it; here the caller is a model, and telling it *proceed without this or escalate* is [an instruction it can act on](/book/tools/errors-as-instructions), which frequently turns a failed run into a partial answer plus a good escalation.
...
**When the dependency is elastic and yours.** A service that scales under load does not need you deciding it is down. Give it [backpressure](/book/reliability/backpressure) instead.

**When call volume is too low to measure.** Three calls an hour cannot produce a meaningful failure rate. Use timeouts and [bounded retries](/book/patterns/failure/fast-slow-retries) alone.

**As a substitute for a rate limiter.** A breaker reacts to failure; [a limiter prevents it](/book/patterns/scale/downstream-rate-limiting). If you are tripping breakers on `429`s, the fix is upstream.

<Aside type="note" title="The breaker is the only component that can see an outage">
[The retry chapter](/book/reliability/timeouts-and-retries) makes the arithmetic case: nested retries multiply, and ninety seconds of degradation becomes forty minutes of self-inflicted load. Jitter spreads that load; it does not reduce it. Retry budgets bound it per client. Neither one *stops* it.
...
Which is why [retry, jitter, and breaker](/book/patterns/failure/fast-slow-retries) are one design rather than three patterns. Retries without a breaker amplify outages by construction. A breaker without retries makes every blip user-visible. Most systems ship the retries and discover the other half during an incident.
</Aside>

## Related

- [Fast and Slow Retries](/book/patterns/failure/fast-slow-retries): the per-call policy this bounds in aggregate
- [Delayed Retry with Jitter](/book/patterns/failure/delayed-retry-jitter): spreading the load the breaker eventually stops
- [Fallback Model Ladder](/book/patterns/failure/fallback-model-ladder): the per-rung breaker, and what an open one routes to
- [Downstream Rate Limiting](/book/patterns/scale/downstream-rate-limiting): preventing the failure rather than reacting to it
- [Partial Result Return](/book/patterns/failure/partial-result-return): what the agent does with a tool it can no longer call
