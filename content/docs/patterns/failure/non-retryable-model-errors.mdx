...
**Declare non-retryable types to the retry machinery.** Do not rely on a `catch` deciding correctly every time. [Activity retry policies take an explicit list](/docs/patterns/durability/tool-as-activity); the SDK then stops on the first attempt instead of looping.

**Split "permanent" into yours and the model's.** `context_too_long` is your bug and the recovery is [compaction](/docs/patterns/context/context-compaction). `account 9917 not found` is the model's mistake and the recovery is [returning the error as an instruction](/docs/tools/errors-as-instructions). A transport retry there produces three identical failures before the model ever sees one.
...
**Vendor error shapes drift.** New codes appear, messages change, and a classifier tuned to one provider is wrong for the [fallback](/docs/patterns/failure/fallback-model-ladder). Keep one classifier per provider and treat an unclassified error as *permanent*. Failing closed is cheaper than looping.
...
**The model-fixable class is not a retry at all.** It belongs in the [dispatcher](/docs/capstone/building-it), returning a result rather than raising. Putting it in the retry policy is a category error that costs three round trips.
...
**As a substitute for preventing the error.** `context_too_long` should mostly not happen: [trigger compaction on a threshold](/docs/patterns/context/rolling-summary) rather than discovering the ceiling by hitting it. Classification is the safety net, not the design.
...
Either way the right response is the same: [escalate](/docs/patterns/control/escalation-ladder), record the refusal with the request, and let a person look at the pattern. A rising refusal rate is [a drift signal](/docs/observability/drift) worth reading, and a retry loop erases it.
</Aside>

## Related

- [An Error Taxonomy for Agents](/docs/reliability/error-taxonomy): the five classes, and the fifth one that never raises
- [Fast and Slow Retries](/docs/patterns/failure/fast-slow-retries): what to do with the errors that *are* retryable
- [Tool as Activity](/docs/patterns/durability/tool-as-activity): where the non-retryable list is declared
- [Errors as Instructions](/docs/tools/errors-as-instructions): the model-fixable class, written for its actual consumer
- [Fallback Model Ladder](/docs/patterns/failure/fallback-model-ladder): why only transient failures may fall back
