...
The agent answers the first from the policy corpus and the second from the warehouse. The third needs the contract terms, which are [out of scope for v1](/docs/capstone/the-brief), so the run escalates, and the human receives a ticket with nothing attached, re-does the two answers the agent already had, and then answers the third.

The same waste appears at every other boundary. A [budget cap fires](/docs/patterns/control/bounded-autonomy) at step nine and the eight completed steps are discarded. A [fan-out](/docs/patterns/scale/fan-out) over ten documents has one failure and returns an error for all ten. A [breaker opens](/docs/patterns/failure/tool-circuit-breaker) on the carrier API and the run fails rather than answering the parts that never needed it.
...
**Coverage is a field, never a phrase.** A model writing *"I could not determine the freight coverage"* inside otherwise fluent prose is not a check anything can gate on. The [output guardrail](/docs/patterns/security/output-guardrail) reads `coverage`, and a partial never reaches a customer automatically.
...
`deliver` is where the pattern earns its keep and where it is most often subverted. The temptation is to send the partial with a hedge, *"I was able to answer two of your questions…"*, which is honest and still leaves the customer with an unanswered question and no owner. The [escalation contract](/docs/first-agent/the-problem) says a person takes it, holding the work already done.
...
**Partials increase escalation volume.** Every partial is a human touch. That is the intended trade, twenty seconds of review against eight minutes of doing it, and it is [a real load on the queue](/docs/patterns/scale/priority-queues) that has to be staffed.
...
**Coverage can be wrong.** The agent believes it answered a sub-question and did not. This is [the semantic class](/docs/reliability/error-taxonomy) again. The defense is [grounding each established answer](/docs/patterns/retrieval/grounded-citations), not a better coverage calculation.
...
Look at what fires a partial: [a budget cap](/docs/patterns/control/bounded-autonomy), a turn cap, a deadline, [an open breaker](/docs/patterns/failure/tool-circuit-breaker), an [exhausted fallback ladder](/docs/patterns/failure/fallback-model-ladder), an out-of-scope sub-question.
...
- [Bounded Autonomy](/docs/patterns/control/bounded-autonomy): the caps whose exhaustion produces a partial
- [Escalation Ladder](/docs/patterns/control/escalation-ladder): the rung a partial lands on, and the handover package
- [Tool Circuit Breaker](/docs/patterns/failure/tool-circuit-breaker): a missing dependency as a cause of partial coverage
- [Grounded Citations](/docs/patterns/retrieval/grounded-citations): why each established answer carries provenance
- [Output Guardrail](/docs/patterns/security/output-guardrail): the gate that reads `coverage` before anything is sent
