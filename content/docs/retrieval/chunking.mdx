...
A long chunk contains the answer *and* five other topics, so its embedding is a blurry average that matches nothing precisely, and it burns [budget](/docs/context/context-as-budget) on material that isn't relevant.
...
**Structured metadata, alongside.** Document ID, version, tier, region, effective date: the fields from [the schema](/docs/retrieval/vector-search) two chapters ago. These aren't for the model; they're for the `WHERE` clause, and for the citation the [acceptance spec](/docs/first-agent/the-problem) requires. Atlas must cite document and version, so every chunk must carry document and version. Chunking is where that becomes possible or impossible.
...
And then the constraint that should shape your appetite for iteration: **re-chunking means re-embedding everything.** From [the embeddings chapter](/docs/retrieval/embeddings), that's a full corpus pass with a real bill, and it invalidates every recall measurement you've taken. Chunking is not a runtime parameter you tune in production; it is a schema decision with a migration cost.
...
Next: [Metadata Is Authorization](/docs/retrieval/metadata-and-authorization), on tenant, department, access level, and why filtering is a security control rather than an optimization.
