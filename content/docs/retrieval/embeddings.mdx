...
- **Score gaps matter more than score values.** "Top result 0.71, second 0.44" is a confident retrieval. "0.83, 0.82, 0.81, 0.81" is a coin flip with high numbers, and it is exactly the [distractor situation](/book/context/context-as-budget) that costs you accuracy.
...
This is [drift](/book/observability/drift), and it argues for the same defense as everything else in this book: a fixed evaluation set that includes new vocabulary, run on a schedule, so degradation shows up as a number rather than as a support complaint.
...
Worth restating from [the routing chapter](/book/retrieval/where-does-the-answer-live), now that the costs are concrete. Do not embed:
...
You have vectors and a standing reason to remake them. Nothing yet stores or searches them. Next: [Vector Search and the Database Question](/book/retrieval/vector-search), which starts at Postgres precisely because Postgres refuses to look like a product.
