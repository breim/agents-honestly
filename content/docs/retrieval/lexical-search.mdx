...
Hold on to the second one, because it is exactly the information the [embeddings chapter](/book/retrieval/embeddings) said gets destroyed:
...
That's the [drift](/book/observability/drift) problem from the embeddings chapter, and lexical search is structurally immune to it. In a corpus that grows new vocabulary, whether products, incident codes, customer names, or acronyms, that immunity compounds.
...
The unifying test from [the routing chapter](/book/retrieval/where-does-the-answer-live) still holds: **if a human would find it with Ctrl-F, use lexical.**
...
These are precisely dense retrieval's strengths, which is not a coincidence: the two methods fail in complementary directions. That complementarity is the entire argument for the [next chapter](/book/retrieval/hybrid-and-reranking).

## In practice: the second index

You do not need another system. In the schema from [two chapters ago](/book/retrieval/vector-search), lexical search is one more index on the same table:
...
The same `WHERE` clauses apply. Tenant, version, and tier are [authorization](/book/retrieval/metadata-and-authorization), and they are not optional on this path either. A second retrieval path is a second place to forget the filter, which is exactly why the last chapter insisted on one function.
...
Both paths, always. `search_policies` runs BM25 over `content_tsv` and dense over `embedding`, plus an exact-match pass over `identifiers` whenever the triage step extracted a part number, which, from [the very first chapter of Part II](/book/first-agent/one-call), it does, because entity extraction was in the v0 schema for exactly this reason.
...
Two retrievers now, each strong where the other fails, and one prompt with room for five chunks. Next: [Hybrid Search and Reranking](/book/retrieval/hybrid-and-reranking), on merging the two lists, then paying a better model to order what survives.
