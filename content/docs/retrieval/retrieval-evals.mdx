...
This is the [chunking chapter's bug](/docs/retrieval/chunking) made measurable. The chunk containing *"opened electrical components may be returned within 30 days"* is the correct retrieval for "can we return opened relays?": recall@5 counts it as a hit, precision looks fine, nDCG is excellent. And the exception clause that changes the answer was in the next chunk.
...
Precision matters more here than it does in web search, for a reason established in [Context as an Allocation Problem](/docs/context/context-as-budget): the irrelevant chunks aren't merely wasted tokens, they're the near-misses most likely to be confused with the answer. Low precision doesn't just cost money; it costs accuracy.

## The labeling problem

All of the above needs ground truth, and that is the actual barrier. Three ways to get it, each with a trap.

**Manual gold sets.** For each question, a human marks which chunks are necessary. Expensive, and the highest quality. Fifty questions labeled carefully beats five hundred labeled sloppily, and you already have twenty from [the problem chapter](/docs/first-agent/the-problem).
...
Real users don't phrase things the way your documents do. That vocabulary mismatch is [precisely the hard case](/docs/retrieval/lexical-search), and synthetic generation is the one method guaranteed not to test it.
...
**The negative set.** From [the authorization chapter](/docs/retrieval/metadata-and-authorization): questions asked by a principal who lacks access, asserting that the restricted document ID never appears in the retrieval trace. Deterministic, binary, and one of the very few hard assertions this field offers.

**Freshness.** Assert that a superseded document is never returned as current, and that a document deleted upstream is no longer retrievable at all. This is the [ingestion lifecycle](/docs/retrieval/ingestion-pipeline) verified from the query side, and it catches the reconciliation job silently failing.
...
Run retrieval evals on every change to chunking, the embedding model, the pipeline, the index parameters, or the reranker, which is to say on every change in Part IV. Each of those alters retrieval behaviour, and several of them ([re-chunking](/docs/retrieval/chunking), [re-embedding](/docs/retrieval/embeddings)) invalidate every number you'd measured before.

Run each question [several times](/docs/foundations/nondeterminism) where any stage involves a model, and report rates rather than single results, since the discipline from Part I applies here exactly as it does everywhere else.
...
The catalog carries the fixes with the code attached: [Parent–Child Chunks](/docs/patterns/retrieval/parent-child-chunks) for the right document and the wrong chunk, [Hybrid Fusion](/docs/patterns/retrieval/hybrid-fusion) and [Two-Stage Rerank](/docs/patterns/retrieval/two-stage-rerank) for recall and precision in that order, [Query Rewriting](/docs/patterns/retrieval/query-rewriting) for conversational phrasing, [Filtered Retrieval](/docs/patterns/retrieval/filtered-retrieval) for the authorization filter, [Freshness Routing](/docs/patterns/retrieval/freshness-routing) for the questions an index answers stale, [Grounded Citations](/docs/patterns/retrieval/grounded-citations) for making a claim checkable, and [Retrieval as a Tool](/docs/patterns/retrieval/retrieval-as-tool) for letting the agent decide whether to search at all.
...
Next: [When Relationships Are the Query](/docs/graphs/when-relationships-are-the-query), on multi-hop questions that similarity search structurally cannot answer, and how to recognize one.
