...
Do not send the original image on every query because it exists. Retrieve by caption and metadata first, then load pixels on demand when the question depends on them. This is [dynamic context selection](/docs/context/dynamic-selection) applied to media.
...
The attack path and defenses are covered in [Prompt Injection](/docs/security/prompt-injection) and [Adversarial Evals](/docs/security/adversarial-evals). The ingestion requirement is simple: derivation may change format, never trust.
...
Next: [Evaluating Retrieval](/docs/retrieval/retrieval-evals), covering recall, precision, evidence fidelity, and whether the answer survived the trip from source to context.
