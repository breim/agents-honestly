...
Because a mangled paragraph produces a slightly worse chunk, while a mangled table produces numbers attached to the wrong labels, which is the [confident-wrong-number failure](/docs/retrieval/sql-is-still-the-answer) arriving through a different door.
...
**Cleaning too much.** Stripping section numbers you need for [citations](/docs/first-agent/the-problem). Collapsing whitespace inside a code block or an address. Removing "boilerplate" that turned out to be the effective-date line. A cleaner tuned on one document type quietly destroys another.
...
| Extracted | Used by |
| --- | --- |
| `document_id`, `version`, `superseded_at` | The version filter: [correctness, not ranking](/docs/retrieval/vector-search) |
| `tenant_id`, `acl`, `classification` | [Authorization](/docs/retrieval/metadata-and-authorization) |
| `tier`, `region`, `effective_date` | Business-scope filtering |
| Identifiers: part numbers, codes | The [exact-match lexical path](/docs/retrieval/lexical-search) |
| Heading trail | [Chunk self-containment](/docs/retrieval/chunking) |
...
Recall from [the problem chapter](/docs/first-agent/the-problem) that Meridian's corpus contains contradictory documents, including superseded versions of the same policy. That's a data problem. It becomes an *agent* problem when the withdrawn version is still in the index because ingestion has no deletion path.
...
Re-running ingestion over the same document must produce the same chunks in the same place, not a second copy. Key chunks by a deterministic identity, such as document ID plus chunk index or a content hash, rather than by an auto-incrementing row. Otherwise a re-run after a partial failure doubles part of your corpus, and duplicate near-identical chunks are exactly the [distractors](/docs/context/context-as-budget) you least want.
...
Roughly 1,200 documents, 41,000 chunks, rebuilt incrementally on change and fully on any pipeline version bump. The full rebuild takes about forty minutes and costs real money, which is the [migration cost](/docs/retrieval/embeddings) made concrete, and the reason `pipeline_ver` is on every row.

---

Next: [Multimodal Document Intelligence](/docs/retrieval/multimodal-document-intelligence), on PDFs, scans, tables, images, and audio without flattening away the evidence.
