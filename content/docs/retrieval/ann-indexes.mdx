...
That is [drift](/book/observability/drift) in a place people don't look for it, and it means IVF wants periodic retraining as a scheduled job rather than a one-time build. It suits large, mostly-static corpora where memory or build cost dominates; it suits a fast-changing corpus badly.
...
And note the shape: search wide and cheap, then cut precisely. That is the same two-stage structure as [selection versus reranking](/book/context/dynamic-selection), and as [hybrid retrieval](/book/retrieval/hybrid-and-reranking) in the next chapters. It keeps recurring because it is the general answer to "recall and precision want opposite things."
...
And hold it at the value it has, which is a ceiling rather than a score. Recall says the right chunk reached the window; it says nothing about whether the model could use it, and [the two move independently often enough to be worth watching separately](/book/retrieval/retrieval-evals). Tune the index on recall, because that is what an index controls, then check that the answers moved.
...
The harness measures how well you find a chunk. Whether the chunk was worth finding is decided by how it was cut. Next: [Chunking](/book/retrieval/chunking), where a policy split mid-sentence is a bug that no metric in this chapter will show you.
