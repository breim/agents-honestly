...
[Part IV opened](/docs/retrieval/where-does-the-answer-live) with the observation that an aggregate is a computation rather than a document, and that top-*k* over rows is structurally incomplete for a question that needs all of them. That argument is short and most people accept it immediately.
...
That is not a small gap. It is the difference between a demo and a system, and it repeats a pattern this book has hit twice already: with [tool-count benchmarks](/docs/first-agent/first-tools) averaging three candidate tools, and with [recall benchmarks](/docs/retrieval/ann-indexes) measured without filters. **The published number is measured in a regime you will never deploy in.**
...
| **Tenancy injected by your code** | The `WHERE tenant_id = ?` is appended by the compiler, never written by the model: [the same rule](/docs/retrieval/metadata-and-authorization) as retrieval, and for the same reason. |
...
**Aggregate in SQL, return small.** Never hand 40,000 rows to the model. You are paying [attention](/docs/context/context-as-budget) for data the database could have reduced to one number. If a result exceeds a few dozen rows, that is a signal the query was underspecified, and returning "12,412 rows matched; add a grouping or narrow the range" is more useful than truncating.
...
Now the trace shows what was actually asked, the model can state its assumptions in the reply, and a human who doubts the number has something to check. Given [the acceptance spec's](/docs/first-agent/the-problem) citation requirement, an unsourced number should be treated exactly like an unsourced policy claim.
...
Next: [Building the Ingestion Pipeline](/docs/retrieval/ingestion-pipeline), covering parse, clean, normalize, enrich, embed, index, and keeping it reproducible when the source changes.
