...
Read what that says. The vector is **a column**, alongside the version, the tier, and the supersession timestamp. `pipeline_ver` is there because of the [last chapter](/book/retrieval/embeddings). And the query that uses it is a query:
...
Everything to here has assumed the search is exact. Next: [Approximate Search, Honestly](/book/retrieval/ann-indexes), where it stops being exact and you choose, knowingly, which of recall, latency, memory and build time to give up.
