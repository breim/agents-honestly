...
[Part I](/book/foundations/what-is-an-agent) argued that a loop whose only stopping condition is the model's own judgment has delegated its termination to a probability distribution. Here is what fixing that costs: three constants and two guard clauses.
...
| --- | --- | --- |
| History policy | Keep everything | Long tickets exceed the window ([compaction](/book/context/compaction)) |
| Tool result size | Truncate at 4,000 chars | A blunt cut loses the field that mattered ([result design](/book/tools/result-design)) |
| Parallel execution | All calls at once | One slow tool blocks the turn |
| Tool errors | Text back to the model | Some errors should retry, not inform ([retries](/book/reliability/timeouts-and-retries)) |
| Authorization | `customerId` passed into `runTool` | Easy to forget on the fifth tool ([tenancy](/book/security/tenant-isolation)) |
...
That authorization row deserves emphasis. `runTool` takes `ticket.customerId` and every tool must filter on it, because the model chooses the *arguments*, and a model that has read a document mentioning another account can ask for that account's data. The filter is not a prompt instruction. It is a parameter your code passes and your tool enforces, which is the [invariant-versus-request](/book/first-agent/the-problem) distinction showing up in a function signature.

## What v0 cannot do

Be precise about this, because the list is the table of contents for the rest of the book.

**It dies with the process.** A deploy mid-run loses the run. Ticket #8823 needs a refund, and a crash between "issue credit" and "email the customer" leaves money moved and nobody told. → [Durable execution](/book/durable-execution/why-durable-execution).

**It cannot wait.** If a customer needs to confirm something, there is no version of this loop that holds for three days. A `while` loop cannot survive lunch. → [Human in the loop](/book/human-in-the-loop/approval-gates).

**It has no memory.** Every ticket starts blank. The same customer explaining the same context, forever. → [Memory](/book/context/memory).

**It cannot repeat safely.** Run it twice on #8823 and you have issued two credits. Nothing here is idempotent. → [Idempotency](/book/tools/idempotency).

**It is invisible.** `console.log` is not observability. When the 2-in-20 confident-wrong case happens in production, there is no way to see which lookup went wrong. → [Tracing](/book/observability/what-a-trace-must-answer).

**Its history grows without bound.** Fine for a four-step ticket, fatal for a forty-step investigation. → [Context engineering](/book/context/context-as-budget).
...
Fourteen of twenty resolved, and a list of six things v0 cannot survive. Next: [What a Framework Buys You](/book/first-agent/what-a-framework-buys), an honest accounting of which of the six you would hand to somebody else's library.
