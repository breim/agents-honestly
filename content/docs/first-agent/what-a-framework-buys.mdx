...
[Atlas v0](/book/first-agent/the-loop-by-hand) resolves fourteen of twenty tickets in about eighty lines you wrote yourself. So: was that worth doing, or was it an exercise you could have skipped by importing something?
...
Two catalog entries are worth reading now rather than later, because they harden what you just built without adding a dependency: [ReAct Loop](/book/patterns/control/react-loop), which is the loop from the last chapter with the five bounds that make it survivable, and [Escalation Ladder](/book/patterns/control/escalation-ladder), which turns [the escalation contract](/book/first-agent/the-problem) into named rungs the rest of the book keeps referring back to.
