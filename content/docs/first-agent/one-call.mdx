...
That is not a warm-up exercise. It is the [leftmost rung](/docs/foundations/when-not-to-build-an-agent), evaluated honestly, before we pay for a higher one.
...
The historical response was to ask for JSON in the prompt and parse the result. We know from [Structured Output](/docs/foundations/structured-output) that this is now a solved problem, and the solution is a schema rather than a parser.
...
Run all twenty through the classifier [several times each](/docs/foundations/nondeterminism), since one run tells you nothing, and compare against the outcomes you wrote down. Suppose it lands roughly like this:
...
Next: [One Tool, Then Many](/docs/first-agent/first-tools), function calling from scratch: the schema, the request, the result, the second request.
