...
[The previous chapters](/docs/human-in-the-loop/approval-gates) left the wait as a condition raced against a timer, and promised that the answer to the timer winning is never *"ask again."* Re-raising an approval nobody answered produces a second unanswered approval and consumes more of a budget that is already the [scarce resource in this part](/docs/human-in-the-loop/risk-tiers).
...
**What the reviewer was shown.** [The approval card is assembled at render time](/docs/human-in-the-loop/approval-gates) from state that has since moved: balances settle, documents get superseded, the account changes. Storing a *reference* to the card proves nothing, because re-rendering it next year produces a different card. **Store the rendered card**, as bytes, alongside the decision. It is the only way to demonstrate that the decision was informed rather than merely recorded.

## The event history is most of this, and not all of it

[Part XI observed](/docs/durability/agent-as-workflow) that the event history is the audit log you were going to build, produced as a by-product of replay. That remains true and is worth being precise about, because it is not the whole requirement.
...
Next: [The Vercel AI SDK](/docs/interface/ai-sdk), Part XIII, and what the user sees while a durable backend does its work.
