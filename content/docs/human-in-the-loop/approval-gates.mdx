...
The waiting is solved. [Part X](/book/durable-execution/signals-and-children) reduced "block for three days on a human" to a condition raced against a timer, costing no process, no socket, and no thread. [The previous chapter](/book/human-in-the-loop/risk-tiers) decided which actions get a gate at all.
...
| **The output** | It leaves the building | [Class ⑤ writes](/book/tools/read-and-write-tools): replies, filings | The work is already paid for |
...
Gating **exceptions** is where the [approval budget](/book/human-in-the-loop/risk-tiers) actually survives contact with volume, and it is underused because it requires trusting your risk signals, which is a testable property, not a leap of faith.

Whatever you choose, one structural rule carries over from [Part VII](/book/langgraph/graph-topologies): **the gate contains exactly one side-effecting call and nothing else**, because resume re-executes from the top of the node. A gate that also sends a notification sends it twice.
...
**Every material claim either links to a source record or marks itself as an inference.** *"Policy RET-14 §3"* links to the document at the version that was retrieved. *"Carrier liability unconfirmed"* is labelled as uncertainty rather than presented as fact. This is [the citation requirement from the acceptance spec](/book/first-agent/the-problem) arriving at the approval screen, and it is the difference between a reviewer checking the agent and a reviewer being persuaded by it.
...
Which is why the source link is not decoration. An approval UI that renders agent-authored justification without provenance has built a very efficient channel for [prompt injection](/book/security/prompt-injection) to reach a human with authority: the one participant in the system who can override every other control.
</Aside>
...
| Deliberate | Money, irreversible, [dual control](/book/patterns/security/dual-control) | Longer, and scheduled |
...
[Signals and Children](/book/durable-execution/signals-and-children) has the code. Two choices from it are worth restating in this context:

**Use an update, not a signal.** The reviewer must learn whether their decision was accepted, and an update can carry a [validator](/book/durable-execution/signals-and-children) that rejects a stale or malformed one before it is recorded. That is how expiry gets enforced rather than merely displayed.

**The wait is a condition raced against a timer.** What happens when the timer wins is [the escalation chapter](/book/human-in-the-loop/escalation-and-audit), and the answer is never "retry the approval."

## Atlas, concretely

`issue_credit` above $1,000 gates on **execution**, in a node containing exactly that call. The card is the one rendered above. It accepts four answers, with `Edit` restricted to lowering the amount. Raising it re-enters the tier check and may require [dual approval](/book/human-in-the-loop/risk-tiers), which is a rule the validator enforces rather than the UI suggests.
...
The gate is specified, and both frameworks in this book claim to implement it. Next: [LangGraph Interrupts vs. Temporal Signals](/book/human-in-the-loop/interrupts-vs-signals), on which one owns the wait, and what breaks when both believe they do.
