...
[Part VIII classified tools by reversibility](/docs/tools/read-and-write-tools): pure read, observed read, reversible write, irreversible write, external write. That classification is correct and insufficient here, because it is a property of the **tool** and risk is a property of the **call**.
...
The fourth is [dual control](/docs/patterns/security/dual-control), and it exists for the same reason it exists in banking: some actions should not be within one person's power, including the power of one person who has been socially engineered.
...
**It is not a prompt.** The model does not decide whether something needs approval, and cannot be persuaded that this particular refund is fine. This is [the invariant/request distinction](/docs/first-agent/the-problem) at the policy layer.
...
At Meridian's own volume the same computation comes out loose. [Nine hundred tickets a week](/docs/first-agent/the-problem) is about a hundred and eighty a day, so forty considered approvals would stretch to gating more than a fifth of them. The $1,000 threshold sits far inside that ceiling and produces [twenty-odd decisions a week](/docs/human-in-the-loop/approval-gates), which means it was chosen by what the business treats as consequential, not by what the reviewers can absorb. Scale decides which of the two binds first, and the capacity ceiling is the one nobody writes down.
...
`send_reply` fires about once per ticket resolved. Atlas covers [the first two of Meridian's three categories](/docs/first-agent/the-problem), most of a hundred-and-eighty-ticket day, so approving every outgoing message is **well over a hundred approvals a day against the forty a reviewer can actually consider**, several times the budget computed two sections ago. That is the answer to *why not simply gate replies*: not that it makes Atlas useless as an autonomous support agent, though it does, but that the arithmetic was never available in the first place.
...
Next: [Approval Gates](/docs/human-in-the-loop/approval-gates), blocking indefinitely on a human decision without holding a process, a socket, or a thread open.
