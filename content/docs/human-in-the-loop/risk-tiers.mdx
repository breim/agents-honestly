...
[Atlas](/book/preface#what-youll-build) can now look up an order, issue a credit, and send the customer a reply, and since [Part XI](/book/durability/not-a-background-job) it survives long enough to finish doing them. The lookup needs no one's permission and a half-million-dollar transfer needs a signature; this part is about everything in between.
...
The tiers say which actions want a person. They say nothing about how an agent waits for one. Next: [Approval Gates](/book/human-in-the-loop/approval-gates), where the waiting turns out to be the easy half.
