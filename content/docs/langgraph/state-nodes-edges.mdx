...
In [Atlas v0](/docs/first-agent/the-loop-by-hand), control flow was `for`, `if`, and `break`. Instructions in a function. At run time that structure is invisible: you cannot ask the program which step it's on, you cannot serialize its position, and you cannot resume it in another process. The loop exists only while the stack frame does.
...
| --- | --- |
| It can be written to a store | [Resume a run](/docs/langgraph/persistence) in another process, after a crash or a deploy |
| It can be paused between nodes | [Human-in-the-loop](/docs/human-in-the-loop/approval-gates) that holds no thread open |
...
The merge itself is where this gets interesting and where graphs go subtly wrong. What happens when two parallel nodes both write `messages`? That's the reducer question, and it's [the next chapter](/docs/langgraph/reducers-and-state), because it deserves one.

## Rebuilding Atlas

Now the actual exercise. The [Part VI decomposition](/docs/workflows/the-determinism-test) was nine steps, one of them an agent loop. Here it is as an explicit graph:
...
**Four of six nodes contain no model call.** `check_policy` is the [invariant](/docs/first-agent/the-problem), the credit limit, as a node that cannot be prompted around. The graph didn't make Atlas more autonomous; it made the non-autonomous parts visible.

**The cycle is one edge.** `gather → gather` is the entire agent loop, and it's bounded by `should_continue`, which is the [step cap from v0](/docs/first-agent/the-loop-by-hand) relocated into a testable function.

**Every path ends at `finalize`.** Including escalation. That's the [outcome union](/docs/first-agent/what-a-framework-buys) enforced structurally. There is no way to leave this graph without logging what happened.
...
**It does not survive a crash by itself.** The graph makes position *serializable*; a checkpointer with a durable store is what persists it. In memory, you have the same volatility as the `while` loop with extra structure. That's [the persistence chapter](/docs/langgraph/persistence), and it has a hard limit worth knowing about early.

**Replay is only as deterministic as your nodes.** Re-running from a checkpoint replays the graph, not the world. A node that calls an API gets a fresh answer, and a node with a side effect does it again. Node purity is a property you maintain, not one the framework grants. It is also the hinge on which [durable execution](/docs/durable-execution/why-durable-execution) later turns.

**It doesn't reduce the number of decisions you own.** The [seven decisions](/docs/first-agent/the-loop-by-hand) from the hand-written loop are all still yours: history policy, result truncation, error handling, authorization, termination. They've moved into nodes and edges, which makes them easier to see and no less your responsibility.

## When it earns its overhead

The honest boundary, which [gets its own chapter](/docs/langgraph/node-or-function) later:
...
Next: [Reducers and State Schemas](/docs/langgraph/reducers-and-state), what flows along an edge, what must not, and why this is where graphs get subtly wrong.
