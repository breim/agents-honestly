...
| --- | --- | --- |
| A checkpoint is written | [Resume, replay, time travel](/book/langgraph/persistence) | A serialize plus a store write, per superstep |
| Execution can pause | [Human gate](/book/human-in-the-loop/approval-gates), interrupt | The node must be re-entrant, because resume re-runs it from the top |
| A retry policy applies | Per-node attempts and backoff | The node must be [idempotent](/book/tools/idempotency) |
| An event is emitted | A [stream update](/book/langgraph/streaming-and-control) and a named [trace span](/book/observability/what-a-trace-must-answer) | A name your consumers now handle |
...
Resume re-executes the node from the top. Not from the `interrupt()` line. From the top. Anything that ran before the interrupt runs again, which is why [the human gate topology](/book/langgraph/graph-topologies) is the shape where this rule earns its keep. That is a correctness constraint, and it is the only hard limit in this chapter. Everything else is cost.
...
| --- | --- | --- |
| Makes a bounded model call and returns fields | **Node** | You want the [retry policy](/book/reliability/timeouts-and-retries) and a named span |
...
| Loops over items with no pause inside | **Function**, or a [fan-out](/book/langgraph/graph-topologies) if you need per-item retry | A `for` loop does not need an edge |
...
Same options on `addNode`, camelCased, as with the rest of the graph API in [State, Nodes, Edges](/book/langgraph/state-nodes-edges). The policies are the transferable idea; the spelling is not.
</Aside>
...
[Part VII opened](/book/langgraph/state-nodes-edges) by deferring this. Three questions, all yes-or-no:
...
Zero yeses is a function, and [Part VI](/book/workflows/composition-patterns) is the chapter for it. One yes is usually the functional API. Two or three is a graph.
...
Part VII ends here. Next: [Tools Are APIs for Models](/book/tools/tools-are-apis-for-models), Part VIII, and the design of what an agent is permitted to do.
