...
**Multi-turn conversation.** The thread *is* the conversation. Turn two loads turn one's state. This is what "memory" means at the graph level, and it's [thread state](/book/context/memory), not durable facts, not a knowledge base.
...
That is [Part X](/book/durable-execution/why-durable-execution), and this chapter is its motivation.
...
A run that can be resumed is still a run nobody can watch or halt. Next: [Streaming and Control](/book/langgraph/streaming-and-control), on emitting what the loop is doing, and putting a hard ceiling on how long it may do it.
