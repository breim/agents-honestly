...
You now know that [tool schemas are billed on every request](/docs/foundations/tokens), that [one changed byte at the front of the prompt costs the whole cache](/docs/foundations/prompt-caching), and that `input_tokens` is only the uncached remainder.
...
**Prebuilt agents as a starting point.** You [wrote the loop](/docs/first-agent/the-loop-by-hand); you know what the eighty lines were and which decisions they contained. A prebuilt agent makes those decisions for you, out of sight, and the whole reason Part II exists is so you don't take that trade without seeing it. Use one when you have measured that its choices match yours.

**Document loaders as an ingestion pipeline.** [Part IV](/docs/retrieval/ingestion-pipeline) argued that ingestion's hard problems are lifecycle, not parsing: change detection, deletion, reconciliation, idempotency. A loader gives you parsing. Treat it as one stage of a pipeline you own, not as the pipeline.
...
The architectural rule, and it's the same list [Part II](/docs/first-agent/what-a-framework-buys) said stays yours across every rewrite:
...
**Use LangGraph when [the determinism test](/docs/workflows/the-determinism-test) says you have a genuine agent step and you need what a graph provides:** explicit state, checkpointing, interruption at node boundaries, and a run shape you can inspect. That's a specific problem with a specific answer, and the next chapters are about it.
...
Next: [State, Nodes, Edges](/docs/langgraph/state-nodes-edges). The graph model, and rebuilding Atlas as an explicit state machine.
