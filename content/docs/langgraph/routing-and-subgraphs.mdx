...
And now the payoff for [splitting the schema](/book/langgraph/reducers-and-state) into decision state and transcript:
...
A loop is a conditional edge pointing backward. The `gather → gather` self-edge from [the Atlas graph](/book/langgraph/state-nodes-edges) is the whole agent loop.
...
Confusing these is a real bug. If your only protection is the framework's limit, then hitting it is an **exception**, not an outcome. And [Part II was emphatic](/book/first-agent/the-loop-by-hand) that "the agent could not finish" is a first-class result that routes to a human, not a stack trace in your logs.
...
[Part III argued](/book/patterns/context/subagent-isolation) that noisy work should run in its own context and return only a conclusion, so whatever confusion happened inside doesn't [poison](/book/context/context-poisoning) the main thread.
...
**No:** "this graph is getting long." A subgraph used once, sharing all state, with no independent tests, is a section header with a checkpointing cost, and it makes the run harder to read, because now the trace has a nesting level. That's the [node-or-function question](/book/langgraph/node-or-function) one level up, and the answer has the same shape.
...
**Checkpoints nest.** The runtime checkpoints subgraph state within the parent's checkpoint, which means the [size arithmetic](/book/langgraph/reducers-and-state) compounds and the replay view has depth. Nice for debugging, less nice for storage.
...
The graph is composable now, and it still dies with the process. Next: [Checkpoints, Persistence, Resumability](/book/langgraph/persistence), which justifies everything Part VII has asked of you, and then shows you exactly where it stops.
