...
That is not a gap in the engineering. It is a property of the failure mode: [a retry policy cannot detect a wrong answer](/book/durable-execution/everything-that-breaks), because from the machine's point of view nothing failed. This part is the only one that can.
...
Then the arithmetic makes it worse. [Part X's compounding](/book/durable-execution/everything-that-breaks) applies within a run, and it applies again across a pipeline: three components at 70% each succeed together **34%** of the time. Evaluating a component in isolation does not predict the system, and a demo evaluates one path through one component on one input.

## The benchmarks are not a rescue

The instinct is to reach for a published benchmark instead. This book has now hit the same wall three times: [tool-selection benchmarks averaging three candidate tools](/book/first-agent/first-tools), [recall benchmarks measured without filters](/book/retrieval/ann-indexes), [text-to-SQL measured on small clean schemas](/book/retrieval/sql-is-still-the-answer). The general figure is consistent with all of them: a reported **37% gap** between controlled benchmark performance and real enterprise deployment.
...
That is why Part XIV is seven chapters rather than one, and why it includes [the path](/book/evals/trajectory-evals) and [the cost](/book/evals/cost-and-latency-evals) alongside the answer.
...
| Question | Where |
| --- | --- |
| What counts as success? | [The acceptance spec](/book/first-agent/the-problem), written before any code |
| On which cases? | [Building a Dataset](/book/evals/datasets) |
| Scored how? | [Scoring](/book/evals/scoring) |
| Gating what? | [CI for Agents](/book/evals/ci-for-agents) |
| Watched where? | [Online Evals](/book/evals/online-evals) |
...
Each piece of that loop has a catalog entry carrying the code: [Golden Set](/book/patterns/evaluation/golden-set) for the fixtures, [Trajectory Assertion](/book/patterns/evaluation/trajectory-assertion) for grading the path rather than the answer, [Judge with Rubric](/book/patterns/evaluation/judge-with-rubric) for what only a model can score, [Shadow Run](/book/patterns/evaluation/shadow-run) and [Canary Eval](/book/patterns/evaluation/canary-eval) for the rungs between fixtures and full traffic, and [Online Guardrail](/book/patterns/evaluation/online-guardrail) for the check that is deliberately not an eval.
...
**It cannot make a stochastic system deterministic.** A pass is a sample, not a proof, and [treating a single green run as a guarantee](/book/foundations/nondeterminism) is how a flaky suite gets trusted.

**It can be wrong.** This is the one nothing else checks: [the measurement-validity failure cluster](/book/durable-execution/everything-that-breaks), and the reason [scoring](/book/evals/scoring) spends its length on calibrating a judge rather than on picking one.
...
That set has done real work: it produced the [scores at every version](/book/first-agent/the-loop-by-hand), it caught the two wrong-but-confident answers, and it is why the resolution numbers in this book are numbers rather than impressions.

It is also, honestly, too small. Twenty cases will catch a catastrophe and will not detect a five-point regression, because at that sample size a five-point move is noise. Knowing *which* claims a set can support is [the subject of the CI chapter](/book/evals/ci-for-agents), and pretending a small set supports large claims is the most common way an eval program produces false confidence rather than less of it.

The next chapter is where twenty becomes a set that can carry weight.

---

Next: [The Testing Pyramid for Agentic Systems](/book/evals/testing-pyramid), where exact tests, contracts, replay, statistical evals, fault injection, and red teams each belong.
