...
The join must survive compaction, Continue-As-New, and a UI reconnect. Use the stable run ID from [What a Trace Must Answer](/docs/observability/what-a-trace-must-answer), not a browser session ID.
...
When reviewers disagree, preserve both labels and send the case back to the rubric. [Building a Dataset](/docs/evals/datasets) already established that disagreement often reveals an undefined success criterion. Do not erase that finding by picking the senior reviewer automatically.
...
Next: [What a Trace Must Answer](/docs/observability/what-a-trace-must-answer), the run record that makes every feedback event diagnosable.
