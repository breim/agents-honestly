...
Atlas scored 14/20 in [Part II](/docs/first-agent/the-loop-by-hand) and this book reported it as 70%.
...
Read the fourth row and then re-read [the dataset chapter's ceiling](/docs/evals/datasets) of about a thousand cases per route. Those two numbers agree, and together they say something specific and worth internalising:
...
That last word is the one to watch, because on the frontier it is no longer free. [Current models reject `temperature`, `top_p` and `top_k` outright](/docs/foundations/nondeterminism), so the cheapest variance reduction in this chapter is unavailable for the system you are actually shipping. The split still holds; the price changed. What is left to pin is the [configuration bundle](/docs/production/versioning), and what replaces the sampler is repetition, which means the variance you used to remove for nothing now comes out of the sample sizes above.
...
| [Unsorted JSON, timestamps in prompts](/docs/foundations/prompt-caching) | Genuine sampling at production settings |
| Tool results arriving in racing order | |
| [Judge drift](/docs/evals/scoring) between runs | |
...
| Detectable regression | ~5 points per route, which takes ~1,350 cases per arm. [Atlas is not there yet](/docs/capstone/hardening-it) |
| Gated | Aggregate resolution rate, [cost per resolved ticket](/docs/evals/cost-and-latency-evals), p95 latency, and every [deterministic assertion](/docs/evals/scoring) |
...
Which is the practical argument for [the scoring ladder](/docs/evals/scoring) restated as a CI property: every criterion you move from a judge to an assertion is a criterion that stops needing a thousand cases to say anything.

---

Next: [Online Evals and Guardrails](/docs/evals/online-evals), scoring production traffic, shadow runs, canaries, and blocking the worst outputs before delivery.
