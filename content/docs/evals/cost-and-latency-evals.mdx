...
Three of them, because [Part XIII established](/book/interface/streaming-ux) that they govern different things:
...
Time-to-first-token and total are not substitutes. [A run can be fast in total and feel broken](/book/interface/streaming-ux), and one can take eight minutes and feel responsive. Gating on total alone optimises the wrong half of the experience.
...
The noise band matters and is [the next chapter's subject](/book/evals/ci-for-agents). A quality score that moves two points between identical runs cannot gate on a two-point regression.
...
**Cache retrieval and tool results.** Plus [prompt caching](/book/foundations/prompt-caching) on the stable prefix.

**Cut the retry budget.** Retries are the quietest line item, and [a retry loop around a model call spends money at a rate worth alerting on](/book/durable-execution/everything-that-breaks).

**Consolidate tools.** [Three chained calls collapsed into one](/book/tools/tools-are-apis-for-models) removes two full model turns. That is the largest single lever, and it is a Part VIII decision rather than a Part XIV one.

**Shape results at the source.** [The multiplier on every unnecessary field is the number of turns after it](/book/tools/result-design).
...
And the human wait is excluded from the action-required budget, because [a three-day approval is not latency](/book/durable-execution/signals-and-children). It is the system working. Including it would make the correct behaviour score as the worst.
...
Part XIV has now produced a number for quality, for the path, and for the bill. Next: [Continuous Integration for Agents](/book/evals/ci-for-agents), on whether any of them holds still enough to stop a release.
