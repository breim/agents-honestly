...
You have [a set](/book/evals/datasets). Now something has to decide whether each case passed, and the instrument you pick determines whether the resulting number means anything.
...
Four assertions and one judgement. [Part IV made this point](/book/retrieval/sql-is-still-the-answer) about warehouse numbers: twenty questions with independently verified answers, `assert result == 184532`, no rubric, no judge, no agreement rate. That opportunity exists more often than it is taken.
...
This is the [measurement-validity failure](/book/durable-execution/everything-that-breaks) in its purest form: a number produced by a component whose accuracy nobody established, used to gate releases.
...
**A missing definition of success.** If the [acceptance spec](/book/first-agent/the-problem) does not say what a good outcome is, the judge is being asked to invent the standard and then apply it.
...
Every instrument here grades the answer. Next: [Evaluating the Path, Not Just the Answer](/book/evals/trajectory-evals), because a correct reply reached by seven wasteful steps is not a result you would ship twice.
