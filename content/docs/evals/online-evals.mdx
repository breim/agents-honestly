...
[The previous chapter](/book/evals/ci-for-agents) ended on a debt: detecting a two-point regression needs roughly 8,400 cases per arm, which no CI budget will buy, so something else has to catch that class of change.
...
The general guidance assumes the shadowed system is a function: same input, compare outputs, no consequences. An agent has [side effects](/book/tools/read-and-write-tools).
...
You cannot judge every turn, because [the judge cost arithmetic](/book/evals/datasets) that capped golden sets applies with more force to unbounded traffic. So online scoring is stratified sampling, and the stratification is where the value is.

Sample a baseline percentage of everything, and then **deliberately oversample the places failures live**:

- Runs that ended in [escalation](/book/human-in-the-loop/escalation-and-audit)
- Runs that hit a retry, or a [repeated identical error](/book/evals/trajectory-evals)
- Runs in the long tail of duration or cost
- Runs where the agent [stated uncertainty](/book/interface/trust-patterns)
- Every run on a route that changed this week

Then close the loop: the failures you find become [the production and replay buckets](/book/evals/datasets) of the golden set. **Online evals are the mechanism by which the offline suite learns**, and a team doing one without the other has either a suite that stops improving or a stream of findings nothing regresses against.
...
Scoring production traffic collects opinions from users as well as from judges. Next: [Learning From Production Feedback](/book/evals/learning-from-production-feedback), on turning corrections and complaints into evidence without treating a click as truth.
