...
The invariants are shared: no unauthorized effect, no duplicate effect, bounded execution, traceable outcome, and safe escalation. [Failure Injection](/book/reliability/failure-injection) and [Adversarial Evals](/book/security/adversarial-evals) differ in method and ownership, not in the safety properties they assert.
...
Next: [Building a Dataset](/book/evals/datasets), the examples and labels that make the statistical layers mean something.
