...
The exceptions are worth naming, because they are exactly where you will need this page. About a dozen short illustrations run in one track only, where the point is a mechanism rather than an idiom, most of them Python, in [Part VII](/docs/langgraph/state-nodes-edges), where the graph API is itself the subject. And [the Temporal LangGraph plugin](/docs/durability/langgraph-on-temporal) is Python-only in the product, not merely in the book.
...
Concurrency looks similar and differs in ways that matter for [fan-out](/docs/patterns/scale/fan-out) and [sliding windows](/docs/patterns/scale/sliding-window).
...
It bites hardest on exactly the values this book cares about. A `Retry-After` of zero means *retry now*; `retry_after_ms(err) or fallback` silently discards it and sleeps. A [token budget](/docs/patterns/cost/token-budget-enforcement) of `0` means *exhausted*; `budget or default` hands back the default. A retrieved `k` of `0` means *retrieve nothing*.
...
That last row is why TypeScript samples in this book narrow with `instanceof` before reading fields off a caught value, and Python samples do not need to. It also means [the error classifier](/docs/reliability/error-taxonomy) has slightly more defensive work to do on the TypeScript side.

## Hashing and canonical JSON

Used for [idempotency keys](/docs/tools/idempotency) and [config bundle hashes](/docs/production/versioning), where an unstable serialization is a real bug rather than a style issue.
...
The three things [durable workflow code must not call directly](/docs/durable-execution/determinism-and-retries).
...
**Context managers.** Python's `async with` is the natural home for [transaction scoping](/docs/security/tenant-isolation) and lease handling; TypeScript uses a callback-taking function (`withTenant(id, async tx => ...)`). Same guarantee, different shape.
...
The one place to read both is the [dispatcher](/docs/capstone/building-it), because the numbered checks line up one-to-one and seeing the same eight steps in two languages is a reasonable check that you have understood what each one is for.
</Aside>

---

Next: [Temporal Pattern Index](/docs/appendix/temporal-pattern-index), every pattern in the Temporal catalog, mapped to where this book uses it.
