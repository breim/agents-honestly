...
**Should this process become an agent?** [Process discovery and value](/docs/foundations/process-discovery-and-value)
...
| If | Build |
| --- | --- |
| The steps are known in advance | A [workflow](/docs/workflows/the-determinism-test). Model calls at the judgment steps only |
| The next action requires having seen what the last one returned | An agent loop. Varied *content* is not this test |
| You can enumerate the operations | Enumerate them. [Do not add `run_python`](/docs/security/sandboxing) |
| It is a single classification or extraction | [One structured call](/docs/foundations/structured-output), no loop |

**What tier of architecture?** [Reference architecture](/docs/production/reference-architecture)
...
| Yes | Yes | Tier 3, [durable execution](/docs/durable-execution/why-durable-execution) |

**One agent or several?** [When not to](/docs/multi-agent/when-not-to)

| Reason given | Verdict |
| --- | --- |
| Context genuinely exceeds a window after compaction and selective retrieval | **Split**, isolation |
| A component must be denied tools or data another holds | **Split**, [trust boundary](/docs/security/prompt-injection) |
...
| "It parallelizes" | True only for disjoint items, that is [map-reduce](/docs/patterns/scale/mapreduce-tree), not conversation |
| You cannot yet state the handoff contract and termination condition | **Not yet** |

**Supervisor or handoff?** [Supervisor and handoff](/docs/multi-agent/supervisor-and-handoff)
...
**Function, MCP, A2A, or durable workflow?** [MCP Is Not A2A](/docs/mcp/remote-agents-and-a2a)
...
**Where does the answer live?** [Part IV](/docs/retrieval/where-does-the-answer-live)
...
| "Why is this account at risk" | [Graph traversal](/docs/graphs/when-relationships-are-the-query) |
| "Where is order 4921 right now" | Live API |
| "What did the customer say last Tuesday" | The conversation store |

**Which context technique?** [Part III](/docs/context/context-as-budget)

| Problem | Technique |
| --- | --- |
| Window fills over a long run | [Compaction](/docs/context/compaction) at a checkpoint, not edge-trimming |
| The same payload is re-sent every turn | Summarize to the fields used; [pointers over payloads](/docs/context/dynamic-selection) |
| Cache keeps missing | [Stable prefix](/docs/patterns/context/stable-prefix), nothing before the breakpoint moves |
| A subtask is noisy | [Sub-agent isolation](/docs/patterns/context/subagent-isolation) |
| The model reasons from its own wrong number | [Typed scratchpad](/docs/context/context-poisoning), only tool-returned values |
...
| Symptom | Fix |
| --- | --- |
| Right document, wrong chunk | [Parent–child chunks](/docs/patterns/retrieval/parent-child-chunks) |
| Exact IDs and codes never match | [Lexical search](/docs/retrieval/lexical-search) alongside semantic |
| Top-5 is noisy but top-50 contains it | [Two-stage rerank](/docs/patterns/retrieval/two-stage-rerank) |
| Conversational phrasing retrieves poorly | [Query rewriting](/docs/patterns/retrieval/query-rewriting) |
| Stale answers on time-sensitive questions | [Freshness routing](/docs/patterns/retrieval/freshness-routing) |
| Cross-account leakage | [Filtered retrieval](/docs/patterns/retrieval/filtered-retrieval), and prefer separate indexes |

## Tools

**Tool class, and what it obliges** [Part VIII](/docs/tools/read-and-write-tools)
...
| ④ Irreversible write | `issue_credit` | [Idempotency key](/docs/tools/idempotency) + paired read + compensation |
...
| Idempotency key | **Your code**, [never the model](/docs/tools/idempotency) |
...
**API, browser automation, or visual computer use?** [Computer-Using Agents](/docs/tools/computer-using-agents)
...
| Source repository that must be inspected and changed | [Coding workspace](/docs/tools/coding-agents-and-workspaces) with a pinned revision, path and command policy, external checks, and patch handoff |
...
**Retry this?** [Error taxonomy](/docs/reliability/error-taxonomy)

| Failure | Retry | Where |
| --- | --- | --- |
| Connection reset, 5xx, overloaded | Yes, backoff + full jitter | SDK only |
| 429 | Yes, obey `Retry-After` | SDK, with [admission control](/docs/reliability/backpressure) behind it |
| Context length exceeded | **No**, deterministic | Compact and re-enter |
| Malformed tool arguments | **No transport retry** | [Return it to the model](/docs/tools/errors-as-instructions) |
| Content refusal | **No** | Escalate |
| Authorization denied | **No** | Escalate |
| Timeout on a write | **No** | Paired read first |
| Wrong answer | Meaningless | [Evals](/docs/evals/eval-discipline) |

**Which timeout?** [Timeouts and retries](/docs/reliability/timeouts-and-retries)
...
**Which fallback rung?** [Fallbacks and breakers](/docs/reliability/fallbacks-and-breakers)
...
**Workflow or activity?** [Part XI](/docs/durability/agent-as-workflow)
...
**Which durable runtime?** [Two Durable Runtimes](/docs/interface/two-durable-runtimes)
...
**In-flight runs during a change** [Rollout](/docs/production/rollout)
...
**What tier is this call?** [Risk tiers](/docs/human-in-the-loop/risk-tiers)
...
**Which control for which threat?** [Part XVII](/docs/security/threat-model)
...
| Agent as superuser | [Delegation](/docs/security/identity): `sub` = user, `act` = agent | Filtering results after a privileged read |
...
**What evidence must exist at each lifecycle stage?** [AI Governance](/docs/production/ai-governance)
...
**Which eval, for which question?** [Part XIV](/docs/evals/eval-discipline)

| Question | Instrument |
| --- | --- |
| Did my change break something? | [CI suite](/docs/evals/ci-for-agents) on fixtures |
| Did my *code* change break something? | [Replay](/docs/observability/replay-debugging), deterministic |
| Did the model change under me? | [Frozen canary](/docs/observability/drift), on a schedule |
| Is it good on real traffic? | [Online scoring](/docs/evals/online-evals) on a sample |
...
**Diagnosing a quality drop** [Drift](/docs/observability/drift)
...
**Which cost lever?** [Cost engineering](/docs/production/cost-engineering)
...
**Who owns this state?** Model, graph, workflow, or database. Most architectural mistakes are [state at the wrong altitude](/docs/the-map).
...
Next: [Glossary](/docs/appendix/glossary), terms this field uses inconsistently, defined once.
