...
1. **A workflow pattern.** A system whose control flow you wrote, with model calls at specific steps. The opposite of an agent, in the sense above. [Part VI](/book/workflows/the-determinism-test).
2. **A durable workflow.** A Temporal (or equivalent) execution whose state survives crashes via event-history replay. [Part X](/book/durable-execution/why-durable-execution).

They are orthogonal. An agent can run *inside* a durable workflow, and [Part XI](/book/durability/agent-as-workflow) is entirely about doing exactly that. This book says **"durable workflow"** wherever the second sense could be misread; a bare "workflow" means the first, and the part you are reading usually settles it: Part VI is the pattern throughout, Parts X–XII the execution.

**Memory.** Three unrelated things share this word, and conflating them is the most common source of confused design discussions in the field. [Part III](/book/context/memory) separates them:
...
This book says **effectively-once**, and adds the caveat that makes it honest: it holds only inside [the dedup window](/book/reliability/idempotency-in-practice).
...
**Compensation.** Undoing a completed effect by performing an opposite one, a credit note rather than an erasure. Distinct from a rollback, which pretends the thing never happened. [Part XI](/book/durability/compensation).

## Failure

**Hallucination.** The model asserting something not supported by its input. Useful, but too narrow to be the diagnosis you actually need: in production the same wrong output more often comes from [a poisoned context](/book/context/context-poisoning), [a stale document](/book/security/untrusted-retrieval), or a tool that returned something wrong. Calling all of it hallucination points the fix at the model when it usually belongs upstream.
...
**Drift.** Any of four different things, and the entire job of [that chapter](/book/observability/drift) is telling them apart: the model changed, the data changed, the users changed, or you changed. "We have drift" is a symptom report, not a diagnosis.

**Deterministic.** Same input, same output. Note that a durable workflow's determinism requirement is narrower and more specific: [the same *sequence of commands* on replay](/book/durable-execution/determinism-and-retries), which permits plenty of computation that is not referentially transparent.
...
The nuance worth keeping: a well-built guardrail has [two tiers](/book/patterns/evaluation/online-guardrail), and only one of them is mitigation. Deterministic assertions against recorded state, *is this chunk ID in the set we retrieved*, have no false negatives and qualify as **invariants**, which is why they may block. Classifiers may only flag. Conflating the tiers is how a detector ends up standing where a boundary belongs.

An **invariant** is enforced in code the model cannot route around and is the reason an action is impossible. [A prompt instruction is neither.](/book/first-agent/the-problem)

**Semantic failure.** The class with no exception type: a `200 OK`, a clean trace, valid arguments, and a wrong answer. Named in [the error taxonomy](/book/reliability/error-taxonomy) because a taxonomy without it lets you believe your error rate is your failure rate.
...
`issue_credit(4471, 250000)` is not harmful output. It is a valid call requested by the wrong party, which is why better safety training does not address it. [Part XVII](/book/security/prompt-injection).
...
**Tenant.** The isolation boundary, which is not always "customer." For Atlas it is one of Meridian's customer accounts. What matters is that it is a property of the *run's identity*, not a field in a mutable state object, and that it rides every asynchronous hop. [Part XVII](/book/security/tenant-isolation).
...
1. **Durable-execution replay.** Re-running workflow code against a recorded event history so it converges to the same position. A correctness mechanism. [Part X](/book/durable-execution/determinism-and-retries).
2. **Debugging replay.** Re-running your code against recorded model and tool responses, to test a fix deterministically. [Part XV](/book/observability/replay-debugging).
...
**Trace / span / run.** A trace is the record of one run. A span is one operation inside it. In agents the trace has to answer [eight specific questions](/book/observability/what-a-trace-must-answer) that a service trace does not.
...
**Escalation.** Handing a task to a person. In this book it is a **first-class outcome**, not a failure path, including as a [fallback rung](/book/reliability/fallbacks-and-breakers) when infrastructure is degraded. An escalation that arrives as "I couldn't handle this" has followed the rule and failed the contract.

**Human in the loop vs. on the loop.** *In* the loop: a person approves before the action. *On* the loop: a person is notified and can intervene after. The [tiers](/book/human-in-the-loop/risk-tiers) are largely a policy for choosing between them, plus the observation that both spend a finite budget of human attention.
...
Next: [Further Reading](/book/appendix/further-reading), primary sources worth your time.
