...
[The coordination tax chapter](/docs/multi-agent/the-coordination-tax) priced this in tokens and seconds. The short version, because it drives everything else:
...
**1 · Context genuinely does not fit.** Not "the prompt is long": the task involves more material than any single window can hold while still reasoning well, and the [budget chapter's](/docs/context/context-as-budget) tools have been exhausted. Auditing four hundred files, or reading two hundred documents to answer one question. Here isolation is a [context-engineering technique](/docs/patterns/context/subagent-isolation) that happens to be implemented with agents, and it is the strongest case on the list.

**2 · A trust or authority boundary must be enforced.** The agent reading untrusted content must not be the one holding the tools, which is [the dual-LLM pattern from Part XVII](/docs/security/prompt-injection), and it is a security architecture rather than a team of collaborators. The "agents" here do not negotiate; one is quarantined by construction.
...
The first row is the common one and worth being blunt about. **A "specialist agent" is usually a system prompt.** If the specialization is a different instruction and a narrower tool list, that is a [routing decision inside one agent](/docs/workflows/routing-and-orchestration), which costs one conditional edge instead of a message boundary, a serialization format, a failure mode, and a fresh context.
...
The alternative is not "one giant prompt." It is the thing [Part VI spent a whole part on](/docs/workflows/the-determinism-test): a workflow whose control flow you wrote, with model calls at the steps that genuinely need judgment.

| You wanted | Build |
| --- | --- |
| A research agent and a writer agent | One graph: retrieve node → draft node → check node |
| A router agent | [A routing edge](/docs/workflows/routing-and-orchestration) |
| A critic agent | [An evaluator-optimizer loop](/docs/workflows/evaluator-optimizer) with a bounded iteration count |
| A team of specialists | One agent with conditional tool exposure |
| Parallel document analysis | [Map-reduce over isolated contexts](/docs/patterns/scale/mapreduce-tree), same shape, no negotiation |

The last row is the one that resolves the parallelism argument honestly. Fanning out over two hundred documents *is* correct and it *is* faster, and it does not require agents that talk to each other. Each worker gets one document, returns a structured result, and never learns the others exist. The coordination tax is near zero because there is no coordination: the [map-reduce pattern](/docs/patterns/context/subagent-isolation) is deterministic orchestration over isolated model calls, and it is what most successful "multi-agent" systems turn out to be underneath.
...
**Write the contract as a schema.** What each agent receives and returns, [validated](/docs/foundations/structured-output), versioned, and tested independently. Prose handoffs are where inter-agent misalignment lives.
...
**Trace across the boundary.** One trace ID spanning every agent, or debugging becomes archaeology across three systems. The [tracing requirements](/docs/observability/what-a-trace-must-answer) get strictly harder here, which is another cost in the tax column.
...
You can build an agent and know [why it is a loop](/docs/first-agent/the-loop-by-hand). You can [budget its context](/docs/context/context-as-budget), [put its answer where it belongs](/docs/retrieval/where-does-the-answer-live), and [tell when it should not have been an agent at all](/docs/workflows/the-determinism-test). You can [design tools that survive a model calling them](/docs/tools/tools-are-apis-for-models), make them [safe to repeat](/docs/tools/idempotency), [survive a crash mid-run](/docs/durable-execution/why-durable-execution), [put a person in the loop without drowning them](/docs/human-in-the-loop/risk-tiers), [know whether a change made it better](/docs/evals/eval-discipline), [find out what happened afterwards](/docs/observability/what-a-trace-must-answer), [contain what a hostile ticket can do](/docs/security/prompt-injection), and [pay a bill you can explain](/docs/production/cost-engineering).
...
Next: [The Brief](/docs/capstone/the-brief), one system, built end to end, with every constraint from the preceding twenty parts applied at once.
