...
Part XIX ends where it started, with one agent and a fan-out that never negotiates, which is the shape the rest of this book has been building all along. Next: [The Brief](/book/capstone/the-brief), Part XX, one system built end to end, with every constraint from the preceding twenty parts applied at once.
