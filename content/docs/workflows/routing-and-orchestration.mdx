...
Routing has quietly been in this book since [Atlas v0](/book/first-agent/one-call): a classification, then a `switch`. It reappeared as the [context selection](/book/context/dynamic-selection) mechanism, and it will reappear again as a cost mechanism in a moment.
...
Two failure modes when it's missing or lazy. If your router has no unknown category, the classifier is *forced* to pick one of the four. And it will, confidently, because that's what the schema demands. (This is exactly the [honest-null problem](/book/foundations/structured-output) from Part I: a schema with no way to say "I don't know" guarantees you will never be told.) If the unknown route exists but silently drops the ticket, you have built a quiet hole in your queue.
...
So gate on confidence where the output is a choice. Where the output is prose, a self-report won't do. You need an actual [quality check](/book/workflows/evaluator-optimizer): a judge, a verifier, a grounding assertion.
</Aside>
...
That is the same three-band shape as the [entity-resolution review queue](/book/graphs/entity-resolution) and the same cheap-wide-then-expensive-narrow structure as [reranking](/book/retrieval/hybrid-and-reranking). At this point in the book it should be a reflex: never spend the expensive operation on cases a cheap one settles.
</Aside>
...
The agent version is fine too, when [the determinism test](/book/workflows/the-determinism-test) says so. The failure is building the second while believing you built the first, which happens because the diagram is identical and the terminology is shared.
...
Two things follow. The [context budget](/book/context/context-as-budget) stays small in every window, including the orchestrator's, which only ever sees conclusions. And confusion is contained: whatever mess a worker got into while searching stays in the worker's discarded window rather than [poisoning](/book/context/context-poisoning) the main thread. That's [sub-agent isolation](/book/patterns/context/subagent-isolation), and it is a correctness mechanism at least as much as a scaling one.
...
**The router that grew to fourteen branches.** Three added last month, each a small PR. That is the [demotion signal](/book/workflows/the-determinism-test): your input space isn't enumerable and you're hand-maintaining a classification. Collapse the branches into a broader category and let a model handle the variety inside it.
...
One shape is left, and it is the one this part's title appears to forbid. Next: [Evaluator–Optimizer Loops](/book/workflows/evaluator-optimizer), generate, critique, revise, bounded tightly enough that it never becomes open-ended.
