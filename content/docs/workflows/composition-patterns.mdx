...
The [last chapter](/docs/workflows/the-determinism-test) placed each step in a column. This one connects them, and the claim of Part VI is that a handful of composition shapes cover most of what teams reach for an agent to do.
...
One line to internalize: **parallelism buys latency, never cost.** Three parallel calls cost exactly what three sequential calls cost. If your problem is the bill, parallelism won't help. [Fewer or smaller calls](/docs/foundations/tokens) will.
...
| Join | How | Watch out |
| --- | --- | --- |
| **Concatenate** | Put all N results in one context | [Context explosion](/docs/context/context-as-budget); 40 summaries is a new budget problem |
...
The default should be **aggregate structurally**, and it's available far more often than people assume. If each branch returned [structured output](/docs/foundations/structured-output), the join is `filter`, `groupBy`, and `count` rather than another model call. Forty ticket classifications don't need a model to combine them; they need a `GROUP BY`.

Reach for synthesis when the branches produced prose that genuinely has to become one piece of prose. And when you do, remember it is a model call reading model output, [a place where errors get laundered into confident summary](/docs/context/context-poisoning), so pass the underlying facts through rather than only the branch conclusions.
...
**Best-effort with disclosure.** Return what succeeded, and say what didn't. Correct for most analysis. The critical part is the disclosure: `{ results: [...37], failed: 3, reason: "timeout" }` is honest; returning 37 results as though the set were complete is the [silent truncation](/docs/graphs/graph-retrieval) failure that this book keeps finding in new places.
...
A detail from [prompt caching](/docs/foundations/prompt-caching) that bites exactly here. A cache entry becomes readable only once the first response *begins streaming*. So firing forty identical-prefix calls simultaneously means all forty pay full price. None can read what the others are still writing.
...
**Chaining is wrong** when one call already does the job, or when a later step needs information only obtainable after seeing an earlier *result you didn't anticipate*. That's [an agent](/docs/workflows/the-determinism-test), not a chain.
...
**Voting** on exactly one step: the escalate-or-answer decision, sampled three times. It's a binary with asymmetric costs, since [the acceptance spec](/docs/first-agent/the-problem) holds escalation recall higher than answer accuracy. Three calls to reduce missed escalations is the cheapest insurance in the system.
...
Next: [Routing and Orchestration](/docs/workflows/routing-and-orchestration), classify first then dispatch. Orchestrator/worker without handing control flow to a model.
