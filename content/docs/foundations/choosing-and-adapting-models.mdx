...
A larger advertised window says that the request may fit. It says nothing about whether the model will use a fact buried at token 180,000. Test the positions and document shapes you will send, using the dilution checks from [The Context Window](/book/foundations/context-window).
</Aside>
...
Do not let an in-flight durable run drift onto a new model. Pin the profile for ordinary behavior and re-evaluate live authorization at the moment of action, the same split [Rollout and In-Flight Migration](/book/production/rollout) applies to every other configuration change.
...
Every lever in this chapter assumes the agent should exist. Next: [When Not to Build an Agent](/book/foundations/when-not-to-build-an-agent), which is the question Part I has been circling, and the cheapest agent is the one you replaced with three if-statements.
