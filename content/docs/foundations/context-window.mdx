...
The fix is a policy decided *before* you hit the wall: summarize, truncate oldest-first, or move history into retrievable storage. Choosing under pressure means choosing badly. See [Rolling Summary](/book/patterns/context/rolling-summary) and [Context Compaction](/book/patterns/context/context-compaction).
...
The defenses are structural, not prompt-level: keep verified state in a typed object rather than in the transcript ([Structured Scratchpad](/book/patterns/context/structured-scratchpad)), give noisy subtasks their own throwaway window ([Sub-Agent Context Isolation](/book/patterns/context/subagent-isolation)), and make tool results the source of truth over anything the model said about them.
...
There is a fifth thing that is not a lever but sits alongside all of them: **prompt caching**. It does not reduce how many tokens you send, it reduces what they cost, by roughly an order of magnitude for the cached portion. It has one hard constraint that dictates how you must order your prompt, and it is the subject of the [next chapter](/book/foundations/prompt-caching).
...
Caching was set aside there as orthogonal, because it changes the price rather than the volume. Next: [Prompt Caching](/book/foundations/prompt-caching), where that discount is large enough to design around and quiet enough to lose without noticing.
