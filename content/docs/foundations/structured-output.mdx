...
Recall from [the loop](/docs/foundations/what-is-an-agent) that generation is one token at a time, and each step produces a score for every token in the vocabulary. Constrained decoding inserts one operation into that step.
...
This matters most where structured output is most tempting. An agent that emits a typed `issue_credit` payload *feels* verified in a way that free text does not. It isn't. The type system stops at the boundary of your process; the model's claim about the world is still a claim. [Tool design](/docs/tools/schema-and-granularity) and [approval gates](/docs/human-in-the-loop/approval-gates) exist because of exactly this gap.
...
Response format is for when there is exactly one thing you want back. Tool schemas are for when the model's job includes deciding *which* thing, which is [the definition of an agent](/docs/foundations/what-is-an-agent), so the tool path is the one the rest of this book lives on.
...
Next: [Why Your Agent Is Flaky](/docs/foundations/nondeterminism), sampling, the reproducibility you can and cannot buy, and what it does to your idea of a test.
