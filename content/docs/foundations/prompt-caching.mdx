...
The [previous chapter](/book/foundations/context-window) ended with four levers for context, and a note that caching sits beside them rather than among them. Send less, compress, externalize, isolate. Those change *how many* tokens you send. Caching changes what those tokens **cost**, by roughly an order of magnitude, and changes nothing else.
...
For an agent this compounds the same way [everything else does](/book/foundations/tokens): the loop resends its whole history on every iteration, so the cached prefix is read once per model call, not once per user turn. A four-call turn reads the cache four times. The break-even arrives inside a single turn.
...
Caching settles what the prompt costs on the way in. Next: [Structured Output](/book/foundations/structured-output), and the other half of that contract: getting a usable value back out.
