...
The [last chapter](/book/foundations/structured-output) bought you a guarantee about shape. This one takes away the guarantee you actually wanted: that the same input produces the same output.
...
Every row is a bug you can fix, and none of them is the model being moody. **Diagnose before you blame the dice.** This is what [tracing](/book/observability/what-a-trace-must-answer) is for, and the single most common outcome of adding it is discovering that your "nondeterministic" agent was deterministically receiving different inputs.

## Why one flipped token becomes a different run

For a single completion, variance is cosmetic: a synonym, a reordered clause. In [the loop](/book/foundations/what-is-an-agent) it is structural, because the model's output is not the answer. It is the **control flow**.
...
Two engineering consequences follow immediately, and both get their own parts of this book. Repeated steps must be safe to repeat, because a retried run may take a different path through the same side effects. That's [idempotency](/book/tools/idempotency). And a run that dies at step six must not restart at step one and take a *different* path. That's [durable execution](/book/durable-execution/why-durable-execution).
...
- **Must hold often enough.** Correctness, grounding, tone. Measured as a rate against a threshold you chose deliberately. This is [scoring](/book/evals/scoring), and it is a different discipline from unit testing.
...
A 90% feature is a fact about one model on one task. Next: [Choosing and Adapting Models](/book/foundations/choosing-and-adapting-models), on picking the one whose ninety percent falls in the right place, and knowing when a model change is the wrong fix.
