...
*Transactional* is the cell teams forget, because it looks like Tier 1. The run finishes inside the request and still moves something in the world. It needs no queue and no state table. Nothing outlives the response. What it needs is everything that makes an effect safe to repeat: [idempotency keys](/docs/reliability/idempotency-in-practice), the [dispatcher choke point](/docs/security/least-privilege), and an audit record. A client retrying a timed-out `POST` does not know whether your agent finished, and neither does your agent.

**Tier 3: durable.** Runs take hours or days, pause for humans, and move money. Now a crash mid-run has to be survivable without repeating effects, and you want [durable execution](/docs/durable-execution/why-durable-execution) rather than a queue plus hope.
...
| **Postgres** | Always. Runs, results, [idempotency](/docs/reliability/idempotency-in-practice), [audit](/docs/human-in-the-loop/escalation-and-audit) | — |
| **Object storage** | You handle files or store rendered artifacts | Low |
| **Vector index** | [Retrieval over unstructured text](/docs/retrieval/where-does-the-answer-live) is genuinely needed | Low, but see the warning below |
| **Queue** | Tier 2 | Low |
...
| **Policy / dispatcher layer** | The first write tool | **Very high**, [Part XVII](/docs/security/threat-model) all lands here |
...
**Evals are worse.** Not because they are hard to build, but because their value is comparative: the number only means something against yesterday's number. A team that adds evals in month six has an eval suite and no history, which tells them how they are doing and not whether it is getting better. [Datasets](/docs/evals/datasets) start accumulating on day one or they start six months late.

**Durable execution changes the code, not the deployment.** Adopting Temporal is not an infrastructure decision you can defer behind an interface. It imposes [determinism constraints on how nodes are written](/docs/durable-execution/determinism-and-retries). Retrofitting it means rewriting the agent, which is why the tier question deserves a real answer up front even if you build Tier 2 first.

**The dispatcher is where the whole security part lands.** [Taint](/docs/security/prompt-injection), [argument scoping](/docs/security/least-privilege), [delegation](/docs/security/identity), [idempotency keys](/docs/tools/idempotency), [risk tiers](/docs/human-in-the-loop/risk-tiers). Every one of those is a check in the path between the model's tool call and the effect. If tool calls dispatch from four different places in your codebase, you have four places to implement all of it, and you will implement it in three.
...
Adding it is cheap. The expensive part is that it quietly commits you to an [ingestion pipeline](/docs/retrieval/ingestion-pipeline), a chunking strategy, a re-embedding path, [retrieval evals](/docs/retrieval/retrieval-evals), a permissions model, and, as [Part XVII established](/docs/security/untrusted-retrieval), a new attacker-writable surface.

[Where does the answer live](/docs/retrieval/where-does-the-answer-live) is the chapter that decides this, and its answer is frequently "a SQL query." Deploy the index when you have a retrieval problem, not when you have a corpus.
</Aside>
...
**Every arrow leaving toward the world passes through the dispatcher.** That is not an aesthetic choice; it is the thing that makes the security part implementable and the [trace](/docs/observability/what-a-trace-must-answer) complete.

**State is a plane, not a box.** The run's state lives in several stores with different durability and retention, and, as [tenancy](/docs/security/tenant-isolation) and [compliance](/docs/security/pii-and-compliance) both showed, forgetting one of them is how the boundary leaks. Draw them together so the list stays visible.
...
- **One place for retries, timeouts, and the retry budget**, which is [exactly what the last part asked for](/docs/reliability/timeouts-and-retries), and impossible if six services each call the provider directly.
- **One place for [fallbacks](/docs/reliability/fallbacks-and-breakers)** to another model or provider.
- **One place cost is attributed**: per run, per tenant, per feature.
- **One place [model versions are pinned](/docs/production/versioning)**, so "we upgraded the model" is a config change with a rollback rather than a deploy across six repos.
- **One place [region and retention terms](/docs/security/pii-and-compliance) are enforced.**
...
**Too early:** a multi-agent topology ([the next part](/docs/multi-agent/when-not-to) is about why), a vector database, a fine-tuned model, a custom orchestration framework, and a Kubernetes install for a service handling four requests a minute.
...
| Orchestrator | LangGraph nodes, [running as Temporal activities](/docs/durability/langgraph-on-temporal) |
...
| Vector index | Yes, two namespaces, split by [trust](/docs/security/untrusted-retrieval) |
| Object storage | Rendered approval cards, customer attachments |
| Durable execution | Temporal. The approval pause is the reason |
| Tracing | OTel from day one, [chunk IDs and tool arguments recorded](/docs/observability/otel-genai) |
| Evals | [CI suite](/docs/evals/ci-for-agents) plus a growing dataset from production |
...
Next: [Versioning Prompts, Models, and Graphs](/docs/production/versioning), treating all three as deployable artifacts with rollback, not as strings in a file.
