...
Do not let an edited reply silently become training data or policy. [Learning From Production Feedback](/book/evals/learning-from-production-feedback) already separates weak signals, labels, adjudication, and release. The interface should preserve those categories.
...
Return to the baseline from [Process Discovery and Value](/book/foundations/process-discovery-and-value).
...
Part XVIII ends with Atlas deployed on purpose: components chosen rather than collected, versioned artifacts, a rollout ladder, a cost position picked deliberately, a gate with evidence behind it, and a support team whose work has been rearranged around it. Next: [The Coordination Tax](/book/multi-agent/the-coordination-tax), Part XIX, on what happens when the work boundary crosses another autonomous system as well as another team.
