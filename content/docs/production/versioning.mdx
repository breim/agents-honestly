...
**`toolCatalogueHash`.** A tool description is prompt text that happens to live in a schema. [Part VIII said so](/book/tools/tool-discovery) for quality reasons and [Part XVII said so](/book/security/least-privilege) for security reasons. Editing a description changes the agent's behavior exactly as editing the system prompt does, and it usually ships in a PR reviewed as a code change.

**`corpusBuild`.** The [drift chapter's](/book/observability/drift) most common culprit. A re-index changes what the agent reads without changing a byte of your code.

**`region`.** Same model string, different endpoint, different serving stack, and a [compliance-relevant](/book/security/pii-and-compliance) difference besides.
...
Every bundle promoted to production should carry the [eval run](/book/evals/ci-for-agents) that justified it, stored with the bundle. This is what makes six-months-later questions answerable: *why did we ship this* has an artifact, not a memory.
...
The last row is the honest limit. You cannot roll back the provider's model. If behavior on a pinned model string shifts, and [it does](/book/observability/drift), your available moves are to switch to a different pinned version if one is offered, to adapt the prompt around the new behavior, or to fall back to [another rung of the ladder](/book/reliability/fallbacks-and-breakers). Which reframes what the version pin is for: it is not a guarantee, it is a **record**, and its value is that when behavior changes you can prove the change was not yours.

<Aside type="caution" title="Rollback does not un-send">
A bad prompt shipped at 14:00 and rolled back at 14:40 still produced forty minutes of replies that reached customers, credits that were issued, and [memory writes](/book/context/memory) that persist.

This is [the class ⑤ problem](/book/tools/read-and-write-tools) at deploy scale, and it is why the next chapter treats rollout as a blast-radius decision rather than a percentage. Fast rollback bounds the damage; it does not reverse it, and any durable state the bad version wrote needs its own cleanup, which is a thing to have thought about before you need it.
</Aside>
...
**A config change is a reviewed change.** Same PR flow, same approvals. Otherwise you have built a production-mutation channel that bypasses every control in [Part XVII](/book/security/least-privilege), and a prompt is exactly the kind of thing an attacker or a well-meaning hurried colleague would want to edit.

**A config change runs the eval suite.** Promotion is gated on the score, not on someone's read of the diff.

**A config change is a deploy event.** It appears in the same timeline as code deploys, because [drift diagnosis](/book/observability/drift) starts with "what changed" and a change invisible to that query is a change that will be blamed on the model.
...
| Graph | Git SHA + `graphVersion` | Code | [Worker versioning](/book/durable-execution/determinism-and-retries) |
...
Every artifact is versioned and rollback is a real move. Neither says what becomes of a run that started before the change. Next: [Rollout and In-Flight Migration](/book/production/rollout), on shipping while thousands of them are mid-execution.
