...
A run resolves its [configuration bundle](/docs/production/versioning) once, when it starts, and uses that bundle until it terminates. Not per turn, not per node, per run.

The reason is the one [the fallback chapter](/docs/reliability/fallbacks-and-breakers) gave for not swapping models mid-conversation, and it applies to every field in the bundle. A run whose first twelve steps were decided under one prompt and whose last eight are decided under another is not on version A or version B. It is a splice, its outcome is attributable to neither, and it pollutes whatever comparison you were trying to run.
...
For the graph and worker code, the mechanism is [worker versioning versus patching](/docs/durable-execution/determinism-and-retries). Old executions finish on old code by default, and patching exists for the cases where they must not. The bundle follows the same principle for the parts that are data rather than code: pinned at start, carried in the run's state, honored on resume.

## The paused run is the interesting case

Run D is where the rule needs a policy rather than a default. A run that paused Tuesday for [human approval](/docs/human-in-the-loop/approval-gates) and resumes Thursday will resume into a world with a new prompt, possibly a new model, and possibly a new policy.
...
The third row is not negotiable and it is the one that gets missed. If a [risk tier](/docs/human-in-the-loop/risk-tiers) threshold was lowered on Wednesday because a credit limit was too generous, a run resuming Thursday must not issue Tuesday's credit under Tuesday's rules. This is the same argument [identity](/docs/security/identity) made about re-deriving authorization at the moment of action rather than restoring it from a checkpoint: **authorization and policy are re-evaluated at the point of effect; everything else can be pinned.**
...
| [Input tokens per turn](/docs/observability/cost-accounting) | Minutes | The prompt got expensive |
| Escalation rate | ~an hour | The agent lost confidence or authority |
| Error and refusal rate | ~an hour | — |
| [Sampled quality score](/docs/evals/online-evals) | Hours to days | The thing you actually care about |
...
The **holdout** is worth the extra line. A small set of tenants that never receives canaries gives you a stable baseline to compare against when a metric moves. Otherwise a slow global shift and a bad canary look identical, which is [the drift diagnosis problem](/docs/observability/drift) arriving during a rollout.

## Blast radius, not percentage

The last idea, and it is the one that separates agent rollouts from feature flags.

A 1% canary on a read-only feature exposes 1% of users to a worse experience. A 1% canary on an agent that issues credits and sends email exposes 1% of users to **irreversible actions taken by an unvalidated configuration**, and [rollback does not un-send](/docs/production/versioning).

So gate the rollout on the same axis [risk tiers](/docs/human-in-the-loop/risk-tiers) already gave you:
...
Never move the prompt and the model in the same rollout. When the metric shifts you will not know which caused it, and, because you [cannot roll back a model](/docs/production/versioning), you may not be able to unwind half of it.
...
Next: [Cost Engineering](/docs/production/cost-engineering), model routing, cache tiers, batch APIs, and choosing your position on the cost/quality frontier deliberately.
