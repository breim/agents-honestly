...
The chart is the same statement with the constant left in. Bars are what each turn sends; the line is the same conversation with history capped by [compaction](/docs/patterns/context/context-compaction).
...
**Turn count is the strongest lever you have.** Because input cost is roughly quadratic in turns, a change that removes four turns from a twenty-turn run saves far more than the same percentage change in price-per-token. This is why [tool granularity](/docs/tools/schema-and-granularity) and [result design](/docs/tools/result-design) are cost decisions and not only quality ones.

**Retries and failures are billed at full price.** A run that fails at step eighteen paid for eighteen steps. [The retry budget](/docs/reliability/timeouts-and-retries) is a cost control that happens to live in the reliability chapter.
...
| **Shrink what's re-sent each turn** | Large; compounds with turn count | [Context engineering](/docs/context/context-as-budget) |
...
| **[Prompt caching](/docs/foundations/prompt-caching)** | Cache reads run roughly an order of magnitude below input price | Prefix discipline |
| **[Batch endpoints](/docs/patterns/cost/batch-api-offload)** | ~50% off, for latency-tolerant work | Asynchrony |
| **[Model routing / cascade](/docs/patterns/cost/model-cascade)** | Commonly 40–70% overall | A routing decision that can be wrong |
| **[Early exit](/docs/patterns/cost/early-exit)** | Removes whole runs | A confidence signal you trust |
...
| [Summarize for compaction](/docs/context/compaction) | Faithfulness, no invention | Mid |
...
Nothing here is a runtime guess. The node knows what it is, so the model is a property of the node, which also means the routing is testable, reviewable, and rollback-able in the [gateway config](/docs/production/reference-architecture) rather than emergent from a classifier's mood.

The one runtime decision worth making is **escalation**: run small, and promote to large when a validator fails, a confidence check trips, or the [risk tier](/docs/human-in-the-loop/risk-tiers) says this call matters. That is a cascade with an honest gate, and the gate is the part to build carefully.
...
The `degrade` rung is what makes this usable. A budget with only a hard stop produces truncated runs at the worst moment; one that degrades first produces a cheaper, slightly worse completion, which is [the same shape as a fallback](/docs/reliability/fallbacks-and-breakers), applied to money instead of availability.
...
**3 · Measure the trade with evals, not with intuition.** [The eval suite](/docs/evals/eval-discipline) is what makes "20% cheaper, 2% worse on the rubric" a sentence you can say. Without it, every cost change is a gamble settled by whoever complains loudest.
...
Attribution, per-run, per-tenant, per-node cost recorded on the trace, is [Cost Accounting](/docs/observability/cost-accounting) in Part XV, and it is a prerequisite rather than a follow-up. The [model gateway](/docs/production/reference-architecture) is where it gets recorded, which is the fifth reason that box is worth building on day one.
</Aside>
...
Next: [The Operating Manual](/docs/production/operating-manual), dashboards, incident patterns, on-call for a probabilistic system, and the first ninety days.
