...
| [Canary](/docs/observability/drift) score | Any sustained drop | 3 |
...
- **Per bundle version** makes the [rollout](/docs/production/rollout) attribution automatic: the alert arrives already saying which configuration is responsible.

Two things deliberately absent: **no alert on a single bad output**, because in a probabilistic system one bad output is the expected tail and paging on it trains people to ignore the pager. And **no alert on total spend**, which is a lagging monthly artifact. [Tokens per minute is the leading indicator](/docs/observability/cost-accounting).
...
| --- | --- | --- |
| **Provider degradation** | Latency and 5xx up, [breaker](/docs/reliability/fallbacks-and-breakers) tripping, canary also affected | Confirm breaker opened; check fallback rate; status page |
...
| **Corpus regression** | Retrieval hit rate down, [cited-chunk turnover](/docs/observability/drift) up | Check the last ingestion diff; repoint the index build |
...
The single highest-value habit is that **the first question in every one of these is "what changed"**, and it should be answerable in one query across code deploys, config repoints, index builds, and provider incidents. A timeline that only shows code deploys will send you looking for a bug that was a re-index, which is why [config changes have to be deploy events](/docs/production/versioning).
...
**The fix is usually not a deploy.** A repointed manifest, a lowered `k`, a disabled fallback rung, a raised cap. This is good, it makes mitigation fast, and it means the config-change path needs to be safe at 3am, which is the [reviewed-and-eval-gated](/docs/production/versioning) discipline paying off precisely when someone wants to skip it.
...
**Days 1–14 · See it.** [Tracing](/docs/observability/what-a-trace-must-answer) with the eight questions answerable, `config_hash` on every run, [cost attribution](/docs/observability/cost-accounting) at the gateway, and a `runs` table with terminal states. Ship nothing else until a single run can be reconstructed.

**Days 15–30 · Bound it.** Run budget with a degrade rung, turn cap, [risk tiers](/docs/human-in-the-loop/risk-tiers) with tier 1+ gated, [argument scoping](/docs/security/least-privilege) on every write, and the [dispatcher choke point](/docs/production/reference-architecture) if it is not already there. This is where the blast radius stops being unbounded.

**Days 31–60 · Measure it.** A [CI eval suite](/docs/evals/ci-for-agents) with a real dataset, a [frozen canary](/docs/observability/drift) on a schedule, [online scoring](/docs/evals/online-evals) on a sample, and the tier-2 proxy metrics on a dashboard with their baseline variance. Now a change has a number.

**Days 61–90 · Break it.** [Failure injection](/docs/reliability/failure-injection) in CI, the [fallback path evaluated as its own variant](/docs/reliability/fallbacks-and-breakers), a first game day, and the incident-pattern runbook written from what you actually saw in the first two months.

The sequence is deliberately the opposite of how these systems usually get built: capability first, legibility later. [The reference architecture chapter](/docs/production/reference-architecture) named the reason: capability demos well and legibility does not, so capability gets funded first. Ninety days spent the other way round produces a system that is less impressive in month two and still operable in month twelve.

## Where Part XVIII leaves you

Five chapters on everything that has to be true before you point users at it.

An [architecture sized by two questions](/docs/production/reference-architecture) rather than by ambition. [Versioning](/docs/production/versioning) of the resolved bundle, because the git SHA does not identify what the model saw. [Rollout](/docs/production/rollout) that pins configuration per run and gates on blast radius rather than percentage. [Cost engineering](/docs/production/cost-engineering) that starts with turn count rather than model choice. And an operating posture that watches the tier nothing else watches.
...
Next: [The Go-Live Readiness Review](/docs/production/go-live-readiness-review), the evidence-backed gate for authority, recovery, quality, cost, security, ownership, and rollback.
