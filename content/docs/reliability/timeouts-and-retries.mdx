...
Retrying a permanent failure is pure cost, and half of a well-behaved policy is knowing what to abandon immediately. The [error taxonomy](/book/reliability/error-taxonomy) draws the full map; the operational summary is this:
...
| Context length exceeded | **No.** Retrying sends the same oversized prompt | Fix or fail: [compact](/book/context/compaction) and re-enter |
...
| Authorization denied | **No** | [Escalate](/book/human-in-the-loop/approval-gates) |
| Write tool, unknown outcome | **Only if [idempotent](/book/tools/idempotency)** | Next chapter |
| Semantic: it answered, wrongly | Not a retry at all | Evals, not reliability |

The two "no" rows people get wrong are the third and the fifth. A context-length error is *deterministic*: the same request will fail identically three more times, and each attempt bills you for a prompt the provider rejected. And a malformed-arguments error should not be retried by your code at all. [Returning it to the model as an instruction](/book/tools/errors-as-instructions) is the recovery path, and wrapping a transport retry around it produces three identical failures before the model ever gets to see one.
...
For streaming, the useful bound is **inactivity** rather than total duration. If no token has arrived in fifteen seconds, the stream is dead regardless of how long it has been running. A total cap still belongs there as a backstop, because [an agent that loops](/book/langgraph/graph-topologies) will happily consume any budget you leave open.
...
That last clause is the part that feels wrong and is the point. During a widespread outage, retries are not helping. Everything is failing, so retrying mostly converts one failure into three. Passing the failure through immediately is both cheaper and faster to recover from, and it hands the decision to the layer that can actually do something useful: [a fallback or a circuit breaker](/book/reliability/fallbacks-and-breakers).

For agents, budget the money as well as the count. Retries consume the same [token budget](/book/patterns/cost/token-budget-enforcement) as productive work, and a run that has spent 80% of its allowance on retried attempts should stop rather than finish poorly at full price.
...
This is what [checkpointing](/book/langgraph/persistence) and [durable execution](/book/durable-execution/why-durable-execution) were for, arriving here as a cost argument rather than a correctness one:
...
The precondition for the third row is that steps be individually replayable, which means [reads are safe to repeat and writes are idempotent](/book/tools/read-and-write-tools), the property the next chapter is entirely about.
...
Next: [Idempotency in Practice](/book/reliability/idempotency-in-practice). Keys, dedup windows, and the exactly-once illusion you are actually building.
