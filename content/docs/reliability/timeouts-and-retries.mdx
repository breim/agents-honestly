...
Resuming from step fourteen is only sane if step fourteen can run again without paying twice. Next: [Idempotency in Practice](/book/reliability/idempotency-in-practice), where Part VIII's key meets dedup windows and the exactly-once story stops being true.
