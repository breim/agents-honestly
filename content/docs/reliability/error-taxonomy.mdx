...
| --- | --- | --- | --- |
| **Transient** | Socket reset, 5xx, 529 overloaded, 429 | Yes, one layer, with backoff | [Timeouts and retries](/book/reliability/timeouts-and-retries) |
| **Permanent** | Context length exceeded, 400 malformed, unknown tool, schema violation | **Never** | Fail the step; fix in code |
| **Policy** | Content refusal, authorization denied, [tier requires approval](/book/human-in-the-loop/risk-tiers), [taint ceiling](/book/security/prompt-injection) | **Never** | [Escalation](/book/human-in-the-loop/escalation-and-audit) |
| **Budget** | Token cap, run deadline, [retry budget](/book/reliability/timeouts-and-retries) exhausted, max turns | No. That is what exhausted means | [Cost engineering](/book/production/cost-engineering) |
| **Semantic** | Wrong answer, invented policy, [poisoned context](/book/context/context-poisoning) | Meaningless | [Evals](/book/evals/eval-discipline), [tracing](/book/observability/what-a-trace-must-answer) |

Three of the five rows are frequently misfiled, and each misfiling has a signature cost.

**Context length exceeded is permanent, not transient.** It arrives as a `400` in the middle of a long run, looks like an API error, and gets retried. The same oversized prompt fails identically three more times and you are billed for each rejection. The recovery is [compaction](/book/context/compaction) and re-entry, which is a different code path entirely.

**A content refusal is policy, not transient.** The reflex is to rephrase and try again. That is the reflex of someone building a workaround for a control. If a refusal is wrong, that is a prompt or a product problem to fix deliberately; if it is right, retrying until it passes is the [worst possible response](/book/security/prompt-injection).
...
Two fields carry the design. `code` is **yours**, stable across vendor rewording. Your metrics, alerts, and [retry policy](/book/reliability/timeouts-and-retries) key on it. And the adapter captures `retryAfterMs` at the boundary because it is available exactly there and nowhere later.
...
| **Permanent: the model's mistake** | Does not retry | **The error, [written as an instruction](/book/tools/errors-as-instructions)** |
...
A semantic failure produces a `200 OK`, a well-formed result, a clean trace, valid arguments, and a wrong answer. Nothing raises. Your error rate does not move. Your latency does not move. Every dashboard in the [operating manual](/book/production/operating-manual) stays green while the product gets worse.

It has appeared in nearly every part of this book under different names: [poisoning](/book/context/context-poisoning), [drift](/book/observability/drift), a [retrieval miss](/book/retrieval/retrieval-evals), a [wrong-but-plausible tool result](/book/tools/result-design), an [injected instruction obeyed](/book/security/prompt-injection). What unites them is the shape: **the system reports success and the outcome is wrong.**
...
Semantic failure emits no signal, so it needs a different apparatus: [graded evals](/book/evals/scoring) that compare output against a rubric, [claim grounding](/book/evals/trajectory-evals) against what the tools actually returned, [online monitoring](/book/evals/online-evals) of production quality, and [traces detailed enough](/book/observability/what-a-trace-must-answer) to reconstruct why.
...
| `429` with `Retry-After` | Transient | Honor the header; feeds [backpressure](/book/reliability/backpressure) |
...
| Taint ceiling / tier gate | Policy | [Approval request](/book/human-in-the-loop/approval-gates), not an error |
| Run cost cap | Budget | Degrade at 70%, clean stop at 100% with a partial result |
| Wrong refund policy quoted | **Semantic** | Nothing here catches it. [Eval suite](/book/evals/ci-for-agents) and grounding checks |
...
Four of the five classes want a retry of some kind, and the word retry has been doing a lot of unexamined work. Next: [Timeouts, Retries, and Backoff](/book/reliability/timeouts-and-retries), where getting the numbers wrong turns a ninety-second blip into an afternoon.
