...
That is what this chapter is about, and it is the last of the three reliability mechanics: [retries](/docs/reliability/timeouts-and-retries) control how you re-attempt, [breakers](/docs/reliability/fallbacks-and-breakers) control when you stop attempting, and admission control decides what gets attempted at all.
...
Now recall the shape of agent traffic from [the cost chapter](/docs/production/cost-engineering): the whole transcript is re-sent every turn, so a run's input tokens grow roughly with the square of its turn count while its request count grows linearly.
...
A `429` is the provider deciding for you, and it is the worst place to find out: you have already paid the round trip, the retry machinery engages, and, as [the retry chapter](/docs/reliability/timeouts-and-retries) covered, that is exactly when amplification starts.
...
Which is why [the retry budget](/docs/reliability/timeouts-and-retries) and admission control are one system, not two. Count retries against the same quota, and when the retry budget is exhausted, fail through. The [breaker](/docs/reliability/fallbacks-and-breakers) is a better answer than a queue at that point.
</Aside>
...
**Downstream, remember tools are rate limited too.** Your database, your search index, and the third-party APIs behind your tools all have limits, and an agent that fans out over two hundred documents can flatten a service that was fine with human traffic. [Downstream rate limiting](/docs/patterns/scale/downstream-rate-limiting) belongs in the same dispatcher as everything else from [Part XVII](/docs/security/least-privilege).

**Bound the fan-out itself.** A [map-reduce over documents](/docs/patterns/scale/mapreduce-tree) with unbounded parallelism is a self-inflicted burst. Cap it, and cap it against the token budget rather than a thread count.

**Shed by value, not by arrival order.** When you must drop work, drop the cheapest to lose: a speculative pre-fetch before a user's live question, a nightly backfill before an SLA-bound ticket. This requires knowing what a run is worth, which is the same per-run value the [cost cap](/docs/production/cost-engineering) was derived from.
...
Next: [Failure Injection](/docs/reliability/failure-injection), kill the worker mid-run, fail the third tool call, do it on purpose before production does it for you.
