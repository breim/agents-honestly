...
| --- | --- | --- |
| **Provider** | 429 with `Retry-After` | The header is honored; [admission](/docs/reliability/backpressure) reacts |
| | 5xx / overloaded burst | [The breaker opens](/docs/reliability/fallbacks-and-breakers) before retries amplify |
| | Timeout with no response | Not retried blindly; the [paired read](/docs/reliability/idempotency-in-practice) resolves it |
...
| **Tool** | Error result | Returned to the model [as an instruction](/docs/tools/errors-as-instructions), not retried |
...
| | Approval arrives after the [dedup window](/docs/reliability/idempotency-in-practice) | **The Friday-to-Monday bug** |
| **Adversarial** | A ticket containing an [injection payload](/docs/security/prompt-injection) | Taint holds; the capability ceiling denies |
| | A [poisoned retrieved document](/docs/security/untrusted-retrieval) | Trust routing holds; high-authority path unaffected |
...
Nothing raises. The agent does not retry, does not escalate, does not log anything unusual. It proceeds, reasons correctly from a false premise, and produces a confident, well-formed, wrong outcome. That is [the semantic class](/docs/reliability/error-taxonomy), the one with no exception type, and the majority of what actually goes wrong.

This injection is the only way to test the defenses built for it:

- Does the [re-read-before-acting](/docs/context/context-poisoning) discipline catch the stale value?
- Does the tool that [re-derives the amount from `order_id`](/docs/tools/idempotency) reject the mismatch?
- Does the grounding check notice the reply cites a number no tool returned?
- Does the [risk tier](/docs/human-in-the-loop/risk-tiers) computed from a corrupted amount still route correctly?
...
The injector belongs where the calls are made, in the [model gateway](/docs/production/reference-architecture) and the tool dispatcher. It should be a normal part of those code paths, disabled by configuration rather than compiled out.
...
A [taxonomy](/docs/reliability/error-taxonomy) that says what to do next, and admits a fifth class that never raises. [Retries](/docs/reliability/timeouts-and-retries) at one layer, budgeted, from a checkpoint rather than from the start. [Idempotency](/docs/reliability/idempotency-in-practice) with a window sized to the human pause rather than the retry policy. [Breakers and a fallback ladder](/docs/reliability/fallbacks-and-breakers) where the fallback is understood to be a different system. [Admission control](/docs/reliability/backpressure) that decides who gets the quota before the shortage. And now the practice that turns all of it from design intent into observed behavior.
...
Next: [The Threat Model](/docs/security/threat-model), what an attacker actually wants from your agent, and which of your components they will use to get it.
