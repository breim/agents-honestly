...
For trusted internal code, a disposable worktree inside a restricted runner may be sufficient. For unknown repositories, generated code, or package scripts, use the [execution and credential boundary](/docs/security/sandboxing): disposable VM or equivalent isolation, no ambient secrets, restricted egress, bounded compute, and a broker for authorized operations.
...
Part VIII ends here. Next: [What MCP Is and Is Not](/docs/mcp/what-mcp-is), when the integration belongs to another owner and becomes a shared protocol boundary.
