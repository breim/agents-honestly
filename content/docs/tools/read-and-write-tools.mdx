...
| | Read | Write |
| --- | --- | --- |
| Retry on failure | Freely | Only if [idempotent](/book/tools/idempotency) |
...
| Undo | Nothing to undo | Needs a [compensation](/book/durability/compensation) you wrote |
| Wrong argument | A wasted turn and some tokens | Money moved, mail sent |

Read the middle rows carefully, because they are load-bearing for everything built so far. [Node retry policies](/book/langgraph/node-or-function), [replay from a checkpoint](/book/langgraph/persistence), [parallel tool calls in one turn](/book/first-agent/first-tools), and, in Part X, durable execution's habit of re-running an activity after a crash. **Every one of those mechanisms assumes safe repetition.** They are correct for reads by default and correct for writes only when you have done additional work.
...
Every one of those is a real shape, and the last two arrive most often by [wrapping an existing endpoint 1:1](/book/tools/tools-are-apis-for-models), because HTTP `GET` handlers with side effects are common and nobody minded until the caller became a model with a retry policy.
...
This book has a name for that already: [invariants versus requests](/book/first-agent/the-problem). Every incident in this genre is that distinction, restated as a bill. The specific corollaries worth carrying:
...
Sorting a write by how hard it is to undo assumes you know whether it happened. Next: [Idempotency](/book/tools/idempotency), where the connection drops before the answer comes back, and the design has to make that question stop mattering.
