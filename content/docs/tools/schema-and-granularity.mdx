...
Every rule so far has applied to all tools equally, which is the last time that will be true. Next: [Read Tools and Write Tools](/book/tools/read-and-write-tools), where one half can be retried all afternoon and the other half spends money.
