...
If an error is a prompt because it stays in the window, so is everything that succeeds. Next: [Tool Result Design](/book/tools/result-design), on shaping what comes back, given that it is re-sent on every turn until the run ends.
