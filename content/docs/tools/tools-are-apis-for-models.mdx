...
A catalogue derived from the ticket set settles which tools exist. It settles nothing about how wide each one should be. Next: [Schema and Granularity](/book/tools/schema-and-granularity), why `execute_sql(query)` is a bad tool and `list_unpaid_invoices(customer_id)` is a good one.
