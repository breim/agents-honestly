...
And the third case is the one no error handler catches: **latency degrading under load with no error returned at all.** Nothing throws. Requests just take four times longer, timeouts start firing somewhere downstream, and your dashboards show a healthy error rate. Timeouts are the only instrument that sees this, which is [why the previous chapters spend so long on them](/docs/durable-execution/determinism-and-retries).
</Aside>

**The tool call ends in "unknown."** The third state from [idempotency](/docs/tools/idempotency): the connection died and most timeouts happen on the *response*, so the operation probably succeeded. Without a paired read, no amount of infrastructure resolves it.

**A poison input never terminates.** A ticket that makes the model loop: call a tool, read the result, call the same tool. Bounds catch it, which is what [bounds are for](/docs/first-agent/the-loop-by-hand), and the run still costs a full step budget to learn nothing.

**Time itself fails.** The approval that waits three days outlives the token that authenticated it and the [idempotency key window](/docs/tools/idempotency) that would have deduplicated the retry. Long waits are not just long. They cross the expiry boundary of everything around them.
...
- A [tool result truncated](/docs/tools/result-design) mid-payload and reported as complete.
- A [selector that dropped the right document](/docs/context/dynamic-selection), so the model answered from the second-best source.
- A [run that halted on a bound](/docs/first-agent/the-loop-by-hand) and returned whatever text was lying around, marked handled.
- A [workflow task retrying forever](/docs/durable-execution/determinism-and-retries) while the execution's status says `RUNNING`.
- A [tool that found nothing](/docs/tools/errors-as-instructions) and reported it as an error, or found something and reported nothing.
- A [crashed run](/docs/langgraph/persistence) whose checkpoint sits in a table with nothing scheduled to notice it. Durable, and permanently stopped.
...
| A plan that violates a rule, or never converges | **Not addressed.** That is [Part VI](/docs/workflows/the-determinism-test), and the bounds in Part XXI. |
| Agents overwriting each other | **Not addressed.** That is [Part XIX](/docs/multi-agent/when-not-to), which mostly argues against having them. |
| Injection, exfiltration, unauthorized action | **Not addressed at all.** That is [Part XVII](/docs/security/threat-model). A replayable journal of a breach is still a breach. |
...
Next: [Why Durable Execution](/docs/durable-execution/why-durable-execution). Retries plus a queue plus a cron job is a distributed system you did not design. This is the one you would have arrived at.
