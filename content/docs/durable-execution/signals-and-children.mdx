...
Three days of waiting, and no process is running. This is the mechanism underneath [approval gates](/docs/human-in-the-loop/approval-gates), the chapter that spends its length on *when* a human should be asked. The durable answer to *how you wait* is the eight lines above.
...
The catalog turns the last two chapters into recipes: [Signal With Start](/docs/patterns/durability/signal-with-start) for the inbound event that may arrive before the workflow exists, [Request-Response via Update](/docs/patterns/durability/update-request-response) for the console that needs an answer back, [Updatable SLA Timer](/docs/patterns/durability/sla-timer) for the deadline that moves, and [Continue-As-New for Memory](/docs/patterns/durability/continue-as-new-memory) for the history ceiling this chapter named.

---

Part X ends here. Next: [Not a Background Job](/docs/durability/not-a-background-job), Part XI, where the model finally shows up, and where putting it in the wrong half of this machinery breaks everything above.
