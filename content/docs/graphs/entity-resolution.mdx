...
The [last chapter](/docs/graphs/modeling-entities) decided what a Customer *is*. This one decides which records *are* a given Customer, and those are completely different problems.
...
A false merge is not a data-quality issue. It is a **correctness and possibly a confidentiality failure**: a traversal scoped to one customer now reaches another's tickets, which is precisely the boundary [Part IV insisted](/docs/retrieval/metadata-and-authorization) must never be crossed by a ranking decision. And it is sticky: once other systems have referenced the merged identifier, unpicking it is a migration.
...
And note the shape of ① → ②. Cast wide with something cheap, then apply something expensive to a small candidate set. That is now the **fifth** appearance of this pattern in this book: [ANN then rescore](/docs/retrieval/ann-indexes), [select then rerank](/docs/context/dynamic-selection), [retrieve then cross-encode](/docs/retrieval/hybrid-and-reranking), block then match. It is not a coincidence; it is the general answer whenever an accurate operation is too expensive to run everywhere.
...
That gives you three things: you can undo a bad merge without a data-recovery project, you can audit why two records were joined, and, connecting to [bitemporal modelling](/docs/graphs/modeling-entities), the merge itself is a fact with a validity window, which is exactly what you need when two companies actually do merge in the real world and the records *should* join as of a date.
</Aside>
...
**Downstream sanity checks.** Does any account have two current owners? Does a cluster span two tax IDs? These are ontology violations from [last chapter](/docs/graphs/modeling-entities), and they catch merge errors that pairwise metrics miss.
...
Next: [Graph Retrieval](/docs/graphs/graph-retrieval), on traversal, neighbourhoods, and returning a subgraph small enough to fit in a prompt.
