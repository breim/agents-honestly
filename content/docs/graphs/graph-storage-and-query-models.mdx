...
The fixed query is reviewed for bounds and per-hop authorization. The result is a small typed object rather than a graph serialization dumped into the context window. This is [Tools Are APIs Designed for Models](/docs/tools/tools-are-apis-for-models) applied to a graph.
...
Next: [Entity Resolution](/docs/graphs/entity-resolution). "OpenAI", "Open AI", and "OpenAI, Inc." are still one company no matter which storage model you chose.
