...
You have entities, relationships, and a resolution pipeline that keeps them coherent. Now an agent asks a question, and there is an interface problem: a graph query returns a **subgraph**, and a prompt takes **text**, with a [budget](/book/context/context-as-budget) measured in a few thousand tokens.
...
And **hub nodes** make it abrupt rather than gradual. One node connected to everything, whether a status, a country, or a popular part number, turns a traversal that passes through it into a scan. This is the practical cost of the [attributes-as-entities mistake](/book/graphs/modeling-entities): a `Status: Open` node connects every open ticket in the company, so two hops from any ticket reaches every open ticket in the company.
...
**4 · Precomputed community summaries.** For whole-corpus questions ("what themes appear across our at-risk accounts"), summarize clusters offline and retrieve the summaries. That's [the next chapter's](/book/graphs/graphrag) territory, and it's expensive.

Notice that the first pattern is a **parameterized query**, exactly like the [SQL tools](/book/retrieval/sql-is-still-the-answer) from Part IV. The same discipline applies for the same reasons: you write the traversal, the model supplies the entry point, and it cannot construct a query that walks the whole graph.
...
This is [result design](/book/tools/result-design) applied to graphs, and the rule generalizes: **traverse in the graph, compute in the graph, return the conclusion.** Return the underlying nodes only when the model needs to quote or cite them, and then return the five that matter, not all sixty.
...
Every traversal needs three limits: **maximum depth**, **maximum nodes**, and **a timeout**. That is the same discipline as [the agent loop](/book/first-agent/the-loop-by-hand), for the same reason: an unbounded traversal is an unbounded query.
...
In [Part IV](/book/retrieval/metadata-and-authorization), the filter was applied once: this principal may see these chunks. A traversal is different, because **you can enter at an authorized node and walk to an unauthorized one.** The principal may see the Acme account. Two hops away is a shared supplier; one hop from there is another customer's order. Nothing about the entry check prevents that.
...
One closing reminder, because a graph feels authoritative in a way a vector index doesn't: it is still a copy, built at ingestion time, and everything from [Where Does the Answer Live](/book/retrieval/where-does-the-answer-live) applies.
...
A traversal that returns something is not a traversal that returns something true. Next: [Evaluating Graph Quality](/book/graphs/evaluating-graph-quality), where one false edge produces a clean path and a confident wrong answer.
