...
**Invalidate, don't delete.** A superseded fact gets an end to its validity window; it does not disappear. This is the [memory chapter's](/book/context/memory) `supersedes` field taken seriously, and it's what makes contradiction resolvable instead of a coin flip: highest authority, then most recent *as of the time being asked about*.
...
This also makes the graph honest about staleness in a way an ordinary index isn't. From [Part IV](/book/retrieval/where-does-the-answer-live): every index is a stale copy. A bitemporal graph is a stale copy **that knows it**. It can tell you a fact is from a sync three days ago, rather than presenting it as current truth.

## Model the traversals, not the domain

The scoping rule, carried from [the last chapter](/book/graphs/when-relationships-are-the-query) and worth making concrete.
...
Worth noticing: the metric definitions from [SQL Is Still the Answer](/book/retrieval/sql-is-still-the-answer) and the entity definitions here are the same *kind* of thing: the business's meaning, written down once, owned by someone, versioned, and testable.
...
Next: [Graph Storage and Query Models](/book/graphs/graph-storage-and-query-models), on relational edges, property graphs, RDF, Cypher, and SPARQL without confusing storage with meaning.
