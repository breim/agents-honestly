...
The agent loop you built in [Part II](/book/first-agent/the-loop-by-hand) already does multi-hop retrieval. That's what the Acme trace was: `get_account`, then `get_open_tickets` and `get_order_volume` chosen *because* of what the first call returned. Hops made of tool calls instead of edges.
...
| Determinism | [Varies per run](/book/foundations/nondeterminism) | Exact |
...
This is where the money gets wasted, and the book's position from the [preface](/book/preface) is that most teams reaching for a graph are here.

**"The answer spans two chunks."** That is a [chunking](/book/retrieval/chunking) problem. Parent/child retrieval fixes it for a fraction of the cost, and a graph will not help because the relationship you need is *contiguity in a document*, which chunking already models.
...
[Entity resolution](/book/graphs/entity-resolution) is the unglamorous work that decides whether the graph is usable, and it is most of the project. Budget accordingly, and be suspicious of any plan where it isn't the largest line item.
</Aside>
...
**2 · Log the shape of what it does.** From [tracing](/book/observability/what-a-trace-must-answer): how many hops per ticket, which sequences repeat, how often does a multi-hop investigation time out or halt. This is the measurement the decision needs.
...
This is the same discipline as [not building an agent](/book/foundations/when-not-to-build-an-agent), applied one layer down: take the cheapest mechanism that answers the question, and escalate only when a measurement, not an intuition, says it can't.
...
Before anything can be resolved, the thing being resolved has to be defined. Next: [Entities, Relations, and Ontology](/book/graphs/modeling-entities), on deciding what a Customer is once, so five systems stop each keeping their own answer.
