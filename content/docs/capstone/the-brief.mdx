...
Every row on the right is a decision that belongs at a specific layer of [the map](/docs/the-map), and the mistake this chapter exists to prevent is answering them at the wrong one: putting an authorization rule in a prompt, a durability requirement in a retry loop, or a tenancy boundary in application code.
...
| --- | --- | --- |
| **Interface** | Approval cards a support lead can act on in seconds; streaming; a visible "who is handling this" | [Part XIII](/docs/interface/generative-ui-and-approvals) |
| **Agent runtime** | Turn caps, stopping conditions, routing between the four ticket shapes | [Parts II, VII](/docs/langgraph/state-nodes-edges) |
| **Context & knowledge** | Corpus split by trust; SQL for aggregation; graph for account relationships; per-account retrieval filters | [Parts III–V](/docs/retrieval/where-does-the-answer-live) |
| **Durable execution** | Runs that survive deploys and pause days for approval; compensation for a credit that must be unwound | [Parts X–XII](/docs/durability/agent-as-workflow) |
| **Tools & data** | Five tool classes, argument-scoped, idempotent, with paired reads | [Parts VI, VIII, IX](/docs/tools/read-and-write-tools) |
| **Evals** | The twenty-ticket set grown into a real dataset; CI gate; online scoring | [Part XIV](/docs/evals/eval-discipline) |
| **Observability** | Eight-question traces, `config_hash`, cost attribution, drift canary | [Part XV](/docs/observability/what-a-trace-must-answer) |
| **Reliability** | Retry at one layer, breakers, admission control, dedup window sized to the approval pause | [Part XVI](/docs/reliability/error-taxonomy) |
| **Security** | Taint from ticket bodies, delegation, tenancy below the app, argument scoping | [Part XVII](/docs/security/threat-model) |
...
**1 · Does a run outlive one request?** Yes. Tier-1 credits wait for a human, and the human might be asleep. → **Tier 3**, [durable execution](/docs/production/reference-architecture).

**2 · Does a run have effects the world can see?** Yes: `issue_credit` and `send_reply`. → idempotency, compensation, approval gates, and a [dispatcher choke point](/docs/production/reference-architecture) from the first write.

**3 · Is it multi-tenant?** Yes, Meridian's eleven customer accounts, and one of them is a competitor of another. → tenancy [enforced below the application](/docs/security/tenant-isolation), not in the query builder.

**4 · Is the input attacker-controlled?** Yes, entirely. A ticket body is written by anyone who can email support. → [taint at the fetcher](/docs/security/prompt-injection), argument scoping, and a capability ceiling for tainted runs.
...
The honest way to state the project is not *"make Atlas better."* It is **"replace a human watching every action with machinery that does the same job."** That framing puts the [approval gates](/docs/human-in-the-loop/approval-gates), the invariants, and the escalation contract on the critical path instead of the backlog, where the pilot's success would otherwise have put them.
</Aside>

## The scope negotiation

Meridian asked for the whole support org. Here is the counter-proposal, and the reasoning is [the answering-versus-acting tension](/docs/first-agent/the-problem) from the original brief, now with numbers attached.
...
**Invariants**, enforced in code, never measured. These nine are Atlas's *acceptance* invariants, a different list from [the six reliability invariants every fault injection asserts](/docs/reliability/failure-injection):

| Invariant | Enforced by |
| --- | --- |
| No cross-account data, ever | [Row-level security + per-tenant namespaces](/docs/security/tenant-isolation) |
| No credit above the tier-0 cap unassisted | [Argument scoping in the dispatcher](/docs/security/least-privilege) |
...
| **A tainted run cannot reach a class ④–⑤ tool** | [Taint ceiling](/docs/security/prompt-injection) |
| **No recipient argument on `send_reply`** | Resolved from the ticket contact server-side |
| Every effect idempotent, with a window ≥ the approval SLA | [Dedup table, 30 days](/docs/reliability/idempotency-in-practice) |
| Every action names user, agent, and run | [Delegation tokens](/docs/security/identity) |
...
The order that feels natural is capability-first: get it answering well, then add gates, then add measurement, then add security. The order that works is the [ninety-day sequence](/docs/production/operating-manual), **see it, bound it, measure it, break it**, because each stage makes the next one assessable.
...
Next: [The Atlas Lab Environment](/docs/capstone/lab-environment), the executable companion, shared fixtures, and the invariants both language tracks must preserve.
