...
You have the trace. It has all eight answers [the trace chapter](/book/observability/what-a-trace-must-answer) demanded: the prompt, the model, the tools, the tokens, the money, the timing, and where it went sideways. That was step nine, where the agent read a policy chunk and concluded it lacked authority.
...
The retrieval row is the one most often half-done. Recording "we retrieved 5 chunks" makes the run unreplayable, because the corpus has since been re-indexed and the same query now returns different text. Record the IDs *and* the content, and note that this is the same instrumentation [incident scoping needed](/book/security/untrusted-retrieval) to answer "which runs saw the poisoned document." Two requirements, one field.

<Aside type="note" title="If you are not on durable execution">
You do not need Temporal for this. A `run_events` table with `(run_id, seq, kind, request, response)` appended by the [dispatcher and the model gateway](/book/production/reference-architecture) gives you the same capability, and it is a day of work.

What you cannot do is reconstruct it later. This is the [tracing argument](/book/production/reference-architecture) again: last Tuesday's run either recorded its boundaries or it is permanently unreplayable, and no deploy today changes that.
</Aside>
...
**Two checks, because neither can see the other's failure.** `promptDelta` compares prompts, so it catches the code change that rebuilt the question. It is blind to a model upgrade, or to a change in whatever knob shapes generation on that model, which alter no prompt text at all and make every recorded response stale at once. On current frontier models that knob is [`effort` rather than temperature](/book/foundations/nondeterminism), which changes what you compare and not whether you have to. A harness that only compares prompts will replay last month's recording against a new model and report a clean pass. That is why the harness checks the serving slice separately, once, and exactly: [the `config_hash` on the run row](/book/production/versioning) is what turns that check into a lookup rather than an archaeology exercise.
...
**5 · Promote.** The fixture joins the [regression corpus](/book/evals/datasets), and every future commit replays it.
...
The [dataset chapter's](/book/evals/datasets) caveats apply on the way in: these fixtures contain customer data, so scrub and pseudonymize at extraction. The [eval-set row](/book/security/pii-and-compliance) in the compliance table is exactly this pipeline.
...
| Tighten [argument scoping](/book/security/least-privilege) | Replay | The false-positive rate on real traffic, before enforcing |
| Change the prompt | **Re-run** | Replay is invalid; the recorded responses are stale |
| Upgrade the model | **Re-run** | Same |

Row three is the single most useful application in this chapter. [The least-privilege chapter](/book/security/least-privilege) asked for shadow mode before enforcing a mined policy; replay is how you get that answer in an afternoon instead of a week, against traffic that already happened, with no risk to anyone.
...
Replay answers what a change would have done to one outcome. What the same change costs on every run afterwards is invisible to it. Next: [Token and Cost Accounting](/book/observability/cost-accounting), and the dashboard that stops a bad deploy becoming a bad invoice.
