...
[Part XIV](/book/evals/online-evals) can tell you that Atlas got ticket #8823 wrong. It cannot tell you why, and the difference between those two capabilities is the whole of this part.
...
An agent trace has to answer a different question, **why did it decide that**, and answering it requires reconstructing not just the request but [the accumulated state that shaped it](/book/context/context-as-budget).
...
| 5 | **What came back?** | Results, **and whether they were truncated** | [Silent truncation](/book/tools/result-design) reads as a model failure |
...
**Row 2** matters because the model can change underneath you without a deploy on your side, [which is its own chapter](/book/observability/drift), and which is undiagnosable if the trace records `claude-opus-5` and nothing about which served version answered.
...
**Keep 100% of** escalations, errors, [guardrail blocks](/book/evals/online-evals), cost and duration outliers, runs with a repeated identical error, and anything on a route that changed this week. **Sample the rest.**

That is deliberately the same stratification as [online eval sampling](/book/evals/online-evals), and it should be, because they are the same runs. A trace you kept and a run you scored being different sets is how a team ends up with a finding they cannot investigate.
...
The trace has to be joinable to the [event history](/book/durability/agent-as-workflow), the [eval score](/book/evals/online-evals), the [audit record](/book/human-in-the-loop/escalation-and-audit), and the ticket itself. Otherwise 3am becomes an exercise in matching timestamps across four systems.

The good news is that [Part XI already made this free](/book/durability/not-a-background-job): the workflow ID *is* the business entity. Use `ticket-8823` as the trace correlation ID and every system lines up without a mapping table, the same decision paying off for the fourth time.

<Aside type="caution" title="This is the third copy of your customers' data">
Worth counting, because no single team usually does.

The [event history](/book/durability/agent-as-workflow) is a durable record of every payload passed through it. The [golden set](/book/evals/datasets) is a second copy, harvested from production. The trace payload store is a third, and it is the one with the most data, the loosest access controls, and the retention policy nobody set.

**Three copies, three lifetimes, and usually only one of them has been thought about.** Scrub and mask at the instrumentation layer, before content reaches any backend, and decide the retention deliberately. It is the same conversation [Part XII](/book/human-in-the-loop/escalation-and-audit) forced for the audit trail, arriving with a larger volume and less ceremony.
</Aside>
...
Eight questions, and nothing so far says what to call the fields that answer them. Next: [OpenTelemetry for GenAI](/book/observability/otel-genai), on the names everybody else has already agreed to use.
