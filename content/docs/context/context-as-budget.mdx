...
[Part I](/docs/foundations/context-window) established that the context window is a budget you re-spend on every request. This part is about spending it on purpose.
...
Five well-reranked chunks beat twenty raw ones, not only on cost, but on accuracy. This is why [reranking](/docs/retrieval/hybrid-and-reranking) earns its latency, and why "just increase `k`" is a fix that measures well in a retrieval eval and badly in an end-to-end one.
...
At step 1, almost everything you pay for is fixed overhead, which is why [caching](/docs/foundations/prompt-caching) that prefix is the highest-leverage thing available, and why trimming tool descriptions pays off on every single request forever.

By step 12, history dominates, and no amount of schema trimming helps. That is the regime where [compaction](/docs/context/compaction) is the only lever that matters. Applying the late-run fix to an early-run problem, or vice versa, is the most common wasted optimization in this field.
...
Next: [The System Prompt](/docs/context/system-prompt), what belongs in it, what does not, and why the instructions you inherited from an older model now misfire.
