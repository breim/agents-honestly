...
[Part I](/docs/foundations/context-window) put poisoning alongside overflow and dilution: a wall, a slope, and a feedback loop. Here is the mechanism behind the third.
...
| --- | --- | --- |
| **Semantic** | Meaning shifts a little on each pass | Repeated summarization ([compaction](/docs/context/compaction)) |
...
The third is the terminal state, and it is the one that escapes a single run. Once an unverified claim gets written to a [fact store](/docs/context/memory), it stops being a poisoned turn and becomes poisoned data, which is why the previous chapter insisted you never write the model's inferences.
...
| **Injection** | Text in a retrieved document written to be read as instruction ([Part XVII](/docs/security/prompt-injection)) |
...
You cannot prompt your way out of a corrupted input. This is the same lesson as the [invariant-versus-request](/docs/context/system-prompt) distinction, arriving from a different direction: if it must not happen, it has to be structural.
...
Next: [Where Does the Answer Live?](/docs/retrieval/where-does-the-answer-live), the routing decision that precedes every retrieval choice.
