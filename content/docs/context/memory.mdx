...
**The knowledge base** is Meridian's policy corpus. It is not memory at all. It exists whether or not anyone ever talks to Atlas, and it is [Part IV's subject](/docs/retrieval/where-does-the-answer-live) rather than this chapter's.
...
That works until it doesn't, for two reasons already established. The transcript grows without bound, so it eventually gets [compacted](/docs/context/compaction) and your state is summarized away. And the transcript is [poisonable](/docs/context/context-poisoning): a wrong intermediate conclusion sits there as established fact for every subsequent step.
...
Three properties that the transcript version doesn't have: it is typed, so a malformed conclusion fails at the boundary rather than propagating; it is small, so it survives compaction; and it is *yours*, so the model can read it but cannot silently rewrite it. That is the [structured scratchpad](/docs/patterns/context/structured-scratchpad), and it is the single highest-value pattern in this part of the book.
...
Next: [Compaction and Summarization](/docs/context/compaction), rewriting history so the agent can keep going, without losing the thing it needed.
