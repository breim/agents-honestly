...
| "Respond ONLY with valid JSON, no markdown" + a parser + a retry loop | [Structured output](/book/foundations/structured-output); the *surrounding code* is cruft too |
...
**It must be frozen.** No timestamps, no user names, no session IDs, no `if (flag)` sections. From [prompt caching](/book/foundations/prompt-caching): the system prompt sits near the front of the prefix, so interpolating anything per-request invalidates the cache for the entire conversation behind it. Dynamic context goes later, in the messages.
...
**It is not free.** Every token here is billed on every request of every step of every conversation. Two hundred words is nothing; two thousand is a real line in the [budget](/book/context/context-as-budget) and should have earned its place.
...
That second property matters more than the caching one. The older workaround was to inject the instruction as text inside a user turn, and text inside a user turn can be forged by anything that writes into user-visible content: a retrieved document, a tool result, a customer's message. A genuine system role cannot be spoofed by content. It is the difference between an operator channel and a convention, and [prompt injection](/book/security/prompt-injection) is the chapter about why that difference is load-bearing.
...
**Measurement.** Remove one thing at a time and run the [ticket set](/book/first-agent/the-problem). Not because the change is risky, but because "the model told me it doesn't need that instruction" is not a measurement, and neither is one passing run.
...
That leaves the system prompt frozen and small, with everything a particular request needs still outside it. Next: [Dynamic Context Selection](/book/context/dynamic-selection), on choosing per request instead of shipping everything every time.
