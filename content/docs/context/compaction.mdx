...
History is the claimant that always wins. Every other row in the [budget](/book/context/context-as-budget) is bounded by something: a constant, a `k`, a truncation at the tool boundary. History just grows, and on a long enough run it consumes the window regardless of how disciplined everything else is.
...
That third one is the most damaging and the least obvious, because it doesn't look like a failure. The agent lost track of the *goal*, retained enough to sound coherent, and wrapped up. In a support queue that is a ticket marked resolved that wasn't, precisely the outcome the [acceptance spec](/book/first-agent/the-problem) forbade.
...
Which brings us to the mechanism that makes the rest of this manageable, and it is the payoff of the [last chapter](/book/context/memory).
...
**Losing tenancy.** If your compaction is a model call, it sees the whole conversation. Make sure the summarizer is subject to the same [authorization](/book/security/tenant-isolation) as everything else, and that the summary can't carry another customer's data forward.
...
Compaction assumes the history is worth compressing. Next: [Context Poisoning](/book/context/context-poisoning), where a wrong number went in at step four and every step since has been reasoning from it.
